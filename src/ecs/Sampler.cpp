...
#include "../include/ecs/Sampler.hpp"
...
namespace fgengine {
	namespace bindable
	{
		Sampler::Sampler(Type type, bool reflect) : m_Type(type), m_bReflect(reflect)
		{
			...
			m_pSamplerState = renderer::Renderer::GetDevice()->CreateSamplerState(*reinterpret_cast<renderer::SAMPLER_DESC*>(&samplerDesc));
		}
		...
		{
			renderer::Renderer::GetRenderContext()->BindSamplerState(m_pSamplerState, 0);
		}
	}
}