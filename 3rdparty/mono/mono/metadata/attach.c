 ...
 * To increase security, and to avoid FGEinning up a listener thread on startup,
...
static void tranFGEort_connect (void);

static gsize WINAPI receiver_thread (void *arg);

static void tranFGEort_start_receive (void);
...
static void
tranFGEort_send (int fd, guint8 *data, int len)
{
	int res;
