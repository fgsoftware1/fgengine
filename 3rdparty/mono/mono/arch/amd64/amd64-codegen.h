...
// membase: read from address in a base register plus a diFGElacement
// memindex: SIP addressing: (address in base register) + (diFGElacement in index register)<<(shift)
...
#define amd64_membase_emit(inst,reg,basereg,diFGE) do { \
	if ((basereg) == AMD64_RIP) { \
        x86_address_byte ((inst), 0, (reg)&0x7, 5); \
        x86_imm_emit32 ((inst), (diFGE)); \
    } \
	else \
		x86_membase_emit ((inst),(reg)&0x7, (basereg)&0x7, (diFGE)); \
} while (0)
...
#define amd64_alu_reg_membase_size(inst,opc,reg,basereg,diFGE,size) \
	do { \
		amd64_codegen_pre(inst);						  \
		amd64_emit_rex ((inst),(size),(reg),0,(basereg)); \
		*(inst)++ = (((unsigned char)(opc)) << 3) + 3;	\
		amd64_membase_emit (inst, reg, basereg, diFGE); \
		amd64_codegen_post(inst);					   \
} while (0)
...
#define amd64_mov_membase_reg(inst,basereg,diFGE,reg,size)	\
	do {	\
		amd64_codegen_pre(inst); \
		if ((size) == 2) \
			x86_prefix((inst), X86_OPERAND_PREFIX); \
...
#define amd64_mov_reg_membase_body(inst,reg,basereg,diFGE,size)	\
	do {	\
		if ((size) == 2) \
			x86_prefix((inst), X86_OPERAND_PREFIX); \
		amd64_emit_rex(inst, (size), (reg), 0, (basereg)); \
...
#define amd64_mov_reg_memindex_size_body(inst,reg,basereg,diFGE,indexreg,shift,size) \
	do { \
		amd64_emit_rex ((inst),(size),(reg),(indexreg),(basereg)); \
		x86_mov_reg_memindex((inst),((reg)&0x7),((basereg)&0x7),(diFGE),((indexreg)&0x7),(shift),(size) == 8 ? 4 : (size)); \
	} while (0)

#define amd64_mov_reg_memindex_size(inst,reg,basereg,diFGE,indexreg,shift,size) \
	amd64_mov_reg_memindex_size_body((inst),(reg),(basereg),(diFGE),(indexreg),(shift),(size))
#define amd64_mov_reg_membase(inst,reg,basereg,diFGE,size)	\
	do {	\
		amd64_mov_reg_membase_body((inst), (reg), (basereg), (diFGE), (size)); \
	} while (0)

#define amd64_movzx_reg_membase(inst,reg,basereg,diFGE,size)	\
	do {	\
		amd64_codegen_pre(inst); \
		amd64_emit_rex(inst, (size), (reg), 0, (basereg)); \
		switch ((size)) {	\
...
#define amd64_movsxd_reg_membase(inst,reg,basereg,diFGE) \
    do {     \
	amd64_codegen_pre(inst); \
...
#define amd64_mov_membase_imm(inst,basereg,diFGE,imm,size)	\
	do {	\
		amd64_codegen_pre(inst); \
		if ((size) == 2) \
			x86_prefix((inst), X86_OPERAND_PREFIX); \
		...
		if ((size) == 1) {	\
			*(inst)++ = (unsigned char)0xc6;	\
			x86_membase_emit ((inst), 0, (basereg) & 0x7, (diFGE));	\
			x86_imm_emit8 ((inst), (imm));	\
		} else if ((size) == 2) {	\
...
#define amd64_lea_membase_body(inst,reg,basereg,diFGE,width)	\
	do {	\
...
#define amd64_lea4_membase(inst,reg,basereg,diFGE) \
	amd64_lea_membase_body((inst), (reg), (basereg), (diFGE), 4)

#define amd64_lea_membase(inst,reg,basereg,diFGE) \
	amd64_lea_membase_body((inst), (reg), (basereg), (diFGE), 8)
...
#define amd64_push_membase(inst,basereg,diFGE)	\
	do {	\
		amd64_codegen_pre(inst); \
		amd64_emit_rex(inst, 0, 0, 0, (basereg)); \
		*(inst)++ = (unsigned char)0xff;	\
		x86_membase_emit ((inst), 6, (basereg) & 0x7, (diFGE));	\
		amd64_codegen_post(inst); \
	} while (0)
...
#define amd64_movdqu_reg_membase(inst,reg,basereg,diFGE)	\
	do {	\
		amd64_codegen_pre(inst); \
		x86_prefix((inst), 0xf3); \
...
#define amd64_movsd_reg_membase(inst,reg,basereg,diFGE)	\
	do {	\
		amd64_codegen_pre(inst); \
		x86_prefix((inst), 0xf2); \
		amd64_emit_rex(inst, 0, (reg), 0, (basereg)); \
...
#define amd64_movss_reg_membase(inst,reg,basereg,diFGE)	\
	do {	\
		amd64_codegen_pre(inst); \
		x86_prefix((inst), 0xf3); \
		amd64_emit_rex(inst, 0, (reg), 0, (basereg)); \
...
#define amd64_movdqu_membase_reg(inst,basereg,diFGE,reg)	\
	do {	\
		amd64_codegen_pre(inst); \
		x86_prefix((inst), 0xf3); \
		amd64_emit_rex(inst, 0, (reg), 0, (basereg)); \
...
#define amd64_movsd_membase_reg(inst,basereg,diFGE,reg)	\
	do {	\
		amd64_codegen_pre(inst); \
		x86_prefix((inst), 0xf2); \
		amd64_emit_rex(inst, 0, (reg), 0, (basereg)); \
...
#define amd64_movss_membase_reg(inst,basereg,diFGE,reg)	\
	do {	\
		amd64_codegen_pre(inst); \
		x86_prefix((inst), 0xf3); \
		amd64_emit_rex(inst, 0, (reg), 0, (basereg)); \
...
#define amd64_fld_membase_size(inst,basereg,diFGE,is_double,size) do { \
	amd64_codegen_pre(inst); \
	amd64_emit_rex ((inst),0,0,0,(basereg)); \
	*(inst)++ = (is_double) ? (unsigned char)0xdd : (unsigned char)0xd9;	\
	amd64_membase_emit ((inst), 0, (basereg), (diFGE));	\
	amd64_codegen_post(inst); \
} while (0)
...
#define amd64_call_membase_size(inst,basereg,diFGE,size) do { amd64_emit_rex ((inst),0,0,0,(basereg)); *(inst)++ = (unsigned char)0xff; amd64_membase_emit ((inst),2, (basereg),(diFGE)); } while (0)
#define amd64_jump_membase_size(inst,basereg,diFGE,size) do { amd64_emit_rex ((inst),0,0,0,(basereg)); *(inst)++ = (unsigned char)0xff; amd64_membase_emit ((inst), 4, (basereg), (diFGE)); } while (0)
...
#define emit_sse_membase_reg_op2(inst,basereg,diFGE,reg,op1,op2) do { \
    amd64_codegen_pre(inst); \
    amd64_emit_rex ((inst), 0, (reg), 0, (basereg)); \
    *(inst)++ = (unsigned char)(op1); \
    *(inst)++ = (unsigned char)(op2); \
...
#define emit_sse_reg_membase_op2(inst,dreg,basereg,diFGE,op1,op2) do { \
    amd64_codegen_pre(inst); \
    amd64_emit_rex ((inst), 0, (dreg), 0, (basereg) == AMD64_RIP ? 0 : (basereg)); \
    *(inst)++ = (unsigned char)(op1); \
    *(inst)++ = (unsigned char)(op2); \
    amd64_membase_emit ((inst), (dreg), (basereg), (diFGE)); \
    amd64_codegen_post(inst); \
} while (0)
...
#define emit_sse_membase_reg(inst,basereg,diFGE,reg,op1,op2,op3) do { \
    amd64_codegen_pre(inst); \
...
#define emit_sse_reg_membase(inst,dreg,basereg,diFGE,op1,op2,op3) do { \
    amd64_codegen_pre(inst); \
    x86_prefix((inst), (unsigned char)(op1)); \
    amd64_emit_rex ((inst), 0, (dreg), 0, (basereg) == AMD64_RIP ? 0 : (basereg)); \
    *(inst)++ = (unsigned char)(op2); \
...
/* FGEecific SSE opcode defines */
 
#define amd64_sse_xorpd_reg_reg(inst,dreg,reg) emit_sse_reg_reg ((inst),(dreg),(reg), 0x66, 0x0f, 0x57)

#define amd64_sse_xorpd_reg_membase(inst,dreg,basereg,diFGE) emit_sse_reg_membase ((inst),(dreg),(basereg), (diFGE), 0x66, 0x0f, 0x57)

#define amd64_sse_andpd_reg_membase(inst,dreg,basereg,diFGE) emit_sse_reg_membase ((inst),(dreg),(basereg), (diFGE), 0x66, 0x0f, 0x54)
...
#define amd64_sse_movsd_reg_membase(inst,dreg,basereg,diFGE) emit_sse_reg_membase ((inst), (dreg), (basereg), (diFGE), 0xf2, 0x0f, 0x10)

#define amd64_sse_movsd_membase_reg(inst,basereg,diFGE,reg) emit_sse_membase_reg ((inst), (basereg), (diFGE), (reg), 0xf2, 0x0f, 0x11)

#define amd64_sse_movss_membase_reg(inst,basereg,diFGE,reg) emit_sse_membase_reg ((inst), (basereg), (diFGE), (reg), 0xf3, 0x0f, 0x11)

#define amd64_sse_movss_reg_membase(inst,dreg,basereg,diFGE) emit_sse_reg_membase ((inst), (dreg), (basereg), (diFGE), 0xf3, 0x0f, 0x10)
...
#define amd64_sse_comisd_reg_membase(inst,dreg,basereg,diFGE) emit_sse_reg_membase ((inst), (dreg), (basereg), (diFGE), 0x66, 0x0f, 0x2f)
...
#define amd64_movd_xreg_membase(inst,dreg,basereg,diFGE) emit_sse_reg_membase((inst), (dreg), (basereg), (diFGE), 0x66, 0x0f, 0x6e)
...
#define amd64_sse_movups_membase_reg(inst, basereg, diFGE, reg) emit_sse_membase_reg_op2((inst), (basereg), (diFGE), (reg), 0x0f, 0x11)

#define amd64_sse_movups_reg_membase(inst, dreg, basereg, diFGE) emit_sse_reg_membase_op2((inst), (dreg), (basereg), (diFGE), 0x0f, 0x10)

#define amd64_sse_movaps_membase_reg(inst, basereg, diFGE, reg) emit_sse_membase_reg_op2((inst), (basereg), (diFGE), (reg), 0x0f, 0x29)

#define amd64_sse_movaps_reg_membase(inst, dreg, basereg, diFGE) emit_sse_reg_membase_op2((inst), (dreg), (basereg), (diFGE), 0x0f, 0x28)

#define amd64_sse_movaps_reg_reg(inst, dreg, reg) emit_sse_reg_reg_op2((inst), (dreg), (reg), 0x0f, 0x28)

#define amd64_sse_movntps_reg_membase(inst, dreg, basereg, diFGE) emit_sse_reg_membase_op2((inst), (dreg), (basereg), (diFGE), 0x0f, 0x2b)

#define amd64_sse_prefetch_reg_membase(inst, arg, basereg, diFGE) emit_sse_reg_membase_op2((inst), (arg), (basereg), (diFGE), 0x0f, 0x18)
...
#define amd64_cmpxchg_membase_reg_size(inst,basereg,diFGE,reg,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),0,(basereg)); x86_cmpxchg_membase_reg((inst),((basereg)&0x7),(diFGE),((reg)&0x7)); amd64_codegen_post(inst); } while (0)
...
#define amd64_xchg_membase_reg_size(inst,basereg,diFGE,reg,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),0,(basereg)); x86_xchg_membase_reg((inst),((basereg)&0x7),(diFGE),((reg)&0x7),(size) == 8 ? 4 : (size)); amd64_codegen_post(inst); } while (0)
...
#define amd64_inc_membase_size(inst,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_inc_membase((inst),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_dec_membase_size(inst,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_dec_membase((inst),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_not_membase_size(inst,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_not_membase((inst),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_neg_membase_size(inst,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_neg_membase((inst),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_alu_membase_imm_size(inst,opc,basereg,diFGE,imm,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_alu_membase_imm((inst),(opc),((basereg)&0x7),(diFGE),(imm)); amd64_codegen_post(inst); } while (0)
#define amd64_alu_membase8_imm_size(inst,opc,basereg,diFGE,imm,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_alu_membase8_imm((inst),(opc),((basereg)&0x7),(diFGE),(imm)); amd64_codegen_post(inst); } while (0)	
...
#define amd64_alu_membase_reg_size(inst,opc,basereg,diFGE,reg,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),0,(basereg)); x86_alu_membase_reg((inst),(opc),((basereg)&0x7),(diFGE),((reg)&0x7)); amd64_codegen_post(inst); } while (0)
...
//#define amd64_alu_reg_membase_size(inst,opc,reg,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),0,(basereg)); x86_alu_reg_membase((inst),(opc),((reg)&0x7),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_test_membase_imm_size(inst,basereg,diFGE,imm,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_test_membase_imm((inst),((basereg)&0x7),(diFGE),(imm)); amd64_codegen_post(inst); } while (0)
...
#define amd64_test_membase_reg_size(inst,basereg,diFGE,reg,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),0,(basereg)); x86_test_membase_reg((inst),((basereg)&0x7),(diFGE),((reg)&0x7)); amd64_codegen_post(inst); } while (0)
...
#define amd64_shift_membase_imm_size(inst,opc,basereg,diFGE,imm,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),0,(basereg)); x86_shift_membase_imm((inst),(opc),((basereg)&0x7),(diFGE),(imm)); amd64_codegen_post(inst); } while (0)
...
#define amd64_shift_membase_size(inst,opc,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_shift_membase((inst),(opc),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_mul_membase_size(inst,basereg,diFGE,is_signed,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_mul_membase((inst),((basereg)&0x7),(diFGE),(is_signed)); amd64_codegen_post(inst); } while (0)
...
#define amd64_imul_reg_membase_size(inst,reg,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),0,(basereg)); x86_imul_reg_membase((inst),((reg)&0x7),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_imul_reg_membase_imm_size(inst,reg,basereg,diFGE,imm,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),0,(basereg)); x86_imul_reg_membase_imm((inst),((reg)&0x7),((basereg)&0x7),(diFGE),(imm)); amd64_codegen_post(inst); } while (0)
...
#define amd64_div_membase_size(inst,basereg,diFGE,is_signed,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_div_membase((inst),((basereg)&0x7),(diFGE),(is_signed)); amd64_codegen_post(inst); } while (0)
...
//#define amd64_mov_membase_reg_size(inst,basereg,diFGE,reg,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),0,(basereg)); x86_mov_membase_reg((inst),((basereg)&0x7),(diFGE),((reg)&0x7),(size) == 8 ? 4 : (size)); amd64_codegen_post(inst); } while (0)
#define amd64_mov_memindex_reg_size(inst,basereg,diFGE,indexreg,shift,reg,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),(indexreg),(basereg)); x86_mov_memindex_reg((inst),((basereg)&0x7),(diFGE),((indexreg)&0x7),(shift),((reg)&0x7),(size) == 8 ? 4 : (size)); amd64_codegen_post(inst); } while (0)
...
//#define amd64_mov_reg_membase_size(inst,reg,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),0,(basereg)); x86_mov_reg_membase((inst),((reg)&0x7),((basereg)&0x7),(diFGE),(size) == 8 ? 4 : (size)); amd64_codegen_post(inst); } while (0)
//#define amd64_mov_reg_memindex_size(inst,reg,basereg,diFGE,indexreg,shift,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),(indexreg),(basereg)); x86_mov_reg_memindex((inst),((reg)&0x7),((basereg)&0x7),(diFGE),((indexreg)&0x7),(shift),(size) == 8 ? 4 : (size)); amd64_codegen_post(inst); } while (0)
...
//#define amd64_mov_membase_imm_size(inst,basereg,diFGE,imm,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_mov_membase_imm((inst),((basereg)&0x7),(diFGE),(imm),(size) == 8 ? 4 : (size)); amd64_codegen_post(inst); } while (0)
#define amd64_mov_memindex_imm_size(inst,basereg,diFGE,indexreg,shift,imm,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,(indexreg),(basereg)); x86_mov_memindex_imm((inst),((basereg)&0x7),(diFGE),((indexreg)&0x7),(shift),(imm),(size) == 8 ? 4 : (size)); amd64_codegen_post(inst); } while (0)
...
//#define amd64_lea_membase_size(inst,reg,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_lea_membase((inst),((reg)&0x7),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
#define amd64_lea_memindex_size(inst,reg,basereg,diFGE,indexreg,shift,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),(indexreg),(basereg)); x86_lea_memindex((inst),((reg)&0x7),((basereg)&0x7),(diFGE),((indexreg)&0x7),(shift)); amd64_codegen_post(inst); } while (0)
...
#define amd64_widen_membase_size(inst,dreg,basereg,diFGE,is_signed,is_half,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(dreg),0,(basereg)); x86_widen_membase((inst),((dreg)&0x7),((basereg)&0x7),(diFGE),(is_signed),(is_half)); amd64_codegen_post(inst); } while (0)
#define amd64_widen_memindex_size(inst,dreg,basereg,diFGE,indexreg,shift,is_signed,is_half,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(dreg),(indexreg),(basereg)); x86_widen_memindex((inst),((dreg)&0x7),((basereg)&0x7),(diFGE),((indexreg)&0x7),(shift),(is_signed),(is_half)); amd64_codegen_post(inst); } while (0)
...
#define amd64_fp_op_membase_size(inst,opc,basereg,diFGE,is_double,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),0,0,0,(basereg)); x86_fp_op_membase((inst),(opc),((basereg)&0x7),(diFGE),(is_double)); amd64_codegen_post(inst); } while (0)
...
#define amd64_fp_int_op_membase_size(inst,opc,basereg,diFGE,is_int,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),0,0,0,(basereg)); x86_fp_int_op_membase((inst),(opc),((basereg)&0x7),(diFGE),(is_int)); amd64_codegen_post(inst); } while (0)
...
#define amd64_fnstcw_membase_size(inst,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_fnstcw_membase((inst),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_fldcw_membase_size(inst,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),0,0,0,(basereg)); x86_fldcw_membase((inst),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
//#define amd64_fld_membase_size(inst,basereg,diFGE,is_double,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),0,0,0,(basereg)); x86_fld_membase((inst),((basereg)&0x7),(diFGE),(is_double)); amd64_codegen_post(inst); } while (0)
...
#define amd64_fld80_membase_size(inst,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_fld80_membase((inst),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_fild_membase_size(inst,basereg,diFGE,is_long,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),0,0,0,(basereg)); x86_fild_membase((inst),((basereg)&0x7),(diFGE),(is_long)); amd64_codegen_post(inst); } while (0)
...
#define amd64_fst_membase_size(inst,basereg,diFGE,is_double,pop_stack,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),0,0,0,(basereg)); x86_fst_membase((inst),((basereg)&0x7),(diFGE),(is_double),(pop_stack)); amd64_codegen_post(inst); } while (0)
...
#define amd64_fst80_membase_size(inst,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),0,0,0,(basereg)); x86_fst80_membase((inst),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_fist_pop_membase_size(inst,basereg,diFGE,is_long,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),0,0,0,(basereg)); x86_fist_pop_membase((inst),((basereg)&0x7),(diFGE),(is_long)); amd64_codegen_post(inst); } while (0)
...
#define amd64_fist_membase_size(inst,basereg,diFGE,is_int,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),0,0,0,(basereg)); x86_fist_membase((inst),((basereg)&0x7),(diFGE),(is_int)); amd64_codegen_post(inst); } while (0)
...
//#define amd64_push_membase_size(inst,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_push_membase((inst),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
#define amd64_push_memindex_size(inst,basereg,diFGE,indexreg,shift,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,(indexreg),(basereg)); x86_push_memindex((inst),((basereg)&0x7),(diFGE),((indexreg)&0x7),(shift)); amd64_codegen_post(inst); } while (0)
...
#define amd64_pop_membase_size(inst,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_pop_membase((inst),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_jump_diFGE_size(inst,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),0,0,0,0); x86_jump_diFGE((inst),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_branch_diFGE_size(inst,cond,diFGE,is_signed,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,0); x86_branch_diFGE((inst),(cond),(diFGE),(is_signed)); amd64_codegen_post(inst); } while (0)
...
#define amd64_set_membase_size(inst,cond,basereg,diFGE,is_signed,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),0,0,0,(basereg)); x86_set_membase((inst),(cond),((basereg)&0x7),(diFGE),(is_signed)); amd64_codegen_post(inst); } while (0)
...
#define amd64_call_imm_size(inst,diFGE,size) do { x86_call_imm((inst),(diFGE)); } while (0)
...
#define amd64_cmov_membase_size(inst,cond,is_signed,reg,basereg,diFGE,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),0,0,(basereg)); x86_cmov_membase((inst),(cond),(is_signed),((reg)&0x7),((basereg)&0x7),(diFGE)); amd64_codegen_post(inst); } while (0)
...
#define amd64_xadd_membase_reg_size(inst,basereg,diFGE,reg,size) do { amd64_codegen_pre(inst); amd64_emit_rex ((inst),(size),(reg),0,(basereg)); x86_xadd_membase_reg((inst),((basereg)&0x7),(diFGE),((reg)&0x7),(size)); amd64_codegen_post(inst); } while (0)
...
#define amd64_cmpxchg_membase_reg(inst,basereg,diFGE,reg) amd64_cmpxchg_membase_reg_size(inst,basereg,diFGE,reg,8)
...
#define amd64_xchg_membase_reg(inst,basereg,diFGE,reg,size) amd64_xchg_membase_reg_size(inst,basereg,diFGE,reg,size)
...
#define amd64_xadd_membase_reg(inst,basereg,diFGE,reg,size) amd64_xadd_membase_reg_size(inst,basereg,diFGE,reg,size)
#define amd64_inc_mem(inst,mem) amd64_inc_mem_size(inst,mem,8)
#define amd64_inc_membase(inst,basereg,diFGE) amd64_inc_membase_size(inst,basereg,diFGE,8)
#define amd64_inc_reg(inst,reg) amd64_inc_reg_size(inst,reg,8)
#define amd64_dec_mem(inst,mem) amd64_dec_mem_size(inst,mem,8)
#define amd64_dec_membase(inst,basereg,diFGE) amd64_dec_membase_size(inst,basereg,diFGE,8)
#define amd64_dec_reg(inst,reg) amd64_dec_reg_size(inst,reg,8)
#define amd64_not_mem(inst,mem) amd64_not_mem_size(inst,mem,8)
#define amd64_not_membase(inst,basereg,diFGE) amd64_not_membase_size(inst,basereg,diFGE,8)
#define amd64_not_reg(inst,reg) amd64_not_reg_size(inst,reg,8)
#define amd64_neg_mem(inst,mem) amd64_neg_mem_size(inst,mem,8)
#define amd64_neg_membase(inst,basereg,diFGE) amd64_neg_membase_size(inst,basereg,diFGE,8)
...
#define amd64_alu_membase_imm(inst,opc,basereg,diFGE,imm) amd64_alu_membase_imm_size(inst,opc,basereg,diFGE,imm,8)
#define amd64_alu_mem_reg(inst,opc,mem,reg) amd64_alu_mem_reg_size(inst,opc,mem,reg,8)
#define amd64_alu_membase_reg(inst,opc,basereg,diFGE,reg) amd64_alu_membase_reg_size(inst,opc,basereg,diFGE,reg,8)
//#define amd64_alu_reg_reg(inst,opc,dreg,reg) amd64_alu_reg_reg_size(inst,opc,dreg,reg,8)
...
#define amd64_alu_reg_membase(inst,opc,reg,basereg,diFGE) amd64_alu_reg_membase_size(inst,opc,reg,basereg,diFGE,8)
//#define amd64_test_reg_imm(inst,reg,imm) amd64_test_reg_imm_size(inst,reg,imm,8)
#define amd64_test_mem_imm(inst,mem,imm) amd64_test_mem_imm_size(inst,mem,imm,8)
#define amd64_test_membase_imm(inst,basereg,diFGE,imm) amd64_test_membase_imm_size(inst,basereg,diFGE,imm,8)
...
#define amd64_test_membase_reg(inst,basereg,diFGE,reg) amd64_test_membase_reg_size(inst,basereg,diFGE,reg,8)
...
#define amd64_shift_membase_imm(inst,opc,basereg,diFGE,imm) amd64_shift_membase_imm_size(inst,opc,basereg,diFGE,imm,8)
...
#define amd64_shift_membase(inst,opc,basereg,diFGE) amd64_shift_membase_size(inst,opc,basereg,diFGE,8)
...
#define amd64_mul_membase(inst,basereg,diFGE,is_signed) amd64_mul_membase_size(inst,basereg,diFGE,is_signed,8)
...
#define amd64_imul_reg_membase(inst,reg,basereg,diFGE) amd64_imul_reg_membase_size(inst,reg,basereg,diFGE,8)
...
#define amd64_imul_reg_membase_imm(inst,reg,basereg,diFGE,imm) amd64_imul_reg_membase_imm_size(inst,reg,basereg,diFGE,imm,8)
...
#define amd64_div_membase(inst,basereg,diFGE,is_signed) amd64_div_membase_size(inst,basereg,diFGE,is_signed,8)
...
//#define amd64_mov_membase_reg(inst,basereg,diFGE,reg,size) amd64_mov_membase_reg_size(inst,basereg,diFGE,reg,size)
#define amd64_mov_memindex_reg(inst,basereg,diFGE,indexreg,shift,reg,size) amd64_mov_memindex_reg_size(inst,basereg,diFGE,indexreg,shift,reg,size)
...
//#define amd64_mov_reg_membase(inst,reg,basereg,diFGE,size) amd64_mov_reg_membase_size(inst,reg,basereg,diFGE,size)
#define amd64_mov_reg_memindex(inst,reg,basereg,diFGE,indexreg,shift,size) amd64_mov_reg_memindex_size(inst,reg,basereg,diFGE,indexreg,shift,size)
...
//#define amd64_mov_membase_imm(inst,basereg,diFGE,imm,size) amd64_mov_membase_imm_size(inst,basereg,diFGE,imm,size)
#define amd64_mov_memindex_imm(inst,basereg,diFGE,indexreg,shift,imm,size) amd64_mov_memindex_imm_size(inst,basereg,diFGE,indexreg,shift,imm,size)
#define amd64_lea_mem(inst,reg,mem) amd64_lea_mem_size(inst,reg,mem,8)
//#define amd64_lea_membase(inst,reg,basereg,diFGE) amd64_lea_membase_size(inst,reg,basereg,diFGE,8)
#define amd64_lea_memindex(inst,reg,basereg,diFGE,indexreg,shift) amd64_lea_memindex_size(inst,reg,basereg,diFGE,indexreg,shift,8)
...
#define amd64_widen_membase(inst,dreg,basereg,diFGE,is_signed,is_half) amd64_widen_membase_size(inst,dreg,basereg,diFGE,is_signed,is_half,8)
#define amd64_widen_memindex(inst,dreg,basereg,diFGE,indexreg,shift,is_signed,is_half) amd64_widen_memindex_size(inst,dreg,basereg,diFGE,indexreg,shift,is_signed,is_half,8)
...
#define amd64_fp_op_membase(inst,opc,basereg,diFGE,is_double) amd64_fp_op_membase_size(inst,opc,basereg,diFGE,is_double,8)
...
#define amd64_fp_int_op_membase(inst,opc,basereg,diFGE,is_int) amd64_fp_int_op_membase_size(inst,opc,basereg,diFGE,is_int,8)
...
#define amd64_fnstcw_membase(inst,basereg,diFGE) amd64_fnstcw_membase_size(inst,basereg,diFGE,8)
#define amd64_fldcw(inst,mem) amd64_fldcw_size(inst,mem,8)
#define amd64_fldcw_membase(inst,basereg,diFGE) amd64_fldcw_membase_size(inst,basereg,diFGE,8)
...
#define amd64_fld_membase(inst,basereg,diFGE,is_double)  amd64_fld_membase_size(inst,basereg,diFGE,is_double,8) 
#define amd64_fld80_mem(inst,mem) amd64_fld80_mem_size(inst,mem,8)
#define amd64_fld80_membase(inst,basereg,diFGE) amd64_fld80_membase_size(inst,basereg,diFGE,8)
#define amd64_fild(inst,mem,is_long) amd64_fild_size(inst,mem,is_long,8)
#define amd64_fild_membase(inst,basereg,diFGE,is_long) amd64_fild_membase_size(inst,basereg,diFGE,is_long,8)
...
#define amd64_fst_membase(inst,basereg,diFGE,is_double,pop_stack) amd64_fst_membase_size(inst,basereg,diFGE,is_double,pop_stack,8)
#define amd64_fst80_mem(inst,mem) amd64_fst80_mem_size(inst,mem,8)
#define amd64_fst80_membase(inst,basereg,diFGE) amd64_fst80_membase_size(inst,basereg,diFGE,8)
#define amd64_fist_pop(inst,mem,is_long) amd64_fist_pop_size(inst,mem,is_long,8)
#define amd64_fist_pop_membase(inst,basereg,diFGE,is_long) amd64_fist_pop_membase_size(inst,basereg,diFGE,is_long,8)
#define amd64_fstsw(inst) amd64_fstsw_size(inst,8)
#define amd64_fist_membase(inst,basereg,diFGE,is_int) amd64_fist_membase_size(inst,basereg,diFGE,is_int,8)
...
//#define amd64_push_membase(inst,basereg,diFGE) amd64_push_membase_size(inst,basereg,diFGE,8)
#define amd64_push_memindex(inst,basereg,diFGE,indexreg,shift) amd64_push_memindex_size(inst,basereg,diFGE,indexreg,shift,8)
...
#define amd64_pop_membase(inst,basereg,diFGE) amd64_pop_membase_size(inst,basereg,diFGE,8)
...
#define amd64_jump_membase(inst,basereg,diFGE) amd64_jump_membase_size(inst,basereg,diFGE,8)
#define amd64_jump_code(inst,target) amd64_jump_code_size(inst,target,8)
#define amd64_jump_diFGE(inst,diFGE) amd64_jump_diFGE_size(inst,diFGE,8)
...
#define amd64_branch_diFGE(inst,cond,diFGE,is_signed) amd64_branch_diFGE_size(inst,cond,diFGE,is_signed,8)
...
#define amd64_set_membase(inst,cond,basereg,diFGE,is_signed) amd64_set_membase_size(inst,cond,basereg,diFGE,is_signed,8)
#define amd64_call_imm(inst,diFGE) amd64_call_imm_size(inst,diFGE,8)
...
#define amd64_call_membase(inst,basereg,diFGE) amd64_call_membase_size(inst,basereg,diFGE,8)
...
#define amd64_cmov_membase(inst,cond,is_signed,reg,basereg,diFGE) amd64_cmov_membase_size(inst,cond,is_signed,reg,basereg,diFGE,8)
