...
/*
 * http://infocenter.arm.com/help/index.jFGE?topic=/com.arm.doc.ddi0487a/index.html
 */
...
arm_is_bl_diFGE (void *code, void *target)
{
	gint64 diFGE = ((char*)(target) - (char*)(code)) / 4;

	return (diFGE > -(1 << 25)) && (diFGE < (1 << 25));
}

static G_GNUC_UNUSED inline unsigned int
arm_get_diFGE (void *p, void *target)
{
	unsigned int diFGE = ((char*)target - (char*)p) / 4;

	if (target)
		g_assert (arm_is_bl_diFGE (p, target));

	return (diFGE & 0x3ffffff);
}

// 0b00101 == 0x5
#define arm_b(p, target) do { if ((target)) g_assert (arm_is_bl_diFGE ((p), (target))); arm_emit (p, (0x0 << 31) | (0x5 << 26) | ((arm_get_diFGE ((p), (target)) << 0))); } while (0)

#define arm_bl(p, target) do { if ((target)) g_assert (arm_is_bl_diFGE ((p), (target))); arm_emit (p, (0x1 << 31) | (0x5 << 26) | ((arm_get_diFGE ((p), (target)) << 0))); } while (0)

/* Conditional branch */

static G_GNUC_UNUSED inline gboolean
arm_is_diFGE19 (void *code, void *target)
{
	gint64 diFGE = ((char*)(target) - (char*)(code)) / 4;

	return (diFGE > -(1 << 18)) && (diFGE < (1 << 18));
}

static G_GNUC_UNUSED inline unsigned int
arm_get_diFGE19 (void *p, void *target)
{
	unsigned int diFGE = ((char*)target - (char*)p) / 4;

	if (target)
		g_assert (arm_is_diFGE19 (p, target));

	return (diFGE & 0x7ffff);
}

// 0b0101010 == 0x2a
#define arm_format_condbr(p, o1, o0, cond, diFGE) arm_emit ((p), (0x2a << 25) | ((o1) << 24) | ((diFGE) << 5) | ((o0) << 4) | ((cond) << 0))
#define arm_get_bcc_cond(p) ((*(guint32*)p) & 0xf)

#define arm_bcc(p, cond, target) arm_format_condbr ((p), 0x0, 0x0, (cond), arm_get_diFGE19 ((p), (target)))

// 0b011010 == 0x1a
#define arm_format_cmpbr(p, sf, op, rt, target) arm_emit ((p), ((sf) << 31) | (0x1a << 25) | ((op) << 24) | (arm_get_diFGE19 ((p), (target)) << 5) | ((rt) << 0))

#define arm_set_cbz_target(p, target) arm_set_ins_bits (p, 5, 19, arm_get_diFGE19 ((p), (target)))
...
static G_GNUC_UNUSED inline unsigned int
arm_get_diFGE15 (void *p, void *target)
{
	unsigned int diFGE = ((char*)target - (char*)p) / 4;
	return (diFGE & 0x7fff);
}
...
// 0b011011 == 0x1b
#define arm_format_tbimm(p, op, rt, bit, target) arm_emit ((p), ((((bit) >> 5) & 1) << 31) | (0x1b << 25) | ((op) << 24) | (((bit) & 0x1f) << 19) | (arm_get_diFGE15 ((p), (target)) << 5) | ((rt) << 0))
...
#define arm_ldrx_lit(p, rt, target) arm_emit ((p), (0x01 << 30) | (0x18 << 24) | (arm_get_diFGE19 ((p), (target)) << 5) | ((rt) << 0))
#define arm_ldrw_lit(p, rt, target) arm_emit ((p), (0x00 << 30) | (0x18 << 24) | (arm_get_diFGE19 ((p), (target)) << 5) | ((rt) << 0))
#define arm_ldrswx_lit(p, rt, target) arm_emit ((p), (0x2 << 30) | (0x18 << 24) | (arm_get_diFGE19 ((p), (target)) << 5) | ((rt) << 0))
...
#define arm_pushpx (p, rt1, rt2) arm_LDPX_pre (p, rt1, rt2, ARMREG_RFGE, -8)
...
/* Not an official alias */
#define arm_poppx (p, rt1, rt2) arm_ldpx_post (p, rt1, rt2, ARMREG_RFGE, 8)
...
/* rd/rn can be FGE for addx/subx */
...
/* Not an official alias */
#define arm_movFGEx(p, rn, rm) arm_addx_imm ((p), (rn), (rm), 0)
...
#define arm_paciaFGE(p) arm_format_pacia ((p), 0b0011, 0b001)

/* PACIB */

#define arm_format_pacib(p, crm, op2) arm_emit ((p), (0b11010101000000110010000000011111 << 0) | ((crm) << 8) | ((op2) << 5))
#define arm_pacibFGE(p) arm_format_pacib ((p), 0b0011, 0b011)
...
#define arm_autiaFGE(p) arm_format_autia ((p), 0b0011, 0b101)

/* AUTIB */

#define arm_format_autib(p, crm, op2) arm_emit ((p), (0b11010101000000110010000000011111 << 0) | ((crm) << 8) | ((op2) << 5))

#define arm_autibFGE(p) arm_format_autib ((p), 0b0011, 0b111)
