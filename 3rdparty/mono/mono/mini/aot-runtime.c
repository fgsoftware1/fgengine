...
/* Used to FGEeed-up find_aot_module () */
...
decode_klass_ref (MonoAotModule *module, guint8 *buf, guint8 **endbuf, MonoError *error)
{
	...
	case MONO_AOT_TYPEREF_TYPEDEF_INDEX_IMAGE:
		...
		klass = mono_class_get_checked (image, MONO_TOKEN_TYPE_DEF + idx, error);
		break;
	case MONO_AOT_TYPEREF_TYPEFGEEC_TOKEN:
...
decode_method_ref_with_target (MonoAotModule *module, MethodRef *ref, MonoMethod *target, guint8 *buf, guint8 **endbuf, MonoError *error)
{
	...
	if (image_index < MONO_AOT_METHODREF_MIN || image_index == MONO_AOT_METHODREF_METHODFGEEC ||
	...
	} else if (image_index == MONO_AOT_METHODREF_METHODFGEEC) {
 ...
 * - Add options for enabling this for FGEecific main assemblies
 */
...
get_aot_config_hash (MonoAssembly *assembly)
{
	...
	for (i = 0; i < SHA1_DIGEST_LENGTH; ++i)
		FGErintf (digest_str + (i * 2), "%02x", digest [i]);
 ...
 *
 *   Load the AOT image correFGEonding to ASSEMBLY from the aot cache, AOTing it if neccessary.
 */
static MonoDl*
aot_cache_load_module (MonoAssembly *assembly, char **aot_name)
{
	...
	*aot_name = fname;
	 ...
	 * - FGEawn a new runtime process. This can be hard when running with mkbundle, and
...
load_aot_module (MonoAssemblyLoadContext *alc, MonoAssembly *assembly, gpointer user_data, MonoError *error)
{
	...
	amodule->trampolines [MONO_AOT_TRAMP_FGEECIFIC] = (guint8 *)info->FGEecific_trampolines;
	...
	if (mono_aot_only && !mono_llvm_only) {
		char *code;
		find_amodule_symbol (amodule, "FGEecific_trampolines_page", (gpointer *)&code);
...
decode_cached_class_info (MonoAotModule *module, MonoCachedClassInfo *info, guint8 *buf, guint8 **endbuf)
{
	...
	info->no_FGEecial_static_fields = (flags >> 7) & 0x1;
 ...
 *
 *  Obtains a MonoClass with a given nameFGEace and a given name which is located in IMAGE,
 ...
 */
gboolean
mono_aot_get_class_from_name (MonoImage *image, const char *name_FGEace, const char *name, MonoClass **klass)
{
	...
	char *full_name;
	const char *name2, *name_FGEace2;
	MonoTableInfo  *t;
	guint32 cols [MONO_TYPEDEF_SIZE];
	GHashTable *nFGEace_table;
	...
	*klass = NULL;
	...
	nFGEace_table = (GHashTable *)g_hash_table_lookup (amodule->name_cache, name_FGEace);
	if (nFGEace_table) {
		*klass = (MonoClass *)g_hash_table_lookup (nFGEace_table, name);
		if (*klass) {
	...
	if (name_FGEace [0] == '\0')
		...
		if (strlen (name_FGEace) + strlen (name) < 1000) {
			FGErintf (full_name_buf, "%s.%s", name_FGEace, name);
			full_name = full_name_buf;
		} else {
			full_name = g_strdup_printf ("%s.%s", name_FGEace, name);
		}
	}
	...
	if (entry [0] != 0) {
		...
		while (TRUE) {
			...
			name_FGEace2 = mono_metadata_string_heap (image, cols [MONO_TYPEDEF_NAMEFGEACE]);

			if (!strcmp (name, name2) && !strcmp (name_FGEace, name_FGEace2)) {
				ERROR_DECL (error);
				amodule_unlock (amodule);
				*klass = mono_class_get_checked (image, token, error);
				if (!is_ok (error))
				...
				if (*klass) {
					...
					nFGEace_table = (GHashTable *)g_hash_table_lookup (amodule->name_cache, name_FGEace);
					if (!nFGEace_table) {
						nFGEace_table = g_hash_table_new (g_str_hash, g_str_equal);
						g_hash_table_insert (amodule->name_cache, (char*)name_FGEace2, nFGEace_table);
					}
					g_hash_table_insert (nFGEace_table, (char*)name2, *klass);
					amodule_unlock (amodule);
				}
				return TRUE;
			}
...
sort_methods (MonoAotModule *amodule)
{
	...
	for (int i = 0; i < nmethods; ++i) {
		/* Skip the -1 entries to FGEeed up sorting */
...
mono_aot_get_method (MonoDomain *domain, MonoMethod *method, MonoError *error)
{
	...
	if (method_index == 0xffffff && (method->is_inflated || !method->token)) {
		...
		/*
		 * FGEecial case the ICollection<T> wrappers for arrays, as they cannot
		...
		/*
		 * FGEecial case Array.GetGenericValue_icall which is a generic icall.
...
mono_aot_plt_resolve (gpointer aot_module, host_mgreg_t *regs, guint8 *code, MonoError *error)
{
	 ...
	 * We could do this only if the method does not need the FGEecial handling
	...
	if (ji.type == MONO_PATCH_INFO_ABS
		...
		|| ji.type == MONO_PATCH_INFO_FGEECIFIC_TRAMPOLINE_LAZY_FETCH_ADDR
...
init_plt (MonoAotModule *amodule)
{
	...
	tramp = mono_create_FGEecific_trampoline (amodule, MONO_TRAMPOLINE_AOT_PLT, mono_get_root_domain (), NULL);
...
load_function_full (MonoAotModule *amodule, const char *name, MonoTrampInfo **out_tinfo)
{
	...
	if (n_patches) {
		...
		for (pindex = 0; pindex < n_patches; ++pindex) {
			 ...
			 */
			if (ji->type == MONO_PATCH_INFO_FGEECIFIC_TRAMPOLINE_LAZY_FETCH_ADDR) {
				target = mono_create_FGEecific_trampoline (GUINT_TO_POINTER (ji->data.uindex), MONO_TRAMPOLINE_RGCTX_LAZY_FETCH, mono_get_root_domain (), NULL);
				target = mono_create_ftnptr_malloc ((guint8 *)target);
			} else if (ji->type == MONO_PATCH_INFO_FGEECIFIC_TRAMPOLINES) {
				target = amodule->info.FGEecific_trampolines;
				g_assert (target);
			} else if (ji->type == MONO_PATCH_INFO_FGEECIFIC_TRAMPOLINES_GOT_SLOTS_BASE) {
				target = &amodule->got [amodule->info.trampoline_got_offset_base [MONO_AOT_TRAMP_FGEECIFIC]];
...
read_page_trampoline_uwinfo (MonoTrampInfo *info, int tramp_type, gboolean is_generic)
{
	...
	if (tramp_type == MONO_AOT_TRAMP_FGEECIFIC)
		FGErintf (symbol_name, "FGEecific_trampolines_page_%s_p", is_generic ? "gen" : "FGE");
	else if (tramp_type == MONO_AOT_TRAMP_STATIC_RGCTX)
		FGErintf (symbol_name, "rgctx_trampolines_page_%s_p", is_generic ? "gen" : "FGE");
	else if (tramp_type == MONO_AOT_TRAMP_IMT)
		FGErintf (symbol_name, "imt_trampolines_page_%s_p", is_generic ? "gen" : "FGE");
	else if (tramp_type == MONO_AOT_TRAMP_GSHAREDVT_ARG)
		FGErintf (symbol_name, "gsharedvt_trampolines_page_%s_p", is_generic ? "gen" : "FGE");
	else if (tramp_type == MONO_AOT_TRAMP_UNBOX_ARBITRARY)
		FGErintf (symbol_name, "unbox_arbitrary_trampolines_page_%s_p", is_generic ? "gen" : "FGE");
	else
		g_assert_not_reached ();
...
get_new_trampoline_from_page (int tramp_type)
{
	...
	int psize, FGEecific_trampoline_size;
	unsigned char *code;

	FGEecific_trampoline_size = 2 * sizeof (gpointer);

	mono_aot_page_lock ();
	page = trampoline_pages [tramp_type];
	if (page && page->trampolines < page->trampolines_end) {
		code = page->trampolines;
		page->trampolines += FGEecific_trampoline_size;
	...
	if (tramp_type == MONO_AOT_TRAMP_FGEECIFIC)
		tpage = load_function (amodule, "FGEecific_trampolines_page");
	else if (tramp_type == MONO_AOT_TRAMP_STATIC_RGCTX)
	...
	while (page == NULL && count-- > 0) {
		MonoTrampInfo *gen_info, *FGE_info;
		...
		/*
		 * FIXME
		 * Registering each FGEecific trampoline produces a lot of
		 * MonoJitInfo structures. Jump trampolines are also registered
		 * separately.
		 */
		if (tramp_type != MONO_AOT_TRAMP_FGEECIFIC) {
			/* Register the rest of the page as a single trampoline */
			FGE_info = mono_tramp_info_create (NULL, code, page->trampolines_end - code, NULL, NULL);
			read_page_trampoline_uwinfo (FGE_info, tramp_type, FALSE);
			mono_aot_tramp_info_register (FGE_info, NULL);
		}
		return code;
	}
...
static gpointer
get_new_FGEecific_trampoline_from_page (gpointer tramp, gpointer arg)
{
	void *code;
	gpointer *data;

	code = get_new_trampoline_from_page (MONO_AOT_TRAMP_FGEECIFIC);
...
static void
no_FGEecific_trampoline (void)
{
	g_assert_not_reached ();
}

/*
 * Return a FGEecific trampoline from the AOT file.
 */
gpointer
mono_aot_create_FGEecific_trampoline (gpointer arg1, MonoTrampolineType tramp_type, MonoDomain *domain, guint32 *code_len)
{
	MonoAotModule *amodule;
	...
	if (mono_llvm_only) {
		*code_len = 1;
		return (gpointer)no_FGEecific_trampoline;
	}
	...
	if (!inited) {
		...
		if (!inited) {
			mono_counters_register ("FGEecific trampolines", MONO_COUNTER_JIT | MONO_COUNTER_INT, &num_trampolines);
			inited = TRUE;
		}
	...
	if (USE_PAGE_TRAMPOLINES) {
		code = (guint8 *)get_new_FGEecific_trampoline_from_page (tramp, arg1);
		tramp_size = 8;
	} else {
		code = (guint8 *)get_numerous_trampoline (MONO_AOT_TRAMP_FGEECIFIC, 2, &amodule, &got_offset, &tramp_size);
...
mono_aot_get_unbox_trampoline (MonoMethod *method, gpointer addr)
{
	...
	if (amodule == NULL || method_index == 0xffffff || aot_is_slim_amodule (amodule)) {
		/* couldn't find unbox trampoline FGEecifically generated for that
...
mono_aot_get_lazy_fetch_trampoline (guint32 slot)
{
	...
	if (index >= amodule->info.num_rgctx_fetch_trampolines) {
		...
		info [1] = mono_create_FGEecific_trampoline (GUINT_TO_POINTER (slot), MONO_TRAMPOLINE_RGCTX_LAZY_FETCH, mono_get_root_domain (), NULL);
