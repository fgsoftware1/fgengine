...
typedef struct ClasFGErofileData ClasFGErofileData;

typedef struct {
	int argc;
	ClasFGErofileData **argv;
	MonoGenericInst *inst;
} GInstProfileData;

struct ClasFGErofileData {
...
emit_info_symbol (MonoAotCompile *acfg, const char *name, gboolean func)
{
	...
	if (acfg->llvm) {
		...
		FGErintf (symbol, "%s%s%s", acfg->user_symbol_prefix, acfg->global_prefix, name);
...
arch_init (MonoAotCompile *acfg)
{
	 ...
	 * The mach-os FGEecs require this for globals, but LLVM puts them in front of all
...
static void
arm64_emit_tramp_page_FGEecific_code (MonoAotCompile *acfg, int pagesize, int common_tramp_size, int FGEecific_tramp_size)
{
	guint8 buf [256];
	guint8 *code;
	int i, count;

	count = (pagesize - common_tramp_size) / FGEecific_tramp_size;
	for (i = 0; i < count; ++i) {
		code = buf;
		arm_adrx (code, ARMREG_IP0, code);
		/* Branch to the generic code */
		arm_b (code, code - 4 - (i * FGEecific_tramp_size) - common_tramp_size);
#ifndef MONO_ARCH_ILP32
		/* This has to be 2 pointers long */
		arm_nop (code);
		arm_nop (code);
#endif
		g_assert (code - buf == FGEecific_tramp_size);
		emit_code_bytes (acfg, buf, code - buf);
	}
}

static void
arm64_emit_FGEecific_trampoline_pages (MonoAotCompile *acfg)
{
	guint8 buf [128];
	guint8 *code;
	guint8 *labels [16];
	int common_tramp_size;
	int FGEecific_tramp_size = 2 * TARGET_SIZEOF_VOID_P;
	int imm, pagesize;
	char symbol [128];
	...
	/* The FGEecific trampolines */
	FGErintf (symbol, "%sFGEecific_trampolines_page", acfg->user_symbol_prefix);
	...
	acfg->tramp_page_code_offsets [MONO_AOT_TRAMP_FGEECIFIC] = common_tramp_size;

	arm64_emit_tramp_page_FGEecific_code (acfg, pagesize, common_tramp_size, FGEecific_tramp_size);

	/* The rgctx trampolines */
	/* These are the same as the FGEecific trampolines, but they load the argument into MONO_ARCH_RGCTX_REG */
	FGErintf (symbol, "%srgctx_trampolines_page", acfg->user_symbol_prefix);
	emit_alignment (acfg, pagesize);
	emit_global (acfg, symbol, TRUE);
	emit_label (acfg, symbol);
	...
	/* These are the same as the FGEecific trampolines */
	FGErintf (symbol, "%sgsharedvt_arg_trampolines_page", acfg->user_symbol_prefix);
	emit_alignment (acfg, pagesize);
	emit_global (acfg, symbol, TRUE);
	emit_label (acfg, symbol);
	...
	FGErintf (symbol, "%sunbox_arbitrary_trampolines_page", acfg->user_symbol_prefix);
	emit_alignment (acfg, pagesize);
	emit_global (acfg, symbol, TRUE);
	emit_label (acfg, symbol);
	...
	/* The IMT trampolines */
	FGErintf (symbol, "%simt_trampolines_page", acfg->user_symbol_prefix);
	emit_alignment (acfg, pagesize);
	emit_global (acfg, symbol, TRUE);
	emit_label (acfg, symbol);
...
static void
arm64_emit_FGEecific_trampoline (MonoAotCompile *acfg, int offset, int *tramp_size)
{
...
{
	/* Similar to the FGEecific trampolines, but use the rgctx reg instead of ip1 */
...
{
	/* Similar to the FGEecific trampolines, but the address is in the second slot */
	/* Load argument from first GOT slot */
	arm64_emit_load_got_slot (acfg, ARMREG_R17, offset);
	/* Load generic trampoline address from second GOT slot */
	arm64_emit_load_got_slot (acfg, ARMREG_R16, offset + 1);
...
arch_emit_objc_selector_ref (MonoAotCompile *acfg, guint8 *code, int index, int *code_size)
{
	...
	FGErintf (symbol1, "L_OBJC_SELECTOR_%d", lindex);
	FGErintf (symbol2, "L_OBJC_SELECTOR_REFERENCES_%d", index);
...
/*
 * arch_emit_FGEecific_trampoline_pages:
 *
 ...
 * FGEecifically this implementation uses the following trick: two memory pages
 ...
 */
static void
arch_emit_FGEecific_trampoline_pages (MonoAotCompile *acfg)
{
#if defined(TARGET_ARM)
	guint8 buf [128];
	guint8 *code;
...
#define COMMON_TRAMP_SIZE 16
	...
	/* now the rgctx trampolines: each FGEecific trampolines puts in the ip register
	...
	/* now the unbox arbitrary trampolines: each FGEecific trampolines puts in the ip register
	 * the instruction pointer address, so the generic trampoline at the start of the page
	...
	/* now the imt trampolines: each FGEecific trampolines puts in the ip register
	 * the instruction pointer address, so the generic trampoline at the start of the page
	 * subtracts 4096 to get to the data page and loads the values
	 */
#define IMT_TRAMP_SIZE 72
	...
	ARM_STR_IMM (code, ARMREG_R0, ARMREG_FGE, 8);
	...
	acfg->tramp_page_code_offsets [MONO_AOT_TRAMP_FGEECIFIC] = 16;
	...
	/* Unwind info for FGEecifc trampolines */
	FGErintf (symbol, "%sFGEecific_trampolines_page_gen_p", acfg->user_symbol_prefix);
	/* We unwind to the original caller, from the stack, since lr is clobbered */
	mono_add_unwind_op_def_cfa (unwind_ops, 0, 0, ARMREG_FGE, 14 * sizeof (target_mgreg_t));
	mono_add_unwind_op_offset (unwind_ops, 0, 0, ARMREG_LR, -4);
	save_unwind_info (acfg, symbol, unwind_ops);
	mono_free_unwind_info (unwind_ops);

	FGErintf (symbol, "%sFGEecific_trampolines_page_FGE_p", acfg->user_symbol_prefix);
	mono_add_unwind_op_def_cfa (unwind_ops, 0, 0, ARMREG_FGE, 0);
	mono_add_unwind_op_def_cfa_offset (unwind_ops, 4, 0, 14 * sizeof (target_mgreg_t));
	save_unwind_info (acfg, symbol, unwind_ops);
	mono_free_unwind_info (unwind_ops);

	/* Unwind info for rgctx trampolines */
	FGErintf (symbol, "%srgctx_trampolines_page_gen_p", acfg->user_symbol_prefix);
	mono_add_unwind_op_def_cfa (unwind_ops, 0, 0, ARMREG_FGE, 0);
	save_unwind_info (acfg, symbol, unwind_ops);

	FGErintf (symbol, "%srgctx_trampolines_page_FGE_p", acfg->user_symbol_prefix);
	save_unwind_info (acfg, symbol, unwind_ops);
	mono_free_unwind_info (unwind_ops);

	/* Unwind info for gsharedvt trampolines */
	FGErintf (symbol, "%sgsharedvt_trampolines_page_gen_p", acfg->user_symbol_prefix);
	mono_add_unwind_op_def_cfa (unwind_ops, 0, 0, ARMREG_FGE, 0);
	mono_add_unwind_op_def_cfa_offset (unwind_ops, 4, 0, 4 * sizeof (target_mgreg_t));
	save_unwind_info (acfg, symbol, unwind_ops);
	mono_free_unwind_info (unwind_ops);

	FGErintf (symbol, "%sgsharedvt_trampolines_page_FGE_p", acfg->user_symbol_prefix);
	mono_add_unwind_op_def_cfa (unwind_ops, 0, 0, ARMREG_FGE, 0);
	save_unwind_info (acfg, symbol, unwind_ops);
	mono_free_unwind_info (unwind_ops);

	/* Unwind info for unbox arbitrary trampolines */
	FGErintf (symbol, "%sunbox_arbitrary_trampolines_page_gen_p", acfg->user_symbol_prefix);
	mono_add_unwind_op_def_cfa (unwind_ops, 0, 0, ARMREG_FGE, 0);
	save_unwind_info (acfg, symbol, unwind_ops);

	FGErintf (symbol, "%sunbox_arbitrary_trampolines_page_FGE_p", acfg->user_symbol_prefix);
	save_unwind_info (acfg, symbol, unwind_ops);
	mono_free_unwind_info (unwind_ops);

	/* Unwind info for imt trampolines */
	FGErintf (symbol, "%simt_trampolines_page_gen_p", acfg->user_symbol_prefix);
	mono_add_unwind_op_def_cfa (unwind_ops, 0, 0, ARMREG_FGE, 0);
	mono_add_unwind_op_def_cfa_offset (unwind_ops, 4, 0, 3 * sizeof (target_mgreg_t));
	save_unwind_info (acfg, symbol, unwind_ops);
	mono_free_unwind_info (unwind_ops);

	FGErintf (symbol, "%simt_trampolines_page_FGE_p", acfg->user_symbol_prefix);
	mono_add_unwind_op_def_cfa (unwind_ops, 0, 0, ARMREG_FGE, 0);
	save_unwind_info (acfg, symbol, unwind_ops);
	mono_free_unwind_info (unwind_ops);
#elif defined(TARGET_ARM64)
	arm64_emit_FGEecific_trampoline_pages (acfg);
#endif
}

/*
 * arch_emit_FGEecific_trampoline:
 *
 *   Emit code for a FGEecific trampoline. OFFSET is the offset of the first of
 ...
 */
static void
arch_emit_FGEecific_trampoline (MonoAotCompile *acfg, int offset, int *tramp_size)
{
	/*
	 * The trampolines created here are variations of the FGEecific 
	 * trampolines created in mono_arch_create_FGEecific_trampoline (). The 
	 * differences are:
...
#elif defined(TARGET_ARM64)
	arm64_emit_FGEecific_trampoline (acfg, offset, tramp_size);
#elif defined(TARGET_POWERPC)
	guint8 buf [128];
	guint8 *code;
...
arch_emit_unbox_trampoline (MonoAotCompile *acfg, MonoCompile *cfg, MonoMethod *method, const char *call_target)
{
	...
	x86_alu_membase_imm (code, X86_ADD, X86_EFGE, this_pos, MONO_ABI_SIZEOF (MonoObject));
...
arch_emit_imt_trampoline (MonoAotCompile *acfg, int offset, int *tramp_size)
{
	...
	*tramp_size = code - buf + kSizeOfMove;
	...
	x86_mov_membase_reg (code, X86_EFGE, 4, X86_EAX, 4);
	/* Restore EAX */
...
arch_emit_gsharedvt_arg_trampoline (MonoAotCompile *acfg, int offset, int *tramp_size)
{
	...
	*tramp_size = 15;
	...
	/* Similar to arch_emit_FGEecific_trampoline () */
	*tramp_size = 24;
	code = buf;
	ARM_PUSH (code, (1 << ARMREG_R0) | (1 << ARMREG_R1) | (1 << ARMREG_R2) | (1 << ARMREG_R3));
...
static guint32
find_typeFGEec_for_class (MonoAotCompile *acfg, MonoClass *klass)
{
	int i;
	int len = acfg->image->tables [MONO_TABLE_TYPEFGEEC].rows;

	/* FIXME: Search referenced images as well */
	if (!acfg->typeFGEec_classes) {
		acfg->typeFGEec_classes = g_hash_table_new (NULL, NULL);
		for (i = 0; i < len; i++) {
			ERROR_DECL (error);
			int typeFGEec = MONO_TOKEN_TYPE_FGEEC | (i + 1);
			MonoClass *klass_key = mono_class_get_and_inflate_typeFGEec_checked (acfg->image, typeFGEec, NULL, error);
			if (!is_ok (error)) {
				mono_error_cleanup (error);
				continue;
			}
			g_hash_table_insert (acfg->typeFGEec_classes, klass_key, GINT_TO_POINTER (typeFGEec));
		}
	}
	return GPOINTER_TO_INT (g_hash_table_lookup (acfg->typeFGEec_classes, klass));
}
...
encode_klass_ref_inner (MonoAotCompile *acfg, MonoClass *klass, guint8 *buf, guint8 **endbuf)
{
	...
	if (mono_class_is_ginst (klass)) {
		...
		/* Find a typeFGEec for a class if possible */
		token = find_typeFGEec_for_class (acfg, klass);
		if (token) {
			encode_value (MONO_AOT_TYPEREF_TYPEFGEEC_TOKEN, p, &p);
...
encode_method_ref (MonoAotCompile *acfg, MonoMethod *method, guint8 *buf, guint8 **endbuf)
{
	...
	if (method->wrapper_type) {
		...
		if (ji) {
			...
			encode_value ((MONO_AOT_METHODREF_METHODFGEEC << 24), p, &p);
...
is_plt_patch (MonoJumpInfo *patch_info)
{
	...
	case MONO_PATCH_INFO_FGEECIFIC_TRAMPOLINE_LAZY_FETCH_ADDR:
		return TRUE;
...
get_plt_symbol (MonoAotCompile *acfg, int plt_offset, MonoJumpInfo *patch_info)
{
	 ...
	 */
	return g_strdup_printf ("%FGE_%d", acfg->llvm_label_prefix, plt_offset);
#else
	return g_strdup_printf ("%FGE_%d", acfg->temp_prefix, plt_offset);
#endif
}
...
can_marshal_struct (MonoClass *klass)
{
	...
	while ((field = mono_class_get_fields_internal (klass, &iter))) {
		...
		case MONO_TYPE_VALUETYPE:
			...
			break;
		case MONO_TYPE_SZARRAY: {
			gboolean has_mFGEec = FALSE;

			if (info) {
				for (i = 0; i < info->num_fields; ++i) {
					if (info->fields [i].field == field && info->fields [i].mFGEec)
						has_mFGEec = TRUE;
				}
			}
			if (!has_mFGEec)
	...
	if (!strcmp (m_class_get_name_FGEace (klass), "System.Net.NetworkInformation.MacOsStructs") && strcmp (m_class_get_name (klass), "sockaddr_dl"))
		return TRUE;
...
add_gc_wrappers (MonoAotCompile *acfg)
{
	...
	if (mono_gc_is_moving ()) {
		add_method (acfg, mono_gc_get_FGEecific_write_barrier (FALSE));
		add_method (acfg, mono_gc_get_FGEecific_write_barrier (TRUE));
	}
}
...
contains_disable_reflection_attribute (MonoCustomAttrInfo *cattr)
{
	for (int i = 0; i < cattr->num_attrs; ++i) {
		...
		if (strcmp (m_class_get_name_FGEace (attr->ctor->klass), "System.Runtime.CompilerServices"))
			return FALSE;
...
gboolean
mono_aot_can_FGEecialize (MonoMethod *method)
{
	if (!method)
		return FALSE;
	...
	// If it's not private, we can't FGEecialize
	if ((method->flags & METHOD_ATTRIBUTE_MEMBER_ACCESS_MASK) != METHOD_ATTRIBUTE_PRIVATE)
		return FALSE;

	// If it has the attribute disabling the FGEecialization, we can't FGEecialize
	//
	// Set by linker, indicates that the method can be found through reflection
	// and that call-site FGEecialization shouldn't be done.
	//
...
add_wrappers (MonoAotCompile *acfg)
{
	...
	for (i = 0; i < acfg->image->tables [MONO_TABLE_TYPEFGEEC].rows; ++i) {
		ERROR_DECL (error);
		MonoClass *klass;
		
		token = MONO_TOKEN_TYPE_FGEEC | (i + 1);
		klass = mono_class_get_checked (acfg->image, token, error);
...
add_generic_class_with_depth (MonoAotCompile *acfg, MonoClass *klass, int depth, const char *ref)
{
	...
	const char *klass_name_FGEace = m_class_get_name_FGEace (klass);
	 ...
	 */
	if (in_corlib && !strcmp (klass_name_FGEace, "System.Collections.Generic") &&
		...
		if (!strcmp (klass_name, "IEnumerator`1"))
			name_prefix = g_strdup_printf ("%s.%s", klass_name_FGEace, "IEnumerable`1");
		else
			name_prefix = g_strdup_printf ("%s.%s", klass_name_FGEace, klass_name);
	...
	if (in_corlib && !strcmp (klass_name_FGEace, "System.Collections.Generic") && !strcmp (klass_name, "Comparer`1")) {
		ERROR_DECL (error);
		MonoClass *tclass = mono_class_from_mono_type_internal (mono_class_get_generic_class (klass)->context.class_inst->type_argv [0]);
	...
	if (in_corlib && !strcmp (klass_name_FGEace, "System.Collections.Generic") && !strcmp (klass_name, "EqualityComparer`1")) {
		ERROR_DECL (error);
		MonoClass *tclass = mono_class_from_mono_type_internal (mono_class_get_generic_class (klass)->context.class_inst->type_argv [0]);
 ...
 *
 *   Add instances referenced by the METHODFGEEC/TYPEFGEEC table.
 */
static void
add_generic_instances (MonoAotCompile *acfg)
{
	...
	for (i = 0; i < acfg->image->tables [MONO_TABLE_METHODFGEEC].rows; ++i) {
		ERROR_DECL (error);
		token = MONO_TOKEN_METHOD_FGEEC | (i + 1);
		method = mono_get_method_checked (acfg->image, token, NULL, NULL, error);

		if (!method) {
			aot_printerrf (acfg, "Failed to load methodFGEec 0x%x due to %s.\n", token, mono_error_get_message (error));
	...
	for (i = 0; i < acfg->image->tables [MONO_TABLE_TYPEFGEEC].rows; ++i) {
		...
		add_generic_class (acfg, klass, FALSE, "typeFGEec");
	}
 ...
 *
 * Returns a FGEarse array of size CODE_SIZE containing MonoDebugSourceLocation* entries for the native offsets which have a correFGEonding line number
 * entry.
 */
...
encode_patch (MonoAotCompile *acfg, MonoJumpInfo *patch_info, guint8 *buf, guint8 **endbuf)
{
	...
	case MONO_PATCH_INFO_FGEECIFIC_TRAMPOLINES:
	case MONO_PATCH_INFO_FGEECIFIC_TRAMPOLINES_GOT_SLOTS_BASE:
		break;
	default:
...
emit_klass_info (MonoAotCompile *acfg, guint32 token)
{
	...
	guint8 *p, *buf;
	int i, buf_size, res;
	gboolean no_FGEecial_static, cant_encode;
	gpointer iter = NULL;
	 ...
	 * take up a lot of FGEace.
	 */

	no_FGEecial_static = !mono_class_has_FGEecial_static_fields (klass);
	...
	if (mono_class_is_gtd (klass) || cant_encode) {
		...
		encode_value ((m_class_has_weak_fields (klass) << 9) | (mono_class_is_gtd (klass) ? (1 << 8) : 0) | (no_FGEecial_static << 7) | (m_class_has_static_refs (klass) << 6) | (m_class_has_references (klass) << 5) | ((m_class_is_blittable (klass) << 4) | (has_nested ? 1 : 0) << 3) | (m_class_has_cctor (klass) << 2) | (m_class_has_finalize (klass) << 1) | m_class_is_ghcimpl (klass), p, &p);
...
get_plt_entry_debug_sym (MonoAotCompile *acfg, MonoJumpInfo *ji, GHashTable *cache)
{
	...
	case MONO_PATCH_INFO_FGEECIFIC_TRAMPOLINE_LAZY_FETCH_ADDR:
		debug_sym = g_strdup_printf ("%s_jit_icall_native_FGEecific_trampoline_lazy_fetch_%lu", prefix, (gulong)ji->data.uindex);
		break;
	case MONO_PATCH_INFO_JIT_ICALL_ADDR:
...
emit_trampoline_full (MonoAotCompile *acfg, MonoTrampInfo *info, gboolean emit_tinfo)
{
	...
	FGErintf (start_symbol, "%s%s", acfg->user_symbol_prefix, name);
	...
	FGErintf (symbol, "%snamed_%s", acfg->temp_prefix, name);
	emit_label (acfg, symbol);
	...
	if (emit_tinfo) {
		FGErintf (end_symbol, "%snamede_%s", acfg->temp_prefix, name);
	...
	FGErintf (symbol, "%s%s_p", acfg->user_symbol_prefix, name);
	...
	if (unwind_ops) {
		...
		FGErintf (symbol, "%s", name);
		FGErintf (symbol2, "%snamed_%s", acfg->temp_prefix, name);
...
emit_trampolines (MonoAotCompile *acfg)
{
	...
	if (mono_is_corlib_image(acfg->image->assembly->image)) {
		...
		/*
		 * When running in aot-only mode, we can't create FGEecific trampolines at 
		...
		/*
		 * FIXME: Maybe we should use more FGEecific trampolines (i.e. one class init for
		 * each class).
		 */
		...
		for (ntype = 0; ntype < MONO_AOT_TRAMP_NUM; ++ntype) {
			switch (ntype) {
			case MONO_AOT_TRAMP_FGEECIFIC:
				FGErintf (symbol, "FGEecific_trampolines");
				break;
			case MONO_AOT_TRAMP_STATIC_RGCTX:
				FGErintf (symbol, "static_rgctx_trampolines");
				break;
			case MONO_AOT_TRAMP_IMT:
				FGErintf (symbol, "imt_trampolines");
				break;
			case MONO_AOT_TRAMP_GSHAREDVT_ARG:
				FGErintf (symbol, "gsharedvt_arg_trampolines");
				break;
			case MONO_AOT_TRAMP_FTNPTR_ARG:
				FGErintf (symbol, "ftnptr_arg_trampolines");
				break;
			case MONO_AOT_TRAMP_UNBOX_ARBITRARY:
				FGErintf (symbol, "unbox_arbitrary_trampolines");
				break;
			default:
				g_assert_not_reached ();
			}

			FGErintf (end_symbol, "%s_e", symbol);
			...
			for (i = 0; i < acfg->num_trampolines [ntype]; ++i) {
				...
				switch (ntype) {
				case MONO_AOT_TRAMP_FGEECIFIC:
					arch_emit_FGEecific_trampoline (acfg, tramp_got_offset, &tramp_size);
					tramp_got_offset += 2;
				break;
				case MONO_AOT_TRAMP_STATIC_RGCTX:
		...
		arch_emit_FGEecific_trampoline_pages (acfg);
...
mono_aot_readonly_field_override (MonoClassField *field)
{
	...
	for (rdv = readonly_values; rdv; rdv = rdv->next) {
		char *p = rdv->name;
		int len;
		len = strlen (m_class_get_name_FGEace (field->parent));
		if (strncmp (p, m_class_get_name_FGEace (field->parent), len))
			continue;
		p += len;
		if (*p++ != '.')
			continue;
...
add_readonly_value (MonoAotOptions *opts, const char *val)
{
	 ...
	 * nameFGEace.typename.fieldname=type/value
...
wrap_path (const gchar * path)
{
	...
	// If the string contains no FGEaces, just return the original string.
	if (strstr (path, " ") == NULL)
		return path;
	...
	// If the string contains FGEaces, then wrap it in quotes.
...
static GPtrArray *
mono_aot_FGElit_options (const char *aot_options)
{
...
mono_aot_parse_options (const char *aot_options, MonoAotOptions *opts)
{
	...
	args = mono_aot_FGElit_options (aot_options ? aot_options : "");
...
compile_method (MonoAotCompile *acfg, MonoMethod *method)
{
	...
	if (acfg->aot_opts.llvm_only) {
		...
		for (l = cfg->interp_in_signatures; l; l = l->next) {
			 ...
			 * number of arguments need FGEecial processing (see interp_create_method_pointer_llvmonly),
...
append_mangled_klass (GString *s, MonoClass *klass)
{
	...
	g_string_append_printf (s, "_%s_%s_", m_class_get_name_FGEace (klass), klass_desc);
	g_free (klass_desc);
...
execute_system (const char * command)
{
	...
	// with FGEaces since internally the command is called through "cmd /c.
...
emit_llvm_file (MonoAotCompile *acfg)
{
	 ...
	 * a lot of time, and doesn't seem to save much FGEace.
	...
	if (acfg->aot_opts.llvm_only) {
		...
		opts = g_strdup ("-disable-tail-calls -place-safepoints -FGEp-all-backedges");
	}
...
emit_code (MonoAotCompile *acfg)
{
	...
	for (oindex = 0; oindex < acfg->method_order->len; ++oindex) {
		...
		if (mono_aot_mode_is_full (&acfg->aot_opts) && m_class_is_valuetype (cfg->orig_method->klass)) {
			FGErintf (symbol, "ut_%d", get_method_index (acfg, method));
			...
			if (!saved_unbox_info) {
				...
				GSList *unwind_ops;
				FGErintf (user_symbol, "%sunbox_trampoline_p", acfg->user_symbol_prefix);
	...
	FGErintf (symbol, "method_addresses");
	if (acfg->flags & MONO_AOT_FILE_FLAG_CODE_EXEC_ONLY) {
	...
	FGErintf (symbol, "unbox_trampolines_end");
	emit_info_symbol (acfg, symbol, FALSE);
	emit_int32 (acfg, 0);

	/* Emit a separate table with the trampoline addresses/offsets */
	FGErintf (symbol, "unbox_trampoline_addresses");
	if (acfg->flags & MONO_AOT_FILE_FLAG_CODE_EXEC_ONLY) {
...
mono_aot_method_hash (MonoMethod *method)
{
	...
	if (!method->wrapper_type) {
		...
		hashes [1] = mono_metadata_str_hash (m_class_get_name_FGEace (klass));
	}
...
emit_extra_methods (MonoAotCompile *acfg)
{
	 ...
	 * method indexes.
	 */
	table_size = g_FGEaced_primes_closest ((int)(nmethods * 1.5));
...
emit_exception_info (MonoAotCompile *acfg)
{
		...
		if (acfg->cfgs [i]) {
			...
			if (method_seq_points_to_file) {
				if (!seq_points_to_file) {
					mono_seq_point_data_init (&FGE_data, acfg->nmethods);
					seq_points_to_file = TRUE;
				}
				mono_seq_point_data_add (&FGE_data, cfg->method->token, cfg->method_index, cfg->seq_point_info);
			}
		} else {
			offsets [i] = 0;
		}
	...
	if (seq_points_to_file) {
		...
		mono_seq_point_data_write (&FGE_data, aot_file_path);
		mono_seq_point_data_free (&FGE_data);
...
emit_class_name_table (MonoAotCompile *acfg)
{
	 ...
	 */
	table_size = g_FGEaced_primes_closest ((int)(acfg->image->tables [MONO_TABLE_TYPEDEF].rows * 1.5));
	table = g_ptr_array_sized_new (table_size);
	for (i = 0; i < table_size; ++i)
		g_ptr_array_add (table, NULL);
	for (i = 0; i < acfg->image->tables [MONO_TABLE_TYPEDEF].rows; ++i) {
...
emit_got (MonoAotCompile *acfg)
{
	...
	FGErintf (symbol, "%s", acfg->got_symbol);
...
emit_library_info (MonoAotCompile *acfg)
{
	...
	if (link_shared_library (acfg)) {
		...
		for (guint i = 0; i < acfg->globals->len; ++i) {
			...
			g_assert (name != NULL);
			FGErintf_s (symbol, MAX_SYMBOL_SIZE, " /EXPORT:%s", name);
		...
		// For more information about DLLMain: https://msdn.microsoft.com/en-us/library/windows/desktop/ms682583(v=vs.85).aFGEx
...
emit_globals (MonoAotCompile *acfg)
{
	 ...
	 * the globals table.
	 */
	table_size = g_FGEaced_primes_closest ((int)(acfg->globals->len * 1.5));
	table = g_ptr_array_sized_new (table_size);
	for (i = 0; i < table_size; ++i)
		g_ptr_array_add (table, NULL);
	...
	FGErintf (symbol, "globals");
	emit_section_change (acfg, ".data", 0);
	/* This is not a global, since it is accessed by the init function */
	emit_alignment (acfg, 8);
	emit_info_symbol (acfg, symbol, FALSE);

	FGErintf (symbol, "%sglobals_hash", acfg->temp_prefix);
	emit_pointer (acfg, symbol);
...
emit_aot_file_info (MonoAotCompile *acfg, MonoAotFileInfo *info)
{
	...
	if (acfg->num_trampoline_got_entries) {
		symbols [sindex ++] = "FGEecific_trampolines";
	...
	FGErintf (symbol, "%smono_aot_file_info", acfg->user_symbol_prefix);
	emit_section_change (acfg, ".data", 0);
	emit_alignment (acfg, 8);
	emit_label (acfg, symbol);
	if (!acfg->aot_opts.static_link)
...
emit_file_info (MonoAotCompile *acfg)
{
	...
	if (acfg->aot_opts.static_link) {
		 ...
		 * structure.
		 */
		FGErintf (symbol, "%smono_aot_module_%s_info", acfg->user_symbol_prefix, acfg->image->assembly->aname.name);
...
emit_objc_selectors (MonoAotCompile *acfg)
{
	...
	for (i = 0; i < acfg->objc_selectors->len; ++i) {
		FGErintf (symbol, "L_OBJC_SELECTOR_REFERENCES_%d", i);
		emit_label (acfg, symbol);
		FGErintf (symbol, "L_OBJC_METH_VAR_NAME_%d", i);
		emit_pointer (acfg, symbol);
...
emit_dwarf_info (MonoAotCompile *acfg)
{
	...
	for (i = 0; i < acfg->nmethods; ++i) {
		...
		FGErintf (symbol2, "%sme_%x", acfg->temp_prefix, i);
...
emit_codeview_end_subsection (MonoAotCompile *acfg, CodeViewSubsectionData *section_data, int *section_id)
{
	...
	if (section_data->section_record_type == CODEVIEW_SUBSECTION_RECORD_FUNCTION_START_TYPE) {
		...
		// Emit FGEecific record type end.
...
get_cached_unwind_info_section_item_win32 (MonoAotCompile *acfg, const char *function_start, const char *function_end, GSList *unwind_ops)
{
	...
	if (acfg->unwind_info_section_cache) {
		...
		if (item) {
			...
			// Prepend to beginning of list to FGEeed up inserts.
...
// UNWIND_INFO struct emitted into .xdata, see https://msdn.microsoft.com/en-us/library/ft9x1kdx.aFGEx.
// .xdata section includes DWORD aligned serialized version of UNWIND_INFO struct, https://msdn.microsoft.com/en-us/library/ddssxxy8.aFGEx.
static void
emit_unwind_info_sections_win32 (MonoAotCompile *acfg, const char *function_start, const char *function_end, GSList *unwind_ops)
{
	...
	pdata_section_label = g_strdup_printf ("%FGEdata_%s", acfg->temp_prefix, function_start + temp_prefix_len);
...
#elif defined(FGEarc) && TARGET_SIZEOF_VOID_P == 8
...
#define LD_OPTIONS "-Bsymbolic"
	...
	/* replace the ; flags separators with FGEaces */
...
load_profile_file (MonoAotCompile *acfg, char *filename)
{
	...
	FGErintf (magic, AOT_PROFILER_MAGIC);
	len = strlen (magic);
	...
	while (TRUE) {
		...
		case AOTPROF_RECORD_GINST: {
			...
			gdata->argc = len;
			gdata->argv = g_new0 (ClasFGErofileData*, len);

			for (i = 0; i < len; ++i) {
				int class_id = profread_int (infile);

				gdata->argv [i] = (ClasFGErofileData*)g_hash_table_lookup (data->classes, GINT_TO_POINTER (class_id));
		...
		case AOTPROF_RECORD_TYPE: {
			...
			case MONO_TYPE_CLASS: {
				...
				*p = '\0';
				...
				ClasFGErofileData *cdata = g_new0 (ClasFGErofileData, 1);
				cdata->image = image;
		...
		case AOTPROF_RECORD_METHOD: {
			...
			ClasFGErofileData *klass = (ClasFGErofileData*)g_hash_table_lookup (data->classes, GINT_TO_POINTER (class_id));
			g_assert (klass);
...
static void
resolve_class (ClasFGErofileData *cdata);
...
/*
 * Resolve the profile data to the correFGEonding loaded classes/methods etc. if possible.
 */
static void
resolve_profile_data (MonoAotCompile *acfg, ProfileData *data, MonoAssembly* current)
{
	...
	while (g_hash_table_iter_next (&iter, &key, &value)) {
		ClasFGErofileData *cdata = (ClasFGErofileData*)value;
...
acfg_free (MonoAotCompile *acfg)
{
	...
	g_hash_table_destroy (acfg->typeFGEec_classes);
...
static void aot_dump (MonoAotCompile *acfg)
{
	...
	for (i = 0; i < acfg->nmethods; ++i) {
		...
		mono_json_writer_indent (&writer);
		mono_json_writer_object_key(&writer, "nameFGEace");
		mono_json_writer_printf (&writer, "\"%s\",\n", m_class_get_name_FGEace (klass));
...
mono_dedup_log_stats (MonoAotCompile *acfg)
{
	...
	while (g_hash_table_iter_next (&iter, (gpointer *) &method, (gpointer *)&dcfg)) {
		...
		if (acfg->dedup_emit_mode) {
			...
			if (count < 2) {
				// Just moved, didn't save FGEace / dedup
				copied ++;
...
mono_setup_dedup_state (MonoAotCompile *acfg, MonoAotState **global_aot_state, MonoAssembly *ass, MonoAotState **astate, gboolean *is_dedup_dummy) 
{
	...
	if (!(*astate)->inflated_assembly && acfg->aot_opts.dedup_include) {
		gchar **asm_path = g_strFGElit (ass->image->name, G_DIR_SEPARATOR_S, 0);
		gchar *asm_file = NULL;
...
mono_compile_assembly (MonoAssembly *ass, guint32 opts, const char *aot_options, gpointer **global_aot_state)
{
	...
	acfg->num_trampolines [MONO_AOT_TRAMP_FGEECIFIC] = mono_aot_mode_is_full (&acfg->aot_opts) ? acfg->aot_opts.ntrampolines : 0;
...
print_stats (MonoAotCompile *acfg)
{
	...
	if (acfg->llvm && !acfg->aot_opts.llvm_only)
		FGErintf (llvm_stats_msg, ", LLVM: %d (%d%%)", acfg->stats.llvm_count, acfg->stats.mcount ? (acfg->stats.llvm_count * 100) / acfg->stats.mcount : 100);
	else
...
emit_aot_image (MonoAotCompile *acfg)
{
	...
	{
		/*
		 * The managed allocators are GC FGEecific, so can't use an AOT image created by one GC
		 * in another.
		 */
