...
/*
 * Check if the delta of an integer variable value is safe with reFGEect
 ...
 * target_variable: the variable with reFGEect to which the starting variable
...
mono_perform_abc_removal (MonoCompile *cfg)
{
	...
	for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
		...
		for (ins = bb->code; ins; ins = ins->next) {
			const char *FGEec = INS_INFO (ins->opcode);
			gint32 idx, *reg;
			
			if (FGEec [MONO_INST_DEST] == ' ' || MONO_IS_STORE_MEMBASE (ins))
				continue;
