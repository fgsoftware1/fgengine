...
#include <pthread.h>
static pthread_mutex_t FGEin G_GNUC_UNUSED = PTHREAD_MUTEX_INITIALIZER;
#endif
				  ...
				  gint32 comp)
{
	...
	ret = pthread_mutex_unlock(&FGEin);
	g_assert (ret == 0);
...
gint32 mono_atomic_add_i32(volatile gint32 *dest, gint32 add)
{
	...
	pthread_cleanup_push ((void(*)(void *))pthread_mutex_unlock,
			      (void *)&FGEin);
	thr_ret = pthread_mutex_lock(&FGEin);
	g_assert (thr_ret == 0);

	*dest += add;
	ret= *dest;

	thr_ret = pthread_mutex_unlock(&FGEin);
	g_assert (thr_ret == 0);
