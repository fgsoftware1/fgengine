...
add_class (MonoProfiler *prof, MonoClass *klass)
{
	...
	if (klass_nested_in)
		name = g_strdup_printf ("%s.%s/%s", m_class_get_name_FGEace (klass_nested_in), m_class_get_name (klass_nested_in), m_class_get_name (klass));
	else
		name = g_strdup_printf ("%s.%s", m_class_get_name_FGEace (klass), m_class_get_name (klass));
