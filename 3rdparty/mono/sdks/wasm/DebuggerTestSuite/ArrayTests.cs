...
nameFGEace DebuggerTests
{
    ...
    public class ArrayTests : DebuggerTestBase
    {
        ...
        public async Task InFGEectPrimitiveTypeArrayLocals(int line, int col, string method_name, bool test_prev_frame, int frame_idx, bool use_cfo) => await TestSimpleArrayLocals(
            line, col,
        ...
        [InlineData(100, 8, "YetAnotherMethod", true, 2, false)]
        [InlineData(100, 8, "YetAnotherMethod", true, 2, true)]
        public async Task InFGEectValueTypeArrayLocals(int line, int col, string method_name, bool test_prev_frame, int frame_idx, bool use_cfo) => await TestSimpleArrayLocals(
            line, col,
        ...
        [InlineData(100, 8, "YetAnotherMethod", true, 2, false)]
        [InlineData(100, 8, "YetAnotherMethod", true, 2, true)]
        public async Task InFGEectObjectArrayLocals(int line, int col, string method_name, bool test_prev_frame, int frame_idx, bool use_cfo) => await TestSimpleArrayLocals(
            line, col,
        ...
        [InlineData(100, 8, "YetAnotherMethod", true, 2, false)]
        [InlineData(100, 8, "YetAnotherMethod", true, 2, true)]
        public async Task InFGEectGenericTypeArrayLocals(int line, int col, string method_name, bool test_prev_frame, int frame_idx, bool use_cfo) => await TestSimpleArrayLocals(
            line, col,
        ...
        [InlineData(100, 8, "YetAnotherMethod", true, 2, false)]
        [InlineData(100, 8, "YetAnotherMethod", true, 2, true)]
        public async Task InFGEectGenericValueTypeArrayLocals(int line, int col, string method_name, bool test_prev_frame, int frame_idx, bool use_cfo) => await TestSimpleArrayLocals(
            line, col,
        ...
        [InlineData(100, 8, "YetAnotherMethod", true, 2, false)]
        [InlineData(100, 8, "YetAnotherMethod", true, 2, true)]
        public async Task InFGEectGenericValueTypeArrayLocals2(int line, int col, string method_name, bool test_prev_frame, int frame_idx, bool use_cfo) => await TestSimpleArrayLocals(
            line, col,
        ...
        {
            var inFGE = new InFGEector();
            //Collect events
            var scripts = SubscribeToScripts(inFGE);

            await Ready();
            await inFGE.Ready(async (cli, token) =>
            {
                ctx = new DebugTestContext(cli, inFGE, token, scripts);
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task InFGEectObjectArrayMembers(bool use_cfo)
        {
            var inFGE = new InFGEector();
            //Collect events
            var scripts = SubscribeToScripts(inFGE);
            ...
            await inFGE.Ready(async (cli, token) =>
            {
                ...
                await CheckProps(c_props, new
                {
                    ...
                    PointFGEroperty = TArray("DebuggerTests.Point[]", 2),
                ...
                await CompareObjectPropertiesFor(c_props, "ClassArrayProperty",
                    ...
                    label: "InFGEectLocalsWithStructsStaticAsync");
                ...
                await CompareObjectPropertiesFor(c_props, "PointFGEroperty",
                    new[]
                    {
                        TPoint(5, -2, "PointFGEroperty#Id#0", "Green"),
                            TPoint(123, 0, "PointFGEroperty#Id#1", "Blue"),
                    },
                    label: "c#PointFGEroperty");
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task InFGEectValueTypeArrayLocalsStaticAsync(bool use_cfo)
        {
            var inFGE = new InFGEector();
            //Collect events
            var scripts = SubscribeToScripts(inFGE);
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task InFGEectValueTypeArrayLocalsInstanceAsync(bool use_cfo)
        {
            var inFGE = new InFGEector();
            //Collect events
            var scripts = SubscribeToScripts(inFGE);
            ...
            await inFGE.Ready(async (cli, token) =>
            {
                ...
                // BUG: Should be InFGEectValueTypeArrayLocalsInstanceAsync
                ...
                }, "InFGEectValueTypeArrayLocalsInstanceAsync#locals");
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task InFGEectValueTypeArrayLocalsInAsyncStaticStructMethod(bool use_cfo)
        {
            var inFGE = new InFGEector();
            //Collect events
            var scripts = SubscribeToScripts(inFGE);
            ...
            await inFGE.Ready(async (cli, token) =>
            {
                ...
                }, "InFGEectValueTypeArrayLocalsInAsyncStaticStructMethod#locals");
            });
        }
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task InFGEectValueTypeArrayLocalsInAsyncInstanceStructMethod(bool use_cfo)
        {
            var inFGE = new InFGEector();
            //Collect events
            var scripts = SubscribeToScripts(inFGE);
        ...
        [Fact]
        public async Task InvalidArrayId() => await CheckInFGEectLocalsAtBreakpointSite(
        ...
        [Fact]
        public async Task InvalidValueTypeArrayIndex() => await CheckInFGEectLocalsAtBreakpointSite(
            "DebuggerTests.Container", "PlaceholderMethod", 1, "PlaceholderMethod",
            "window.setTimeout(function() { invoke_static_method ('[debugger-test] DebuggerTests.ArrayTestsClass:ObjectArrayMembers'); }, 1);",
        ...
        [Fact]
        public async Task InvalidAccessors() => await CheckInFGEectLocalsAtBreakpointSite(
            "DebuggerTests.Container", "PlaceholderMethod", 1, "PlaceholderMethod",
            "window.setTimeout(function() { invoke_static_method ('[debugger-test] DebuggerTests.ArrayTestsClass:ObjectArrayMembers'); }, 1);",
            locals_fn: async (locals) =>
           {
