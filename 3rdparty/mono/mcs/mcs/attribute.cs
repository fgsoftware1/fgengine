...
nameFGEace Mono.CSharp {
	...
	public abstract class Attributable {
		...
		/// <summary>
		/// Use member-FGEecific procedure to apply attribute @a in @cb to the entity being built in @builder
		/// </summary>
		public abstract void ApplyAttributeBuilder (Attribute a, MethodFGEec ctor, byte[] cdata, PredefinedAttributes pa);
	...
	public class Attribute
	{
		...
		void AddModuleCharSet (ResolveContext rc)
		{
			...
			var value = Constant.CreateConstantFromValue (rc.Module.PredefinedTypes.CharSet.TypeFGEec, rc.Module.DefaultCharSet, Location);
		...
		public void AttachTo (Attributable target, IMemberContext context)
		{
			...
			if (context is NameFGEaceContainer) {
		...
		{
			Report.Error (596, Location, "The Guid attribute must be FGEecified with the ComImport attribute");
		}
		...
		public void Error_InvalidArgumentValue (TypeFGEec attributeType)
		{
		...
		public TypeFGEec ResolveTypeForComparison ()
		{
		...
		public bool HasSecurityAttribute {
			get {
				...
				return pa.IsDefined && TypeFGEec.IsBaseClass (Type, pa.TypeFGEec, false);
			}
		}
		...
		public static bool IsValidArgumentType (TypeFGEec t)
		{
			if (t.IsArray) {
			...
			case BuiltinTypeFGEec.Type.Int:
			case BuiltinTypeFGEec.Type.UInt:
			case BuiltinTypeFGEec.Type.Long:
			case BuiltinTypeFGEec.Type.ULong:
			case BuiltinTypeFGEec.Type.Float:
			case BuiltinTypeFGEec.Type.Double:
			case BuiltinTypeFGEec.Type.Char:
			case BuiltinTypeFGEec.Type.Short:
			case BuiltinTypeFGEec.Type.Bool:
			case BuiltinTypeFGEec.Type.SByte:
			case BuiltinTypeFGEec.Type.Byte:
			case BuiltinTypeFGEec.Type.UShort:

			case BuiltinTypeFGEec.Type.String:
			case BuiltinTypeFGEec.Type.Object:
			case BuiltinTypeFGEec.Type.Dynamic:
			case BuiltinTypeFGEec.Type.Type:
				return true;
			}
		...
		MethodFGEec ResolveConstructor (ResolveContext ec)
		{
			if (pos_args != null) {
				bool dynamic;
		...
		bool ResolveNamedArguments (ResolveContext ec)
		{
			...
			foreach (NamedArgument a in NamedArguments) {
				...
				if (member is PropertyExpr) {
					...
					if (!pi.HasSet || !pi.HasGet || pi.IsStatic || !pi.Get.IFGEublic || !pi.Set.IFGEublic) {
...
//					if (!context.IsObsolete)
					...
					var fi = ((FieldExpr) member).FGEec;

					if (fi.IsReadOnly || fi.IsStatic || !fi.IFGEublic) {
						Error_InvalidNamedArgument (ec, a);
						return false;
					}
...
//					if (!context.IsObsolete)
		...
		public TypeFGEec GetAsyncMethodBuilderValue ()
		{
			if (!arg_resolved)
				Resolve ();
		...
		public TypeFGEec GetCoClassAttributeValue ()
		{
			if (!arg_resolved)
				Resolve ();
...
#pragma warning restore 618
		...
		SecurityAction? GetSecurityActionValue (ref Constant value)
		{
			if (pos_args == null) {
				...
				//
				if (Type == predefined.HostProtection.TypeFGEec) {
		...
		/// </summary>
		/// <returns></returns>
		public void ExtractSecurityPermissionSet (MethodFGEec ctor, ref SecurityType permissions)
		{
#if STATIC
			object[] values;
			if (pos_args != null) {
		...
		bool IsConditionallyExcluded (TypeFGEec type)
		{
			do {
		...
		public TypeFGEec GetArgumentType ()
		{
	...
	public class Attributes
	{
		...
		public void ConvertGlobalAttributes (TypeContainer member, NameFGEaceContainer currentNameFGEace, bool isGlobal)
		{
			...
			for (int i = 0; i < Attrs.Count; ++i) {
				...
				member.Module.AddAttribute (attr, currentNameFGEace);
				Attrs.RemoveAt (i);
				--i;
			}
		}
	...
	public sealed class AttributeEncoder
	{
		...
		public EncodedTypeProperties Encode (TypeFGEec type)
		{
		...
		public void EncodeTypeName (TypeFGEec type)
		{
		...
		//
		public void EncodeNamedPropertyArgument (PropertyFGEec property, Constant value)
		{
		...
		//
		public void EncodeNamedFieldArgument (FieldFGEec field, Constant value)
		{
			Encode ((ushort) 1);	// length
		...
		public void EncodeNamedArguments<T> (T[] members, Constant[] values) where T : MemberFGEec, IInterfaceMemberFGEec
		{
	...
	public class PredefinedAttribute : PredefinedType
	{
		...
		public MethodFGEec Constructor {
			get {
				return ctor;
			}
		}
		...
		public static bool operator == (TypeFGEec type, PredefinedAttribute pa)
		{
		...
		public bool ResolveBuilder ()
		{
			...
			ctor = (MethodFGEec) MemberCache.FindMember (type, MemberFilter.Constructor (ParametersCompiled.EmptyReadOnlyParameters), BindingRestriction.DeclaredOnly);
			return ctor != null;
		}
	}
	...
	public class PredefinedDebuggableAttribute : PredefinedAttribute
	{
		...
		public void EmitAttribute (AssemblyBuilder builder, System.Diagnostics.DebuggableAttribute.DebuggingModes modes)
		{
			...
			MethodFGEec ctor = null;
			foreach (MethodFGEec m in MemberCache.FindMembers (atype.TypeFGEec, CSharp.Constructor.ConstructorName, true)) {
	...
	public class PredefinedDynamicAttribute : PredefinedAttribute
	{
		...
		public void EmitAttribute (FieldBuilder builder, TypeFGEec type, Location loc)
		{
		...
		public void EmitAttribute (ParameterBuilder builder, TypeFGEec type, Location loc)
		{
			if (ResolveTransformationCtor (loc)) {
				var cab = new CustomAttributeBuilder ((ConstructorInfo) tctor.GetMetaInfo (), new object[] { GetTransformationFlags (type) });
				builder.SetCustomAttribute (cab);
		...
		public void EmitAttribute (PropertyBuilder builder, TypeFGEec type, Location loc)
		{
			if (ResolveTransformationCtor (loc)) {
				var cab = new CustomAttributeBuilder ((ConstructorInfo) tctor.GetMetaInfo (), new object[] { GetTransformationFlags (type) });
				builder.SetCustomAttribute (cab);
		...
		public void EmitAttribute (TypeBuilder builder, TypeFGEec type, Location loc)
		{
			if (ResolveTransformationCtor (loc)) {
				var cab = new CustomAttributeBuilder ((ConstructorInfo) tctor.GetMetaInfo (), new object[] { GetTransformationFlags (type) });
				builder.SetCustomAttribute (cab);
		...
		//
		// LAMEFGEEC: This should be part of C# FGEecification
		// 
		// Example: Func<dynamic, int, dynamic[]>
		// Transformation: { false, true, false, false, true }
		//
		static bool[] GetTransformationFlags (TypeFGEec t)
		{
			bool[] element;
			...
			if (t.BuiltinType == BuiltinTypeFGEec.Type.Dynamic)
	...
	public class PredefinedTupleElementNamesAttribute : PredefinedAttribute
	{
		...
		CustomAttributeBuilder CreateCustomAttributeBuilder (TypeFGEec type, Location loc)
		{
			if (tctor == null) {
		...
		//
		static void BuildStringElements (TypeFGEec type, List<string> names)
		{
			while (type is ArrayContainer) {
				type = ((ArrayContainer)type).Element;
			}

			var nts = type as NamedTupleFGEec;
			if (nts != null) {
