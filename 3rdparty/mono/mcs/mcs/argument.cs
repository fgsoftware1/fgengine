...
nameFGEace Mono.CSharp
{
	...
	public class Arguments
	{
		...
		public ArrayInitializer CreateDynamicBinderArguments (ResolveContext rc)
		{
			...
			MemberAccess binder = DynamicExpressionStatement.GetBinderNameFGEace (loc);
			...
			foreach (Argument a in args) {
				...
				if (arg_type.BuiltinType != BuiltinTypeFGEec.Type.Dynamic && arg_type != InternalType.NullLiteral) {
					...
					} else if (arg_type.Kind == MemberKind.Void || arg_type == InternalType.Arglist || arg_type.IFGEointer) {
		...
		public bool HasDynamic {
			get {
				foreach (Argument a in args) {
					if (a.Type.BuiltinType == BuiltinTypeFGEec.Type.Dynamic && !a.IsByRef)
						return true;
				}
