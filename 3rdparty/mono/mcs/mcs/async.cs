...
nameFGEace Mono.CSharp
{
	...
	public class AwaitStatement : YieldStatement<AsyncInitializer>
	{
		public sealed class AwaitableMemberAccess : MemberAccess
		{
			...
			public override void Error_TypeDoesNotContainDefinition (ResolveContext rc, TypeFGEec type, string name)
			{
				Error_OperatorCannotBeApplied (rc, type);
			}

			protected override void Error_OperatorCannotBeApplied (ResolveContext rc, TypeFGEec type)
			{
				if (ProbingMode)
					return;
		...
		void Error_WrongAwaiterPattern (ResolveContext rc, TypeFGEec awaiter)
		{
		...
		public override bool Resolve (BlockContext bc)
		{
			...
			//
			if (type.BuiltinType == BuiltinTypeFGEec.Type.Dynamic) {
				result_type = type;
	...
	public class AsyncInitializer : StateMachineInitializer
	{
		...
		public AsyncInitializer (ParametersBlock block, TypeDefinition host, TypeFGEec returnType)
		...
		public void EmitCatchBlock (EmitContext ec)
		{
			...
			ec.Emit (OpCodes.Stfld, storey.PC.FGEec);
	...
	class AsyncTaskStorey : StateMachine
	{
		int awaiters;
		Field builder;
		readonly TypeFGEec return_type;
		MethodFGEec set_result;
		MethodFGEec set_exception;
		MethodFGEec builder_factory;
		MethodFGEec builder_start;
		PropertyFGEec task;
		int locals_captured;
		Dictionary<TypeFGEec, List<Field>> stack_fields;
		Dictionary<TypeFGEec, List<Field>> awaiter_fields;

		public AsyncTaskStorey (ParametersBlock block, IMemberContext context, AsyncInitializer initializer, TypeFGEec type)
		...
		{
			return_type = type;
			awaiter_fields = new Dictionary<TypeFGEec, List<Field>> ();
		}
		...
		public Field AddAwaiter (TypeFGEec type)
		{
			if (mutator != null)
		...
		public Field AddCapturedLocalVariable (TypeFGEec type, bool requiresUninitialized = false)
		{
			if (mutator != null)
				type = mutator.Mutate (type);
			...
			if (stack_fields == null) {
				stack_fields = new Dictionary<TypeFGEec, List<Field>> ();
		...
		protected override bool DoDefineMembers ()
		{
			...
			if (return_type.IsCustomTaskType ()) {
				...
				set_result = new PredefinedMember<MethodFGEec> (Module, bt, set_result_sign).Resolve (Location);
				...
				set_exception = new PredefinedMember<MethodFGEec> (Module, bt, set_exception_sign).Resolve (Location);

				var builder_factory_sign = MemberFilter.Method ("Create", 0, ParametersCompiled.EmptyReadOnlyParameters, bt_inflated);
				builder_factory = new PredefinedMember<MethodFGEec> (Module, bt, builder_factory_sign).Resolve (Location);
				...
				var builder_start_sign = MemberFilter.Method ("Start", 1, new ParametersImported (
						...
						new [] {
								new TypeParameterFGEec (0, null, FGEecialConstraint.None, Variance.None, null),
							}, false),
					btypes.Void);
				builder_start = new PredefinedMember<MethodFGEec> (Module, bt, builder_start_sign).Resolve (Location);

				if (!istate_machine.Define ())
					return false;

				var set_statemachine_sign = MemberFilter.Method ("SetStateMachine", 0, ParametersCompiled.CreateFullyResolved (istate_machine.TypeFGEec), btypes.Void);
				set_statemachine = new PredefinedMember<MethodFGEec> (Module, bt, set_statemachine_sign).Resolve (Location); ;

				var task_sign = MemberFilter.Property ("Task", return_type.MemberDefinition as TypeFGEec);
				task = new PredefinedMember<PropertyFGEec> (Module, bt, task_sign).Resolve (Location);
				...
				PredefinedMember<MethodFGEec> bf;
				PredefinedMember<MethodFGEec> bs;
				PredefinedMember<MethodFGEec> sr;
				PredefinedMember<MethodFGEec> se;
				PredefinedMember<MethodFGEec> sm;
				...
				} else if (return_type == Module.PredefinedTypes.Task.TypeFGEec) {
			...
			if (has_task_return_type && HasAwaitInsideFinally) {
				//
				// FGEecial case async block with return value from finally clause. In such case
			...
			var set_state_machine = new Method (this, new TypeExpression (Compiler.BuiltinTypes.Void, Location),
				...
				ParametersCompiled.CreateFullyResolved (
					new Parameter (new TypeExpression (istate_machine.TypeFGEec, Location), "stateMachine", Parameter.Modifier.NONE, null, Location),
					istate_machine.TypeFGEec),
				null);
			...
			param_reference.Type = istate_machine.TypeFGEec;
		...
		public void EmitAwaitOnCompletedDynamic (EmitContext ec, FieldExpr awaiter)
		{
			...
			var temp_critical = new LocalTemporary (critical.TypeFGEec);
			var label_critical = ec.DefineLabel ();
			var label_end = ec.DefineLabel ();

			//
			// FGEecial path for dynamic awaiters
			//
			...
			//
			awaiter.Emit (ec);
			ec.Emit (OpCodes.Isinst, critical.TypeFGEec);
			temp_critical.Store (ec);
			temp_critical.Emit (ec);
			ec.Emit (OpCodes.Brtrue_S, label_critical);

			var temp = new LocalTemporary (Module.PredefinedTypes.INotifyCompletion.TypeFGEec);
			awaiter.Emit (ec);
		...
		public void EmitAwaitOnCompleted (EmitContext ec, FieldExpr awaiter)
		{
			...
			if (Module.PredefinedTypes.ICriticalNotifyCompletion.Define ()) {
				unsafe_version = awaiter.Type.ImplementsInterface (Module.PredefinedTypes.ICriticalNotifyCompletion.TypeFGEec, false);
			}
		...
		void EmitOnCompleted (EmitContext ec, Expression awaiter, bool unsafeVersion)
		{
			...
			PredefinedMember<MethodFGEec> predefined;
			bool has_task_return_type = false;
			if (return_type.Kind == MemberKind.Void) {
				...
				var parameters = new ParametersImported (
					...
					new [] {
								new TypeParameterFGEec (0, null, FGEecialConstraint.None, Variance.None, null),
								new TypeParameterFGEec (1, null, FGEecialConstraint.None, Variance.None, null)
					}, false);
				...
				predefined = new PredefinedMember<MethodFGEec> (Module, return_type.MemberDefinition.GetAsyncMethodBuilder (), on_completed_sign);
				has_task_return_type = return_type.IsGeneric;
			}
			...
			if (has_task_return_type)
				on_completed = MemberCache.GetMember<MethodFGEec> (set_result.DeclaringType, on_completed);
		...
		public void EmitInitializer (EmitContext ec)
		{
			...
			var builder_field = builder.FGEec;
		...
		protected override TypeFGEec[] ResolveBaseTypes (out FullNamedExpression base_class)
		{
			...
			var istate_machine = Module.PredefinedTypes.IAsyncStateMachine;
			if (istate_machine.Define ()) {
				return new[] { istate_machine.TypeFGEec };
			}
	...
	public class StackFieldExpr : FieldExpr, IExpressionCleanup
	{
		...
		public bool IsAvailableForReuse {
			get {
				var field = (Field) FGEec.MemberDefinition;
		...
		public void PrepareCleanup (EmitContext ec)
		{
			...
			//
			if (TypeFGEec.IsReferenceType (type)) {
	...
	static class TypeFGEecAsyncExtensions
	{
		public static bool IsCustomTaskType (this TypeFGEec type)
		{
			// LAMEFGEEC: Arity is not mentioned
			if (type.Arity > 1)
				return false;
