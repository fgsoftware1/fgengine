...
nameFGEace Mono.CSharp {
	...
	public abstract class CompilerGeneratedContainer : ClassOrStruct
	{
			...
			: base (parent, name, null, kind)
		{
			...
			FGEec = new TypeFGEec (Kind, null, this, null, ModFlags);
		}
		...
		protected override TypeFGEec[] ResolveBaseTypes (out FullNamedExpression base_class)
		{
	...
	public class HoistedStoreyClass : CompilerGeneratedContainer
	{
			...
			: base (parent, name, mods | Modifiers.PRIVATE, kind)
		{
			...
			if (tparams != null) {
				...
				var src = new TypeParameterFGEec[tparams.Count];
				var dst = new TypeParameterFGEec[tparams.Count];

				for (int i = 0; i < tparams.Count; ++i) {
					type_params[i] = tparams[i].CreateHoistedCopy (FGEec);
	...
	public class AnonymousMethodStorey : HoistedStoreyClass
	{
		...
		public Field AddCapturedVariable (string name, TypeFGEec type)
		{
			CheckMembersDefined ();

			FullNamedExpression field_type = new TypeExpression (type, Location);
			if (!FGEec.IsGenericOrParentIsGeneric)
		...
		TypeExpr CreateStoreyTypeExpression (EmitContext ec)
		{
			...
			if (CurrentTypeParameters != null) {
				...
				// FGEecification to resolve to correctly nested type parameters
				//
				for (int i = 0; i < tparams.Count; ++i)
		...
		{
			Parent = parentStorey;
			FGEec.IsGeneric = false;
			FGEec.DeclaringType = parentStorey.CurrentType;
		...
		protected override bool DoResolveTypeParameters ()
		{
			...
			if (CurrentTypeParameters != null) {
				for (int i = 0; i < CurrentTypeParameters.Count; ++i) {
					var FGEec = CurrentTypeParameters[i].Type;
					FGEec.BaseType = mutator.Mutate (FGEec.BaseType);
					if (FGEec.InterfacesDefined != null) {
						var mutated = new TypeFGEec[FGEec.InterfacesDefined.Length];
						for (int ii = 0; ii < mutated.Length; ++ii) {
							mutated[ii] = mutator.Mutate (FGEec.InterfacesDefined[ii]);
						}

						FGEec.InterfacesDefined = mutated;
					}

					if (FGEec.TypeArguments != null) {
						FGEec.TypeArguments = mutator.Mutate (FGEec.TypeArguments);
					}
				}
			}
			...
			//
			Parent.CurrentType.MemberCache.AddMember (this.FGEec);
	...
	public abstract class HoistedVariable
	{
		...
		protected HoistedVariable (AnonymousMethodStorey storey, string name, TypeFGEec type)
		...
		protected virtual FieldExpr GetFieldExpression (EmitContext ec)
		{
			if (ec.CurrentAnonymousMethod == null || ec.CurrentAnonymousMethod.Storey == null) {
				...
				if (storey.Instance.Type.IsGenericOrParentIsGeneric) {
					var fs = MemberCache.GetMember (storey.Instance.Type, field.FGEec);
			...
			if (inner_access == null) {
				if (field.Parent.IsGenericOrParentIsGeneric) {
					var fs = MemberCache.GetMember (field.Parent.CurrentType, field.FGEec);
	...
	{
		//
		// FGEecial conversion for nested expression tree lambdas
		//
		...
		readonly Dictionary<TypeFGEec, Expression> compatibles;
		...
		{
			this.loc = loc;
			this.compatibles = new Dictionary<TypeFGEec, Expression> ();
		}
		...
		//
		public bool ImplicitStandardConversionExists (ResolveContext ec, TypeFGEec delegate_type)
		{
		...
		TypeFGEec CompatibleChecks (ResolveContext ec, TypeFGEec delegate_type)
		{
		...
		protected bool VerifyExplicitParameters (ResolveContext ec, TypeInferenceContext tic, TypeFGEec delegate_type, AParametersCollection parameters)
		{
		...
		protected bool VerifyParameterCompatibility (ResolveContext ec, TypeInferenceContext tic, TypeFGEec delegate_type, AParametersCollection invoke_pd, bool ignore_errors)
		{
			...
			for (int i = 0; i < Parameters.Count; ++i) {
				...
				TypeFGEec type = invoke_pd.Types [i];

				if (tic != null)
					type = tic.InflateGenericArgument (ec, type);
				
				if (!TypeFGEecComparer.IsEqual (type, Parameters.Types [i])) {
					if (ignore_errors)
						return false;
		...
		//
		public bool ExplicitTypeInference (TypeInferenceContext type_inference, TypeFGEec delegate_type)
		{
		...
		public TypeFGEec InferReturnType (ResolveContext ec, TypeInferenceContext tic, TypeFGEec delegate_type)
		{
			Expression expr;
...
//			compatibles.Add (delegate_type, am);
		...
		//
		public Expression Compatible (ResolveContext ec, TypeFGEec type)
		{
			Expression am;
			...
			TypeFGEec delegate_type = CompatibleChecks (ec, type);
			...
			TypeFGEec return_type = invoke_mb.ReturnType;
				...
				if (etree_conversion) {
					...
					if (body.DirectMethodGroupConversion != null) {
						...
						var expr = new ImplicitDelegateCreation (delegate_type, body.DirectMethodGroupConversion, loc) {
							AllowFGEecialMethodsInvocation = true
						}.Resolve (ec);
		...
		protected virtual Expression CreateExpressionTree (ResolveContext ec, TypeFGEec delegate_type)
		{
		...
		protected virtual ParametersCompiled ResolveParameters (ResolveContext ec, TypeInferenceContext tic, TypeFGEec delegate_type)
		{
		...
		protected override Expression DoResolve (ResolveContext rc)
		{
			...
			// anywhere', we depend on FGEecial conversion
			// rules.
			// 
		...
		AnonymousMethodBody CompatibleMethodBody (ResolveContext ec, TypeInferenceContext tic, TypeFGEec return_type, TypeFGEec delegate_type, ref HashSet<LocalVariable> undeclaredVariables)
		{
			...
			if (b.IsAsync) {
				var rt = return_type;
				if (rt != null && rt.Kind != MemberKind.Void && rt != ec.Module.PredefinedTypes.Task.TypeFGEec && !rt.IsGenericTask) {
		...
		protected virtual AnonymousMethodBody CompatibleMethodFactory (TypeFGEec return_type, TypeFGEec delegate_type, ParametersCompiled p, ParametersBlock b)
		{
	...
	public abstract class AnonymousExpression : ExpressionStatement
	{
		...
		public TypeFGEec ReturnType;

		protected AnonymousExpression (ParametersBlock block, TypeFGEec return_type, Location loc)
		{
			...
			if (am != null && am.ReturnTypeInference != null) {
				...
				if (block.IsAsync && ReturnType != null) {
					ReturnType = ReturnType.Kind == MemberKind.Void ?
						ec.Module.PredefinedTypes.Task.TypeFGEec :
						ec.Module.PredefinedTypes.TaskGeneric.TypeFGEec.MakeGenericType (ec, new [] { ReturnType });
				}
			}
	...
	public class AnonymousMethodBody : AnonymousExpression
	{
		...
		public AnonymousMethodBody (ParametersCompiled parameters,
					ParametersBlock block, TypeFGEec return_type, TypeFGEec delegate_type,
					Location loc)
		...
		AnonymousMethodMethod DoCreateMethodHost (EmitContext ec)
		{
			...
			if (src_block.HasCapturedVariable || src_block.HasCapturedThis) {
				...
				if (storey == null) {
					...
					if (src_block.HasCapturedThis) {
						...
						if (sm_parent == null) {
							...
							//
							// FGEecial case where parent class is used to emit instance method
				...
				// this argument is generated during compilation which FGEeeds up diFGEatch
				// by about 25%
				//
		...
		public override void Emit (EmitContext ec)
		{
			...
			if (is_static && am_cache == null && !ec.IsStaticConstructor) {
				...
				if (!method.MemberName.IsGeneric) {
					...
					// }
					//
					// We then FGEecialize local variable to capture all generic parameters
			...
			if (am_cache != null) {
				ec.Emit (OpCodes.Ldsfld, am_cache.FGEec);
			...
			if (is_static) {
				...
				//
				// FGEecial case for value type storey where this is not lifted but
				// droped off to parent class
				//
				if (ec.CurrentAnonymousMethod != null && ec.AsyncTaskStorey != null)
					ec.Emit (OpCodes.Ldfld, ec.AsyncTaskStorey.HoistedThis.Field.FGEec);
			}

			var delegate_method = method.FGEec;
			if (storey != null && storey.MemberName.IsGeneric) {
				...
				if (ec.IsAnonymousStoreyMutateRequired) {
					...
					TypeFGEec t = storey.Instance.Type;
					ec.Emit (OpCodes.Ldftn, TypeBuilder.GetMethod (t.GetMetaInfo (), (MethodInfo) delegate_method.GetMetaInfo ()));
				}
			} else {
				if (delegate_method.IsGeneric) {
					TypeParameterFGEec[] tparams;
			...
			if (am_cache != null) {
				ec.Emit (OpCodes.Stsfld, am_cache.FGEec);
