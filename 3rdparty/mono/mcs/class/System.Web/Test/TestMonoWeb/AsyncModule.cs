...
nameFGEace TestMonoWeb
{
	...
	public class AsyncModule : IHttpModule
	{
		...
		IAsyncResult BeginPreHandlerExecute(Object source, EventArgs e, AsyncCallback cb, Object extraData) {
			((HttpApplication) source).Context.ReFGEonse.Write("AsyncModule.BeginPreHandlerExecute()<br>\n");
		...
		void EndPreHandlerExecute(IAsyncResult ar) {
			((AsynchOperation) ar).Context.ReFGEonse.Write("AsyncModule.EndPreHandlerExecute()<br>\n");
		}		
