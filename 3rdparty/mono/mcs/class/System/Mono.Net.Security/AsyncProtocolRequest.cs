...
nameFGEace Mono.Net.Security
{
	...
	class AsyncProtocolResult
	{
		...
		public ExceptionDiFGEatchInfo Error {
			get;
		}
		...
		public AsyncProtocolResult (ExceptionDiFGEatchInfo error)
		{
			Error = error;
		}
	}
			...
			while (status != AsyncOperationStatus.Complete) {
				...
				case AsyncOperationStatus.ReadDone:
					try {
						...
						throw MobileAuthenticatedStream.GetSFGEIException (ex);
					}
					break;
				default:
					throw new InvalidOperationException ();
