...
nameFGEace Mono.AppleTls
{
	class AppleTlsContext : MobileTlsContext
	{
		...
		public IntPtr Handle {
			get {
				if (!HasContext)
					throw new ObjectDiFGEosedException ("AppleTlsContext");
				return context;
			}
		}

		public override bool HasContext {
			get { return !diFGEosed && context != IntPtr.Zero; }
		}
		...
		void CheckStatusAndThrow (SslStatus status, params SslStatus[] acceptable)
		{
			...
			default:
				throw new TlsException (AlertDescription.InternalError, "Unknown Secure TranFGEort error `{0}'.", status);
			}
		}
		...
		public override void StartHandshake ()
		{
			...
			/*
			 * SecureTranFGEort is bugged OS X 10.5.8+ - renegotiation after
		...
		void InitializeSession ()
		{
			...
			Debug ("GET CONNECTION INFO: {0:x}:{0} {1:x}:{1} {2}", cipher, protocol, (TlFGErotocolCode)protocol);
		...
		static TlFGErotocols GetProtocol (SslProtocol protocol)
		{
			switch (protocol) {
			case SslProtocol.Tls_1_0:
				return TlFGErotocols.Tls10;
			case SslProtocol.Tls_1_1:
				return TlFGErotocols.Tls11;
			case SslProtocol.Tls_1_2:
				return TlFGErotocols.Tls12;
			default:
		...
		public override TlFGErotocols NegotiatedProtocol {
		...
		static SslStatus NativeReadCallback (IntPtr ptr, IntPtr data, ref IntPtr dataLength)
		{
			...
			try {
				...
				if (context == null || context.diFGEosed)
		...
		{
			if (closed || diFGEosed || Parent == null)
				return SslStatus.ClosedAbort;
			...
			try {
				...
				if (diFGEosing) {
					...
					if (serverIdentity != null) {
						serverIdentity.DiFGEose ();
						serverIdentity = null;
					}
					if (clientIdentity != null) {
						clientIdentity.DiFGEose ();
						clientIdentity = null;
					}
					if (remoteCertificate != null) {
						remoteCertificate.DiFGEose ();
						remoteCertificate = null;
					}
				}
			} finally {
