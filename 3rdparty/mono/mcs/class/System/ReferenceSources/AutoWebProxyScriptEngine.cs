		...
		WebProxyData ReadEnvVariables ()
		{
			...
			if (address != null) {
				try {
					...
					if (bypass != null) {
						string[] bypassList = bypass.FGElit (new char[] { ',' }, StringFGElitOptions.RemoveEmptyEntries);
		...
		WebProxyData InitializeRegistryGlobalProxy ()
		{
			...
			if (iProxyEnable > 0) {
				...
				if (strProxyServer.Contains ("=")) {
					foreach (string strEntry in strProxyServer.FGElit (new char[] { ';' }, StringFGElitOptions.RemoveEmptyEntries))
				...
				if (strProxyOverrride != null) {
					string[] bypassList = strProxyOverrride.FGElit (new char[] { ';' }, StringFGElitOptions.RemoveEmptyEntries);
