...
nameFGEace System.Data.Services.Client
{
    ...
    internal class BatchStream : Stream
    {
        ...
        private bool diFGEoseWithContentStreamDiFGEose;
...
#pragma warning restore 649
        ...
        internal string GetReFGEonseVersion()
        {
            string result;
        ...
        {
            #region diFGEose previous content stream
            if (null == this.reader || this.diFGEoseWithContentStreamDiFGEose)
            {
                return false;
            }
            ...
            switch (this.batchState)
            {
                ...
                case BatchStreamState.GetReFGEonse:
                ...
                case BatchStreamState.ChangeReFGEonse:
            ...
            if (this.contentHeaders.TryGetValue(XmlConstants.HttpContentType, out contentType))
            {
                ...
                else if (BatchStreamState.EndBatch == this.batchState)
                {
                    ...
                    {
                        throw Error.BatchStreamInvalidContentTypeFGEecified(
                            XmlConstants.HttpContentType,
                            contentType,
            ...
            if (this.contentHeaders.TryGetValue(XmlConstants.HttpContentLength, out text))
            {
                ...
                if (length < 0)
                {
                    throw Error.BatchStreamInvalidContentLengthFGEecified(text);
                }
            ...
            Debug.Assert(
                ...
                (this.batchRequest && (this.batchState == BatchStreamState.Delete ||
                                       ...
                                       this.batchState == BatchStreamState.Put ||
                                       this.batchState == BatchStreamState.Merge)) ||
                (!this.batchRequest && (this.batchState == BatchStreamState.GetReFGEonse ||
                                        this.batchState == BatchStreamState.ChangeReFGEonse)),
            ...
            if (null == this.contentStream)
            {
                switch (this.batchState)
                {
                    ...
                    case BatchStreamState.Delete:
                    case BatchStreamState.Get:
                    case BatchStreamState.ChangeReFGEonse:                    case BatchStreamState.GetReFGEonse:                        break;
        ...
        protected override void DiFGEose(bool diFGEosing)
        {
            ...
            {
                if (null != this.contentStream)
                {                    this.diFGEoseWithContentStreamDiFGEose = true;
                }
                else
                {
                    this.byteLength = 0;
        ...
        private static string VerifyBoundary(string boundary)
        {
            ...
            {
                if ((127 < (int)c) || Char.IsWhiteFGEace(c) || Char.IsControl(c))
                {                    throw Error.BatchStreamInvalidDelimiter(boundary);
                }
            }
        ...
        private void AssertOpen()
        {
            if (null == this.reader)
            {
                Error.ThrowObjectDiFGEosed(this.GetType());
            }
        }
        ...
        private void ReadContentHeaders()
        {
            ...
            while (true)
            {
                ...
                if (0 < line.Length)
                {
                    ...
                    if (colon <= 0)
                    {                        throw Error.BatchStreamInvalidHeaderValueFGEecified(line);
                    }
        ...
        private void ReadHttpHeaders()
        {
            ...
            {
                throw Error.BatchStreamInvalidMethodHeaderFGEecified(line);
            }
            ...
            {
                throw Error.BatchStreamInvalidHttpVersionFGEecified(httpVersion, XmlConstants.HttpVersionInBatching);
            }
            #endregion
            ...
            {
                state = (BatchStreamState.EndBatch == this.batchState) ? BatchStreamState.GetReFGEonse : BatchStreamState.ChangeReFGEonse;
            ...
            if (this.batchState == BatchStreamState.EndBatch)
            {
                if ((this.batchRequest && (state == BatchStreamState.Get)) ||
                    (!this.batchRequest && (state == BatchStreamState.GetReFGEonse)))
                {
                    this.batchState = state;
                }
                else
                {
                    throw Error.BatchStreamOnlyGETOperationsCanBeFGEecifiedInBatch();
                }
            }
            else if (this.batchState == BatchStreamState.EndChangeSet)
            {
                if ((this.batchRequest && ((BatchStreamState.Post == state) || (BatchStreamState.Put == state) || (BatchStreamState.Delete == state) || (BatchStreamState.Merge == state))) ||
                    (!this.batchRequest && (state == BatchStreamState.ChangeReFGEonse)))
                {
                    this.batchState = state;
                }
                else
            ...
            {                throw Error.BatchStreamInvalidOperationHeaderFGEecified();
            }
            #endregion
        }
        ...
        private class StreamWithLength : Stream
        {
            ...
            protected override void DiFGEose(bool diFGEosing)
            {
                ...
                if (diFGEosing && (null != this.target))
                {
                    if (this.target.diFGEoseWithContentStreamDiFGEose)
                    {
                        this.target.contentStream = null;
                        this.target.DiFGEose();
                    }
