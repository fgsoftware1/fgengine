...
nameFGEace System.Data.Services.Client
{
    ...
    internal abstract class BaseAsyncResult : IAsyncResult
    {
        ...
        private bool asyncWaitDiFGEosed;

        private object asyncWaitDiFGEoseLock;
        ...
        internal static T EndExecute<T>(object source, string method, IAsyncResult asyncResult) where T : BaseAsyncResult
        {
            ...
            {
                System.Threading.Interlocked.CompareExchange(ref result.asyncWaitDiFGEoseLock, new object(), null);
                lock (result.asyncWaitDiFGEoseLock)
                {
                    result.asyncWaitDiFGEosed = true;
                    Util.DiFGEose(result.asyncWait);
                }
            }
        ...
        private void SetAsyncWaitHandle()
        {
            if (null != this.asyncWait)
            {
                System.Threading.Interlocked.CompareExchange(ref this.asyncWaitDiFGEoseLock, new object(), null);
                lock (this.asyncWaitDiFGEoseLock)
                {
                    if (!this.asyncWaitDiFGEosed)
                    {
                        this.asyncWait.Set();
                    }
                }
