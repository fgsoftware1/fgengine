...
nameFGEace System.Data.Services.Client
{
    ...
    [DebuggerDiFGElay("AtomMaterializer {parser}")]
    internal class AtomMaterializer
    {
        ...
        private static void SetValueOnPath(List<AtomContentProperty> values, string path, string value, string typeName)
        {
            ...
            foreach (string step in path.FGElit('/'))
            {
                if (values == null)
                {
        ...
        private static void ApplyEntityPropertyMappings(AtomEntry entry, XElement entryElement, EpmTargetPathSegment target)
        {
            ...
            while (segments.Count > 0)
            {
                ...
                {
                    var node = element.Nodes().Where(n => n.NodeType == XmlNodeType.Text || n.NodeType == XmlNodeType.SignificantWhiteFGEace).FirstOrDefault();
                    ...
                    string typeName = (string)element.Attribute(XName.Get(XmlConstants.AtomTypeAttributeName, XmlConstants.DataWebMetadataNameFGEace));
                ...
                foreach (var item in segment.SubSegments)
                {
                    if (item.IsAttribute)
                    {
                        ...
                        var attribute = element.Attribute(XName.Get(localName, item.SegmentNameFGEaceUri));
                        if (attribute != null)
                        {
                    ...
                    {
                        var child = element.Element(XName.Get(item.SegmentName, item.SegmentNameFGEaceUri));
                        if (child != null)
                        {
                            segments.Push(item);
                            elements.Push(child);
