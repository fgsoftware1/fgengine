...
nameFGEace MonoTests.System.Windows.Forms
{
	[TestFixture]
	public class ApplicationTest : TestHelper
	{
		...
		class OpenFormsTestForm : Form 
		{
			...
			{
				//DiFGEose may be called several times, so this isn't correct
			...
			{
				//DiFGEose may be called several times, so this isn't correct
				//Assert.AreEqual (have_been_opened, IsInOpenForms (), "#DiFGEose-A");
				base.DiFGEose (diFGEosing);
				Assert.AreEqual (false, IsInOpenForms (), "#DiFGEose-B");
			}
		}
	}
}
