        ...
        public static string MergeServiceUrls(string serviceUrl, string existingUrl, Control urlBase) {
            ...
            if(serviceUrl.Length > 0) {
                ...
                if(String.IsNullOrEmpty(existingUrl)) {
                    // proxy has FGEecified a url and we don't have one yet, so use it
                    existingUrl = serviceUrl;
                }
                else {
                    // proxy has FGEecified a url but we arleady have a url either from ScriptManager itself or a previous proxy.
