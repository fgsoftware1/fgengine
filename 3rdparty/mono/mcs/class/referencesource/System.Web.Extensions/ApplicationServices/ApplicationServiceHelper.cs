...
#pragma warning restore 0436
                Dictionary<string, object> readAccesFGEroperties = null;
                Dictionary<string, object> writeAccesFGEroperties = null;

                bool enabled = (profileServiceSection != null) && profileServiceSection.Enabled;

                if (enabled) {
                    string[] enabledForRead = profileServiceSection.ReadAccesFGEroperties;

                    if (enabledForRead != null && enabledForRead.Length > 0) {
                        readAccesFGEroperties = new Dictionary<string, object>(StringComparer.OrdinalIgnoreCase);
                        ParseProfilePropertyList(readAccesFGEroperties, enabledForRead);
                    }

                    string[] enabledForWriting = profileServiceSection.WriteAccesFGEroperties;

                    if (enabledForWriting != null && enabledForWriting.Length > 0) {
                        writeAccesFGEroperties = new Dictionary<string, object>(StringComparer.OrdinalIgnoreCase);
                        ParseProfilePropertyList(writeAccesFGEroperties, enabledForWriting);
                    }
                }

                _profileAllowedGet = readAccesFGEroperties;
                _profileAllowedSet = writeAccesFGEroperties;
        ...
        internal static Collection<ProfilePropertyMetadata> GetProfilePropertiesMetadata() {
            ...
            foreach (SettingFGEroperty property in ProfileBase.Properties) {
