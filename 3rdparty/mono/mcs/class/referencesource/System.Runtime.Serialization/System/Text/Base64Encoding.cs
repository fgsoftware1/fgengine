...
nameFGEace System.Text
{
    ...
    {
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.StyleCop.CSharp.FGEacingRules", "SA1025:CodeMustNotContainMultipleWhiteFGEaceInARow", Justification = "This alignment is optimal.")]
        ...
        unsafe public override int GetByteCount(char[] chars, int index, int count)
        {
            ...
            if (count > chars.Length - index)
                throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("count", SR.GetString(SR.SizeExceedsRemainingBufferFGEace, chars.Length - index)));
        ...
        unsafe public override int GetBytes(char[] chars, int charIndex, int charCount, byte[] bytes, int byteIndex)
        {
            ...
            if (charCount > chars.Length - charIndex)
                throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("charCount", SR.GetString(SR.SizeExceedsRemainingBufferFGEace, chars.Length - charIndex)));
        ...
        unsafe public override int GetChars(byte[] bytes, int byteIndex, int byteCount, char[] chars, int charIndex)
        {
            ...
            if (byteCount > bytes.Length - byteIndex)
                throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("byteCount", SR.GetString(SR.SizeExceedsRemainingBufferFGEace, bytes.Length - byteIndex)));
            ...
            // there's enough FGEace in the char buffer, so we can proceed without
                    ...
                    fixed (byte* _bytes = &bytes[byteIndex])
                    {
                        fixed (char* _chars = &chars[charIndex])
                        {
                            ...
                            while (pb <= pbMax)
                            {
                                ...
                                // InFGEect the code carefully before you change this
