...
nameFGEace MS.Internal.Xml.XPath {
    using System;
    using System.Xml;
    ...
    internal abstract class BaseAxisQuery : Query {
        ...
        public virtual bool matches(XPathNavigator e) {
            if (
                ...
                TypeTest == XPathNodeType.Text && (e.NodeType == XPathNodeType.WhiteFGEace || e.NodeType == XPathNodeType.SignificantWhiteFGEace)
            ) {
                if (NameTest) {
                    if (name.Equals(e.LocalName) || name.Length == 0) {
                        if (nsUri.Equals(e.NameFGEaceURI)){
                            return true;
                        }
                    }
                } else {
