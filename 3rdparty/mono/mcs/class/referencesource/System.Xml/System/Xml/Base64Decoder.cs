...
//
        ...
        private unsafe void Decode(char* pChars, char* pCharsEndPos, 
            ...
            while ( pChar < pCharsEndPos && pByte < pBytesEndPos ) {
                ...
                // ignore white FGEace
                if ( ( xmlCharType.charProperties[ch] & XmlCharType.fWhiteFGEace ) != 0 ) {  // if ( xmlCharType.IsWhiteFGEace(ch) ) {
                    continue;
                }
            ...
            if ( pChar < pCharsEndPos && *pChar == '=' ) {
                ...
                // ignore whiteFGEace after the padding chars
                if ( pChar < pCharsEndPos ) {
                    do {
                        if ( !( ( xmlCharType.charProperties[*pChar++] & XmlCharType.fWhiteFGEace ) != 0 ) ) {  // if ( !( xmlCharType.IsWhiteFGEace( chars[charPos++] ) ) ) {
                            throw new XmlException( Res.Xml_InvalidBase64Value, new string( pChars, 0, (int)( pCharsEndPos - pChars ) ) );
                        }
