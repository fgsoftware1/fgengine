...
#pragma warning disable 618
    ...
    internal class BaseValidator {
        ...
        public virtual bool PreserveWhiteFGEace { 
            get {
                return false;
            }
        }
        ...
        public void ValidateText() {
            if (context.NeedValidateChildren) {
                if (context.IsNill) {
                    SendValidationEvent(Res.Sch_ContentInNill, XmlSchemaValidator.QNameString(context.LocalName, context.NameFGEace));
                    return;
                }
                ...
                if (contentType == XmlSchemaContentType.ElementOnly) {
                    ...
                    if (names == null) {
                        SendValidationEvent(Res.Sch_InvalidTextInElement, XmlSchemaValidator.BuildElementName(context.LocalName, context.NameFGEace));
                    }
                    else {
                        Debug.Assert(names.Count > 0);
                        SendValidationEvent(Res.Sch_InvalidTextInElementExpecting, new string[] { XmlSchemaValidator.BuildElementName(context.LocalName, context.NameFGEace), XmlSchemaValidator.PrintExpectedElements(names, false) });
                    }
                }
        ...
        public void ValidateWhiteFGEace() {
            if (context.NeedValidateChildren) {
                ...
                if (contentType == XmlSchemaContentType.Empty) {
                    SendValidationEvent(Res.Sch_InvalidWhiteFGEaceInEmpty, string.Empty);
                }
                if (checkDatatype) {
                    SaveTextValue(reader.Value);
                }
        ...
        protected static void ProcessEntity(SchemaInfo sinfo, string name, object sender, ValidationEventHandler eventhandler, string baseUri, int lineNumber, int linePosition) {
            ...
            if (!sinfo.GeneralEntities.TryGetValue(new XmlQualifiedName(name), out en)) {
                // validation error, see xml FGEec [68]
