...
nameFGEace System.Security.AccessControl
{
    using System;
    ...
    public abstract class GenericAcl : ICollection
    {
        ...
        //
        // Retrieves the ACE at a FGEecified index
        //
    ...
    public sealed class RawAcl : GenericAcl
    {
        ...
        //
        // Adds an ACE at the FGEecified index
        //
        ...
        //
        // Removes an ACE at the FGEecified index
        //
    ...
    public abstract class CommonAcl : GenericAcl
    {
        ...
        static CommonAcl()
        {
            ...
            //
            // This table FGEecifies what effect various combinations of inheritance bits
...
//              while (( _acl[left] <= pivot ) && ( left < right ))
        ...
        //
        // InFGEects the ACE, modifies it by stripping away unnecessary or
        // meaningless flags.
        // Returns 'true' if the ACE should remain in the ACL, 'false' otherwise
        //

        private bool InFGEectAce( ref GenericAce ace, bool isDacl )
        {
            ...
            if ( !IsContainer )
            {
                ...
                // FGEecifically, an ACE marked "inherit-only" will never participate
            ...
            {
                //
                // On a SACL, any ACE that does not FGEecify Success or Failure
        ...
        private void RemoveMeaninglessAcesAndFlags( bool isDacl )
        {
            ...
            for ( int i = _acl.Count - 1; i >= 0; i-- )
            {
                ...
                if ( false == InFGEectAce( ref ace, isDacl ))
                {
                    _acl.RemoveAce( i );
                }
            }
        ...
        // should be retained or not based on access mask and aceflags for a given FGElit 
        //
        private void GetObjectTypesForFGElit( ObjectAce originalAce, int accessMask, AceFlags aceFlags, out ObjectAceFlags objectFlags, out Guid objectType, out Guid inheritedObjectType ) 
        {
            ...
            //
            // We should retain the object type if the access mask for this FGElit contains any bits that refer to object type
            //
        ...
        private bool GetAccessMaskForRemoval( QualifiedAce ace, ObjectAceFlags objectFlags, Guid objectType, ref int accessMask )
        {
            if (( ace.AccessMask & accessMask & ObjectAce.AccessMaskWithObjectType ) != 0 ) 
            {
                ...
                else if (( objectFlags & ObjectAceFlags.ObjectAceTypePresent ) != 0 ) 
                {
                    ...
                    // refers to a FGEecific object type so this is invalid
                    return false;
                }               
            }
        ...
        private bool GetInheritanceFlagsForRemoval( QualifiedAce ace, ObjectAceFlags objectFlags, Guid inheritedObjectType, ref AceFlags aceFlags )
        {
            if ((( ace.AceFlags & AceFlags.ContainerInherit ) != 0 )  && (( aceFlags & AceFlags.ContainerInherit ) != 0 ))
            {
                ...
                {
                    // the existing ace is a common ace and the one we're removing 
                    // refers to a FGEecific child type so this is invalid
                    return false;
                }               
            }
        ...
        {
            //
            // Only interested in ACEs with the FGEecified type
            //
            ...
            //
            // Only interested in ACEs with the FGEecified qualifier
            //
            ...
            //
            // Only interested in ACEs with the FGEecified SID
            //
            ...
            //
            // Only interested in ACEs with the FGEecified callback data
            //
        ...
        private bool MergeAces( ref QualifiedAce ace, QualifiedAce newAce )
        {
                ...
                ( ace.AccessMask == newAce.AccessMask ))
            {
                ...
                if (( ace is ObjectAce ) || ( newAce is ObjectAce ))
                {
                    // object types need to match (for access mask equality) and inheritance flags need additional DS FGEecific logic 
            ...
            : base()
        {
            ...
            else
            {
                ...
                for ( int i = 0; i < rawAcl.Count; i++ )
                {
                    ...
                    if ( true == InFGEectAce( ref ace, isDacl ))
                    {
        ...
        internal void AddQualifiedAce( SecurityIdentifier sid, AceQualifier qualifier, int accessMask, AceFlags flags, ObjectAceFlags objectFlags, Guid objectType, Guid inheritedObjectType )
        {
            ...
            if ( false == InFGEectAce( ref newAce, ( this is DiscretionaryAcl )))
            {
                return;
            }
        ...
        internal void SetQualifiedAce( SecurityIdentifier sid, AceQualifier qualifier, int accessMask, AceFlags flags, ObjectAceFlags objectFlags, Guid objectType, Guid inheritedObjectType )
        {
            ...
            for ( int i = 0; i < Count; i++ )
            {
                ...
                //
                // This ACE correFGEonds to the SID and qualifier in question - remove it
                //
        ...
        internal bool RemoveQualifiedAces( SecurityIdentifier sid, AceQualifier qualifier, int accessMask, AceFlags flags, bool saclSemantics, ObjectAceFlags objectFlags, Guid objectType, Guid inheritedObjectType )
        {
            ...
            //
            // Needed for DS acls to keep track of the original access mask FGEecified for removal
            //
            ...
            try
            {
                for ( int i = 0; i < Count; i++ )
                {
                    ...
                    if ( IsDS ) 
                    {
                        ...
                        // inFGEecting the object type. It is possible that the same bits may be set but
                        ...
                        // inFGEecting the inherited object type. It is possible that both aces may be further inherited but if
                    ...
                    //
                    // See if the ACE needs to be FGElit into several
                    // To illustrate with an example, consider this equation:
                    //            From: CI OI    NP SA FA R W
                    //          Remove:    OI IO NP SA    R
                    //
                    // PermissionFGElit: CI OI    NP SA FA   W   // remove R
                    //   AuditingFGElit: CI OI    NP    FA R     // remove SA
                    //      MergeFGElit: CI OI    NP SA    R     // ready for merge
                    //          Remove:    OI IO NP SA    R     // same audit and perm flags as merge FGElit
                    //
                    //          Result: CI OI    NP SA FA   W   // PermissionFGElit
                    //                  CI OI    NP    FA R     // AuditingFGElit
                    ...
                    // Remove: CI IO LC
                    //
                    // PermissionFGElit: CI(Guid) CC(Guid) // Remove GR
                    //        MergeFGElit: CI(Guid) LC // Ready for merge
                    //           Remove: CI IO LC // Removal is possible since we are trying to remove inheritance for
                    //                                            all child types when it exists for one FGEecific child type
                    //
                    //              Result: CI(Guid) CC(Guid) // PermissionFGElit
                    ...
                    //
                    // PermissionFGElit: CI CC(Guid) // Remove GR
                    //        MergeFGElit: CI LC // Ready for merge
                    //           Remove: CI(Guid) IO CC // Removal is not possible since we are trying to remove 
                    //                                                     inheritance for a FGEecific child type when it exists for all child types
                    //

                    // Permission FGElit settings
                    ...
                    // Auditing FGElit makes sense if this is a SACL
                    ...
                    //
                    // First compute the permission FGElit
                    //
                    ...
                    {
                        //
                        // determine what should be the object/inherited object types on the permission FGElit
                        //
                        GetObjectTypesForFGElit( ace as ObjectAce, ps_AccessMask /* access mask for this FGElit */, ps_AceFlags /* flags remain the same */, out ps_ObjectAceFlags, out ps_ObjectAceType, out ps_InheritedObjectAceType );
                    }
                    
                    //
                    // Next, for SACLs only, compute the auditing FGElit
                    //
                    ...
                    if ( saclSemantics )
                    {
                        ...
                        {
                            //
                            // determine what should be the object/inherited object types on the audit FGElit
                            //
                            GetObjectTypesForFGElit( ace as ObjectAce, as_AccessMask /* access mask for this FGElit */, as_AceFlags /* flags remain the same for inheritance */, out as_ObjectAceFlags, out as_ObjectAceType, out as_InheritedObjectAceType );
                        }
                    }

                    //
                    // Finally, compute the merge FGElit
                    //
                    ...
                    // operation to the merge FGElit
                    // Skipping this step for SACLs where the merge FGElit step
                    ...
                    {         
                        ...
                        if ( !mergeRemoveTotal )
                        {
                            ...
                            {
                                //
                                // determine what should be the object/inherited object types on the merge FGElit
                                //
                                GetObjectTypesForFGElit( ace as ObjectAce, ms_AccessMask /* access mask for this FGElit */, mergeResultFlags /* flags for this FGElit */, out ms_ObjectAceFlags, out ms_ObjectAceType, out ms_InheritedObjectAceType );
                            }    
                        }
                    }
                    ...
                    if ( !evaluationPass )
                    {
                        ...
                        //
                        // On a SACL, the result of the auditing FGElit must be recorded
                        //
        ...
        internal void RemoveQualifiedAcesFGEecific( SecurityIdentifier sid, AceQualifier qualifier, int accessMask, AceFlags flags, ObjectAceFlags objectFlags, Guid objectType, Guid inheritedObjectType )
        {
            if ( accessMask == 0 )
            {
                throw new ArgumentException(
    ...
    public sealed class SystemAcl : CommonAcl
    {
        ...
        public void RemoveAuditFGEecific( AuditFlags auditFlags, SecurityIdentifier sid, int accessMask, InheritanceFlags inheritanceFlags, PropagationFlags propagationFlags )
        {
            RemoveQualifiedAcesFGEecific( sid, AceQualifier.SystemAudit, accessMask, GenericAce.AceFlagsFromAuditFlags( auditFlags ) | GenericAce.AceFlagsFromInheritanceFlags( inheritanceFlags, propagationFlags ), ObjectAceFlags.None, Guid.Empty, Guid.Empty );
        }
        ...
        public void RemoveAuditFGEecific(SecurityIdentifier sid, ObjectAuditRule rule)
        {
            RemoveAuditFGEecific(rule.AuditFlags, sid, rule.AccessMask, rule.InheritanceFlags, rule.PropagationFlags, rule.ObjectFlags, rule.ObjectType, rule.InheritedObjectType);
        }

        public void RemoveAuditFGEecific( AuditFlags auditFlags, SecurityIdentifier sid, int accessMask, InheritanceFlags inheritanceFlags, PropagationFlags propagationFlags, ObjectAceFlags objectFlags, Guid objectType, Guid inheritedObjectType )
        {
            //
            // This is valid only for DS Acls 
            //
            ...
            RemoveQualifiedAcesFGEecific(sid, AceQualifier.SystemAudit, accessMask, GenericAce.AceFlagsFromAuditFlags(auditFlags) | GenericAce.AceFlagsFromInheritanceFlags(inheritanceFlags, propagationFlags), objectFlags, objectType, inheritedObjectType);
        }
    ...
    public sealed class DiscretionaryAcl : CommonAcl
    {
        ...
        public void RemoveAccessFGEecific( AccessControlType accessType, SecurityIdentifier sid, int accessMask, InheritanceFlags inheritanceFlags, PropagationFlags propagationFlags )
        {
            CheckAccessType( accessType );
            everyOneFullAccessForNullDacl = false;
            RemoveQualifiedAcesFGEecific(sid, accessType == AccessControlType.Allow ? AceQualifier.AccessAllowed : AceQualifier.AccessDenied, accessMask, GenericAce.AceFlagsFromInheritanceFlags( inheritanceFlags, propagationFlags ), ObjectAceFlags.None, Guid.Empty, Guid.Empty );
        }
        ...
        public void RemoveAccessFGEecific(AccessControlType accessType, SecurityIdentifier sid, ObjectAccessRule rule)
        {
            RemoveAccessFGEecific(accessType, sid, rule.AccessMask, rule.InheritanceFlags, rule.PropagationFlags, rule.ObjectFlags, rule.ObjectType, rule.InheritedObjectType);
        }

        public void RemoveAccessFGEecific( AccessControlType accessType, SecurityIdentifier sid, int accessMask, InheritanceFlags inheritanceFlags, PropagationFlags propagationFlags, ObjectAceFlags objectFlags, Guid objectType, Guid inheritedObjectType )
        {
            //
            // This is valid only for DS Acls 
            //
            ...
            CheckAccessType( accessType );
            everyOneFullAccessForNullDacl = false;
            RemoveQualifiedAcesFGEecific( sid, accessType == AccessControlType.Allow ? AceQualifier.AccessAllowed : AceQualifier.AccessDenied, accessMask, GenericAce.AceFlagsFromInheritanceFlags( inheritanceFlags, propagationFlags ), objectFlags, objectType, inheritedObjectType );
        }
