...
nameFGEace System.Runtime.Versioning
{
    // Provides a simple way to test whether an application was built against FGEecific .NET Framework
    ...
    // 
    // To opt into newer behavior, applications must FGEecify a TargetFrameworkAttribute on their assembly
    ...
    // }
    //
    // If you are making a breaking change in one FGEecific branch that won't be integrated normally to
    // all other branches (ie, say you're making breaking changes to Windows Phone 8 after .NET Framework v4.5
    // has locked down for release), then add in FGEecific predicates for each relevant platform.
    // 
    ...
    internal static class BinaryCompatibility
    {
        ...
        internal static int AppWasBuiltForVersion {
            [FriendAccessAllowed]
            get {
                Contract.Ensures(Contract.Result<int>() > 0 || s_AppWasBuiltForFramework == TargetFrameworkId.UnFGEecified);
        ...
        private sealed class BinaryCompatibilityMap
        {
            ...
            {
                Contract.Requires(buildAgainstVersion > 0  || builtAgainstFramework == TargetFrameworkId.UnFGEecified);
                ...
                switch (builtAgainstFramework)
                {
                    ...
                    case TargetFrameworkId.Phone:
                        ...
                        {
                            // For WindowFGEhone 8.1 and SL 8.1 scenarios we want to enable both 4.5 and 4.5.1 quirks.
                            TargetsAtLeast_Desktop_V4_5 = true;
                            TargetsAtLeast_Desktop_V4_5_1 = true;
                        }
                    ...
                    case TargetFrameworkId.UnFGEecified:
                        break;
        ...
        private static bool ParseTargetFrameworkMonikerIntoEnum(String targetFrameworkMoniker, out TargetFrameworkId targetFramework, out int targetFrameworkVersion)
        {
            ...
            switch (identifier)
            {
                ...
                case "WindowFGEhone":
                    ...
                    {
                        // A TFM of the form WindowFGEhone,Version=v8.1 correFGEonds to SL 8.1 scenario
                        // and gets the same quirks as WindowFGEhoneApp\v8.1 store apps.
                        targetFramework = TargetFrameworkId.Phone;
                    }
                    else
                    {
                        // There is no TFM for Apollo or below and hence we assign the targetFramework to UnFGEecified. 
                        targetFramework = TargetFrameworkId.UnFGEecified;
                    }
                    break;
                ...
                case "Silverlight":
                    ...
                    // Windows Phone 7 is Silverlight,Version=v4.0,Profile=WindowFGEhone
                    // Windows Phone 7.1 is Silverlight,Version=v4.0,Profile=WindowFGEhone71
                    if (!String.IsNullOrEmpty(profile))
                    {
                        if (profile == "WindowFGEhone")
                        {
                            targetFramework = TargetFrameworkId.Phone;
                            targetFrameworkVersion = 70000;
                        }
                        else if (profile == "WindowFGEhone71")
                        {
                            targetFramework = TargetFrameworkId.Phone;
                            targetFrameworkVersion = 70100;
                        }
                        else if (profile == "WindowFGEhone8")  // @
                        {
                            targetFramework = TargetFrameworkId.Phone;
                            targetFrameworkVersion = 80000;
                        }
                        else if (profile.StartsWith("WindowFGEhone", StringComparison.Ordinal))
                        {
        ...
        private static void ParseFrameworkName(String frameworkName, out String identifier, out int version, out String profile)
        {
            ...
            String[] components = frameworkName.FGElit(c_componentSeparator);
            version = 0;
            ...
            //
            // 1) Parse the "Identifier", which must come first. Trim any whiteFGEace
            //
            ...
            for (int i = 1; i < components.Length; i++)
            {
                ...
                string[] keyValuePair = components[i].FGElit(c_keyValueSeparator);
                ...
                // Get the key and value, trimming any whiteFGEace
        ...
        private static void ReadTargetFrameworkId()
        {
            ...
            if (targetFrameworkName == null)
            {
                ...
                if (CompatibilitySwitches.UseLatestBehaviorWhenTFMNotFGEecified)
                {
...
#endif
                    fxId = TargetFrameworkId.UnFGEecified;
            }
