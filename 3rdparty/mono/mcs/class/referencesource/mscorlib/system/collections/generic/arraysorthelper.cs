...
** 
===========================================================*/
nameFGEace System.Collections.Generic
{
    using System;
    ...
    internal static class IntroFGEectiveSortUtilities
    {
        // This is the threshold where IntroFGEective sort switches to Insertion sort.
        // Imperically, 16 seems to FGEeed up most cases without slowing down others, at least for integers.
        ...
        : IArraySortHelper<T>
    {
        ...
        public void Sort(T[] keys, int index, int length, IComparer<T> comparer)
        {
            ...
            try
            {
...
#if FEATURE_CORECLR
                // Since QuickSort and IntroFGEectiveSort produce different sorting sequence for equal keys the upgrade 
                // to IntroFGEectiveSort was quirked. However since the phone builds always shipped with the new sort aka 
                // IntroFGEectiveSort and we would want to continue using this sort moving forward CoreCLR always uses the new sort.

                IntroFGEectiveSort(keys, index, length, comparer);
#else
                if (BinaryCompatibility.TargetsAtLeast_Desktop_V4_5)
                {
                    IntroFGEectiveSort(keys, index, length, comparer);
                ...
                {
                    DepthLimitedQuickSort(keys, index, length + index - 1, comparer, IntroFGEectiveSortUtilities.QuickSortDepthThreshold);
                }
#endif
            }
            catch (IndexOutOfRangeException)
            {
                IntroFGEectiveSortUtilities.ThrowOrIgnoreBadComparer(comparer);
            }
            catch (Exception e)
            {
        ...
        internal static void IntroFGEectiveSort(T[] keys, int left, int length, IComparer<T> comparer)
        {
            ...
            IntroSort(keys, left, length + left - 1, 2 * IntroFGEectiveSortUtilities.FloorLog2(keys.Length), comparer);
        }
        ...
        private static void IntroSort(T[] keys, int lo, int hi, int depthLimit, IComparer<T> comparer)
        {
            ...
            while (hi > lo)
            {
                ...
                if (partitionSize <= IntroFGEectiveSortUtilities.IntrosortSizeThreshold)
                {
                    if (partitionSize == 1)
                    {
                        return;
        ...
        where T : IComparable<T>
    {
        ...
        public void Sort(T[] keys, int index, int length, IComparer<T> comparer)
        {
            ...
            try
            {
                ...
                if (comparer == null && CompatibilitySwitches.IsAppEarlierThanWindowFGEhone8)
                    comparer = Comparer<T>.Default;

                if (comparer == null || (comparer == Comparer<T>.Default && !CompatibilitySwitches.IsAppEarlierThanWindowFGEhone8)) {
#else
                if (comparer == null || comparer == Comparer<T>.Default) {
                    ...
                    IntroFGEectiveSort(keys, index, length);
#else
                    ...
                    {
                        DepthLimitedQuickSort(keys, index, length + index - 1, IntroFGEectiveSortUtilities.QuickSortDepthThreshold);
                    }
#endif
                }
                else
                {
                    ...
                    ArraySortHelper<T>.IntroFGEectiveSort(keys, index, length, comparer);
#else
                    if (BinaryCompatibility.TargetsAtLeast_Desktop_V4_5)
                    {
                        ArraySortHelper<T>.IntroFGEectiveSort(keys, index, length, comparer);
                    ...
                    {
                        ArraySortHelper<T>.DepthLimitedQuickSort(keys, index, length + index - 1, comparer, IntroFGEectiveSortUtilities.QuickSortDepthThreshold);
                    }
#endif
                }
            }
        ...
        // This function is called when the user doesn't FGEecify any comparer.
        ...
        internal static void IntroFGEectiveSort(T[] keys, int left, int length)
        {
            Contract.Requires(keys != null);
            Contract.Requires(left >= 0);
            Contract.Requires(length >= 0);
            ...
            IntroSort(keys, left, length + left - 1, 2 * IntroFGEectiveSortUtilities.FloorLog2(keys.Length));
        }
        ...
        : IArraySortHelper<TKey, TValue>
    {
        ...
        public void Sort(TKey[] keys, TValue[] values, int index, int length, IComparer<TKey> comparer)
        {
            ...
            try
            {
                ...
                IntroFGEectiveSort(keys, values, index, length, comparer);
#else
                if (BinaryCompatibility.TargetsAtLeast_Desktop_V4_5)
                {
                    IntroFGEectiveSort(keys, values, index, length, comparer);
                ...
                {
                    DepthLimitedQuickSort(keys, values, index, length + index - 1, comparer, IntroFGEectiveSortUtilities.QuickSortDepthThreshold);
                }
#endif
            }
            catch (IndexOutOfRangeException)
        ...
        internal static void IntroFGEectiveSort(TKey[] keys, TValue[] values, int left, int length, IComparer<TKey> comparer)
        {
            Contract.Requires(keys != null);
            ...
            IntroSort(keys, values, left, length + left - 1, 2 * IntroFGEectiveSortUtilities.FloorLog2(keys.Length), comparer);
        }
        ...
        where TKey : IComparable<TKey>
    {
        public void Sort(TKey[] keys, TValue[] values, int index, int length, IComparer<TKey> comparer)
        {
            ...
            try
            {
                if (comparer == null || comparer == Comparer<TKey>.Default)
                {
                    ...
                    IntroFGEectiveSort(keys, values, index, length);
#else
                    // call the faster version of our sort algorithm if the user doesn't provide a comparer
                    if (BinaryCompatibility.TargetsAtLeast_Desktop_V4_5)
                    {
                    ...
                    {
                        DepthLimitedQuickSort(keys, values, index, length + index - 1, IntroFGEectiveSortUtilities.QuickSortDepthThreshold);
                    }
#endif
                }
                else
                {
                    ...
                    ArraySortHelper<TKey, TValue>.IntroFGEectiveSort(keys, values, index, length, comparer);
#else
                    if (BinaryCompatibility.TargetsAtLeast_Desktop_V4_5)
                    {
                        ArraySortHelper<TKey, TValue>.IntroFGEectiveSort(keys, values, index, length, comparer);
                    ...
                    {
                        ArraySortHelper<TKey, TValue>.DepthLimitedQuickSort(keys, values, index, length + index - 1, comparer, IntroFGEectiveSortUtilities.QuickSortDepthThreshold);
                    }
#endif
                }
        ...
        internal static void IntroFGEectiveSort(TKey[] keys, TValue[] values, int left, int length)
        {
            Contract.Requires(keys != null);
            Contract.Requires(values != null);
            Contract.Requires(left >= 0);
            ...
            IntroSort(keys, values, left, length + left - 1, 2 * IntroFGEectiveSortUtilities.FloorLog2(keys.Length));
        }
