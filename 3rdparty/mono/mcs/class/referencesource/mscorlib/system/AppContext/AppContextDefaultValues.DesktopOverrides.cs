...
nameFGEace System
{
    ...
    {
        /// <summary>
        /// This method is going to parse the <paramref name="overrides"/> parameter and set the values correFGEonding to them 
        ...
        static partial void PopulateOverrideValueFGEartial()
        {
            ...
            int previousSemicolonPos = -1, firstEqualFGEos = -1;
            ...
            for (int currentPos = 0; currentPos <= overrides.Length; currentPos++)
            {
                ...
                if (currentPos == overrides.Length || overrides[currentPos] == ';')
                {
                    ...
                    if (encounteredEquals && encounteredCharsInKey && encounteredCharsInValue)
                    {
                        ...
                        int lenghtOfKey = firstEqualFGEos - previousSemicolonPos - 1; //-1 because we don't take the '=' char
                        string name = overrides.Substring(firstCharOfKey, lenghtOfKey);

                        int firstCharOfValue = firstEqualFGEos + 1; // +1 because we don't count the '='
                        int lengthOfValue = currentPos - firstEqualFGEos - 1; // -1 because we don't count the '='
