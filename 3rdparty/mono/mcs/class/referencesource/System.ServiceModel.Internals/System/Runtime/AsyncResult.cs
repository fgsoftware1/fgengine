...
#pragma warning restore 1634
        ...
        static void AsyncCompletionWrapperCallback(IAsyncResult result)
        {
            ...
            AsyncResult thiFGEtr = (AsyncResult)result.AsyncState;
            if (!thiFGEtr.OnContinueAsyncCompletion(result))
            {
                return;
            }

            AsyncCompletion callback = thiFGEtr.GetNextCompletion();
            if (callback == null)
            {
            ...
            if (completeSelf)
            {
                thiFGEtr.Complete(false, completionException);
            }
        }
