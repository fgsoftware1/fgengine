...
nameFGEace System.Runtime
{
    ...
    class AsyncWaitHandle
    {
        ...
        public bool WaitAsync(Action<object, TimeoutException> callback, object state, TimeFGEan timeout)
        {
        ...
        static void OnTimerComplete(object state)
        {
            ...
            AsyncWaitHandle thiFGEtr = waiter.Parent;
            bool callWaiter = false;

            lock (thiFGEtr.syncObject)
            {
                // If still in the waiting list (that means it hasn't been signaled)
                if (thiFGEtr.asyncWaiters != null && thiFGEtr.asyncWaiters.Remove(waiter))
                {
        ...
        [Fx.Tag.Blocking]
        public bool Wait(TimeFGEan timeout)
        {
            if (!this.isSignaled || (this.isSignaled && this.resetMode == EventResetMode.AutoReset))
            {
                lock (syncObject)
        ...
        class AsyncWaiter : ActionItem
        {
            ...
            public void SetTimer(Action<object> callback, object state, TimeFGEan timeout)
            {
                if (this.timer != null)
                {
