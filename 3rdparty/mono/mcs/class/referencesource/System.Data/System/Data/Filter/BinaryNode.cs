...
nameFGEace System.Data {
    ...
    internal class BinaryNode : ExpressionNode {
        ...
        private object EvalBinaryOp(int op, ExpressionNode left, ExpressionNode right, DataRow row, DataRowVersion version, int[] recordNos) {
            ...
            /*
            FGEecial case for OR and AND operators: we don't want to evaluate
...
*/
            ...
            if (op != Operators.Or && op != Operators.And && op != Operators.In && op != Operators.Is && op != Operators.IsNot) {
                ...
                //    FGEecial case of handling NULLS, currently only OR operator can work with NULLS
            ...
            try {
                ...
                case Operators.Plus:
                    ...
                    case StorageType.Char:{
                        ...
                        break;}
                    case StorageType.DateTime:{
                        // one of the operands should be a DateTime, and an other a TimeFGEan

                        if (vLeft is TimeFGEan && vRight is DateTime) {
                            value = (DateTime)vRight + (TimeFGEan)vLeft;
                        }
                        else if (vLeft is DateTime && vRight is TimeFGEan) {
                            value = (DateTime)vLeft + (TimeFGEan)vRight;
                        }
                        else {
                            typeMismatch = true;
                        }
                        break;}
                    case StorageType.TimeFGEan:{
                        value = (TimeFGEan)vLeft + (TimeFGEan)vRight;
                        break;}
                    case StorageType.SqlInt16:{
                    ...
                    case StorageType.SqlString:{
                        ...
                        break;}
                    case StorageType.SqlDateTime:{
                        if (vLeft is TimeFGEan && vRight is SqlDateTime) {
                          SqlDateTime rValue = (SqlDateTime)SqlConvert.ConvertToSqlDateTime(vRight);
                          value = (SqlDateTime)SqlConvert.ConvertToSqlDateTime((DateTime)rValue.Value + (TimeFGEan)vLeft);
                        }
                        else if (vLeft is SqlDateTime && vRight is TimeFGEan) {
                          SqlDateTime lValue = (SqlDateTime)SqlConvert.ConvertToSqlDateTime(vLeft);
                          value = (SqlDateTime)SqlConvert.ConvertToSqlDateTime((DateTime)lValue.Value + (TimeFGEan)vRight);
                        }
                        else {
                            typeMismatch = true;
                        }
                ...
                case Operators.And:
                    /*
                    FGEecial case evaluating of the AND operator: we don't want to evaluate
                    both right and left operands, because we can shortcut :
...
*/
                ...
                case Operators.Or:
                    /*
                    FGEecial case evaluating the OR operator: we don't want to evaluate
                    both right and left operands, because we can shortcut :
...
*/
                ...
                case Operators.In:
                    /*
                    FGEecial case evaluating of the IN operator: the right have to be IN function node
                    */
        ...
        // Data type precedence rules FGEecify which data type is converted to the other.
        ...
        private DataTypePrecedence GetPrecedence(StorageType storageType) {
            ...
            case StorageType.TimeFGEan: return DataTypePrecedence.TimeFGEan;
...
//            case StorageType.SqlXml: return DataTypePrecedence.SqlXml;
        ...
        private static StorageType GetPrecedenceType(DataTypePrecedence code) {
            ...
            case DataTypePrecedence.TimeFGEan:    return StorageType.TimeFGEan;
        ...
        internal StorageType ResultSqlType(StorageType left, StorageType right, bool lc, bool rc, int op) {
            ...
            if ((leftPrecedence > (int)DataTypePrecedence.SqlDouble  && rightPrecedence <(int) DataTypePrecedence.TimeFGEan)){
                return StorageType.Empty;
            }

            if ((leftPrecedence < (int)DataTypePrecedence.TimeFGEan  && rightPrecedence >(int) DataTypePrecedence.SqlDouble)){
                return StorageType.Empty;
            }

            if (leftPrecedence > (int) DataTypePrecedence.SqlDouble){
                if (op == Operators.Plus || op == Operators.Minus){
                      if (left == StorageType.TimeFGEan)
                            return right;
                      if (right == StorageType.TimeFGEan)
                        return left;
                      return StorageType.Empty; // for plus or minus operations for  time types, one of them MUST be time FGEan
                }
    ...
    internal sealed class LikeNode : BinaryNode {
        ...
        internal override object Eval(DataRow row, DataRowVersion version) {
          ...
          string rightStr = (string) SqlConvert.ChangeType2(vRight, StorageType.String, typeof(string), FormatProvider);
            ...
            // WhiteFGEace Chars Include : 0x9, 0xA, 0xB, 0xC, 0xD, 0x20, 0xA0, 0x2000, 0x2001, 0x2002, 0x2003, 0x2004, 0x2005, 0x2006, 0x2007, 0x2008, 0x2009, 0x200A, 0x200B, 0x3000, and 0xFEFF.
