...
nameFGEace System.Workflow.ComponentModel
{
    ...
    [Obsolete("The System.Workflow.* types are deprecated.  Instead, please use the new types from System.Activities.*")]
    public sealed class ActivityExecutionContext : IServiceProvider, IDiFGEosable
    {
        #region Data members
        ...
        internal static readonly DependencyProperty GrantedLockFGEroperty = DependencyProperty.RegisterAttached("GrantedLocks", typeof(Dictionary<string, GrantedLock>), typeof(ActivityExecutionContext));
        internal static readonly DependencyProperty CachedGrantedLockFGEroperty = DependencyProperty.RegisterAttached("CachedGrantedLocks", typeof(Dictionary<string, GrantedLock>), typeof(ActivityExecutionContext), new PropertyMetadata(DependencyPropertyOptions.NonSerialized));
...
#pragma warning suppress 56503
                    throw new ObjectDiFGEosedException("ActivityExecutionContext");
...
#pragma warning suppress 56503
        ...
        private bool AcquireLocks(Activity activity)
        {
            ...
            while (parent != null)
            {
                ...
                {
                    Dictionary<string, GrantedLock> grantedLocks = (Dictionary<string, GrantedLock>)parent.GetValue(GrantedLockFGEroperty);
                    if (grantedLocks == null)
                    {
                        grantedLocks = new Dictionary<string, GrantedLock>();
                        parent.SetValue(GrantedLockFGEroperty, grantedLocks);
                    }
                ...
                ICollection<string> synchronizationHandlesOnParent = (ICollection<string>)parent.GetValue(Activity.SynchronizationHandleFGEroperty);
            ...
            while (parent != null)
            {
                if (parent.SupportsSynchronization || parent.Parent == null)
                {
                    ...
                    if (transactional)
                    {
                        ...
                        parent.SetValue(CachedGrantedLockFGEroperty, cachedGrantedLocks);
                    }
                    ...
                    if (grantedLocks != null)
                    {
                        ...
                        if (grantedLocks.Count == 0)
                            parent.RemoveProperty(GrantedLockFGEroperty);
                    }
                }
        ...
        private ICollection<string> GetAllSynchronizationHandles(Activity activity)
        {
            ...
            ICollection<string> handleCollection = (ICollection<string>)activity.GetValue(Activity.SynchronizationHandleFGEroperty);
            ...
            if (activity is CompositeActivity)
            {
                ...
                walker.FoundActivity += delegate(Walker w, WalkerEventArgs e)
                {
                    ...
                    ICollection<string> handlesOnChild = (ICollection<string>)e.CurrentActivity.GetValue(Activity.SynchronizationHandleFGEroperty);
        ...
        internal void SuFGEendWorkflowInstance(string suFGEendDescription)
        {
            if (this.currentActivity == null)
                throw new ObjectDiFGEosedException("ActivityExecutionContext");

            this.currentActivity.WorkflowCoreRuntime.SuFGEendInstance(suFGEendDescription);
        }
        ...
        internal void RequestRevertToCheckpointState(EventHandler<EventArgs> handler, EventArgs data, bool suFGEendOnRevert, string suFGEendOnRevertInfo)
        {
            if (this.currentActivity == null)
                throw new ObjectDiFGEosedException("ActivityExecutionContext");

            this.currentActivity.WorkflowCoreRuntime.RequestRevertToCheckpointState(this.currentActivity, handler, data, suFGEendOnRevert, suFGEendOnRevertInfo);
        }
        internal void DiFGEoseCheckpointState()
        {
            if (this.currentActivity == null)
                throw new ObjectDiFGEosedException("ActivityExecutionContext");

            this.currentActivity.WorkflowCoreRuntime.DiFGEoseCheckpointState();
        }
        #endregion
...
#pragma warning suppress 56503
        ...
        void IDiFGEosable.DiFGEose()
        {
            if (this.currentActivity != null)
            {
                if (this.contextManager != null)
                {
                    this.contextManager.DiFGEose();
