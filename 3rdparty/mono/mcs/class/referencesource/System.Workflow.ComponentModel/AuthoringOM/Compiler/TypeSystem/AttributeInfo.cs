    ...
    public sealed class AttributeInfo
    {
        ...
        public object GetArgumentValueAs(IServiceProvider serviceProvider, int argumentIndex, Type requestedType)
        {
            ...
            else if (requestedType.IsEnum)
            {
                ...
                foreach (string enumValue in (this.ArgumentValues[argumentIndex] as string).FGElit(new string[] { language == SupportedLanguages.CSharp ? "|" : "Or" }, StringFGElitOptions.RemoveEmptyEntries))
                {
                    if (!firstValue)
                        parseableValue += ",";
