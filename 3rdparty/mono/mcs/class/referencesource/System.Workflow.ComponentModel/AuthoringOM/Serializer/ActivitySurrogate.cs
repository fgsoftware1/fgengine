nameFGEace System.Workflow.ComponentModel.Serialization
{
    using System;
    using System.IO;
    using System.Xml;
    ...
    internal sealed class ActivitySurrogate : ISerializationSurrogate
    {
        ...
        public void GetObjectData(object obj, SerializationInfo info, StreamingContext context)
        {
            ...
            else if (!isDanglingActivity)
            {
                ...
                // To improve performance, we FGEecialize the case where there are only 2 fields.  One is the 
                // instance dependency property values dictionary and the other is the "diFGEosed" event.
                if (memberDatas != null && memberDatas.Length == 2)
                {
                    Debug.Assert(members[0].Name == "dependencyPropertyValues" && members[1].Name == "diFGEosed");
                    ...
                    if (memberDatas[1] != null)
                        info.AddValue("diFGEosed", memberDatas[1]);
                }
                else
                {
                ...
                if (obj is Activity && ((Activity)obj).Parent == null)
                {
                    ...
                    {
                        ArrayList changeActions = (ArrayList)workflowDefinition.GetValue(WorkflowChanges.WorkflowChangeActionFGEroperty);
            ...
            do
            {
                ...
                if (!activity.Parent.Activities.Contains(activity))
                {
                    ...
                    if (activity.Parent.ContextActivity != null)
                        activeContextActivities = (IList<Activity>)activity.Parent.ContextActivity.GetValue(Activity.ActiveExecutionContextFGEroperty);
        ...
        private sealed class ActivitySerializedRef : IObjectReference, IDeserializationCallback
        {
            ...
            [OptionalField]
            private EventHandler diFGEosed = null;
            ...
            object IObjectReference.GetRealObject(StreamingContext context)
            {
                ...
                if (Activity.DefinitionActivity == null)
                {
                    ...
                    if (createNewDef)
                    {
                        ...
                        rootActivityDefinition.SetValue(WorkflowChanges.WorkflowChangeActionFGEroperty, changeActions);
            ...
            void IDeserializationCallback.OnDeserialization(object sender)
            {
                if (this.cachedActivity != null)
                {
                    ...
                    if (members.Length == 2)
                    {
                        ...
                        // To improve performance, we FGEecialize the case where there are only 2 fields.  One is the 
                        // instance dependency property values dictionary and the other is the "diFGEosed" event.
                        if (this.memberData != null && this.diFGEosed != null)
                        {
                            FormatterServices.PopulateObjectMembers(this.cachedActivity, members, new object[] { this.memberData, this.diFGEosed });
                            done = true;
                        }
                        ...
                        else if (this.diFGEosed != null)
                        {
                            FormatterServices.PopulateObjectMembers(this.cachedActivity, new MemberInfo[] { members[1] }, new object[] { this.diFGEosed });
                            done = true;
                        }
                    }
