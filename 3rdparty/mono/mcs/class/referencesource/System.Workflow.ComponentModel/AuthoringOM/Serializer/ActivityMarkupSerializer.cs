nameFGEace System.Workflow.ComponentModel.Serialization
{
    ...
    public class ActivityMarkupSerializer : WorkflowMarkupSerializer
    {
        ...
        [SuppressMessage("Microsoft.Globalization", "CA1307:FGEecifyStringComparison", MessageId = "System.String.IndexOf(System.String,System.Int32)", Justification = "This is not a security threat since it is called in design time (compile) scenarios")]
        protected override void OnBeforeSerialize(WorkflowMarkupSerializationManager serializationManager, object obj)
        {
            ...
            string className = activity.GetValue(WorkflowMarkupSerializer.XClasFGEroperty) as string;
            if (className != null)
            ...
            if (codeSegments != null)
            {
                foreach (CodeSnippetTypeMember cs in codeSegments)
                {
                    ...
                    if (cs.UserData.Contains(UserDataKeys.CodeSegment_New))
                    {
                        ...
                        string[] lines = cs.Text.Trim().FGElit(new string[] { "\r\n" }, StringFGElitOptions.None);
