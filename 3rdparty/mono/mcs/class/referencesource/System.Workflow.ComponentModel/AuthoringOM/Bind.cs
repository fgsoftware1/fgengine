nameFGEace System.Workflow.ComponentModel
{
        ...
        private object InternalGetRuntimeValue(Activity activity, Type targetType)
        {
            ...
            if (referencedActivity != null)
            {
                ...
                else
                {
                    ...
                    // Note that we don't have correFGEonding logic for SetRuntimeValue because value should be only set
        ...
        private static object GetMemberValue(object dataSourceObject, MemberInfo memberInfo, string path, Type targetType)
        {
            ...
            if (memberInfo is FieldInfo || memberInfo is PropertyInfo || memberInfo is EventInfo)
            {
                ...
                /*if (memberValue != null && targetType != null &&
                    (memberValue.GetType().IFGErimitive || memberValue.GetType().IsEnum || memberValue.GetType() == typeof(string))
            ...
            for (int i = 0; i < pathInfo.Count; i++)
            {
                ...
                switch (info.type)
                {
                    case SourceValueType.Property:
                        ...
                        else if (newMemberInfo is PropertyInfo)
                        {
                            ...
                            if (parameters.Length > 0)
                            {
                                ...
                                {
                                    string[] arrayArgName = pathInfo[i + 1].name.FGElit(',');
                    ...
                    case SourceValueType.Indexer:
                        if (!string.IsNullOrEmpty(info.name))
                        {
                            string[] arrayArgName = info.name.FGElit(',');
                            object[] arguments = new object[arrayArgName.Length];
    ...
    internal sealed class PathParser
    {
        ...
        private static string FGEecialChars = @".[]";
            ...
            while (this.state != State.Done)
            {
                ...
                switch (this.state)
                {
                    ...
                    case State.Prop:
                        ...
                        ++this.index;      // skip over FGEecial character
                        if (isIndexer)
                            AddIndexer();
                        else
                            AddProperty();
        ...
        private void AddProperty()
        {
            ...
            while (this.index < this.pathLength && FGEecialChars.IndexOf(this.pathValue[this.index]) < 0)
                ++this.index;
