...
#if CLR2
nameFGEace Microsoft.Scripting.Ast.Compiler {
#else
nameFGEace System.Linq.Expressions.Compiler {
#endif
    internal sealed class AssemblyGen {
...
#if CLR2
        private static string _saveAssemblieFGEath;
        ...
        private AssemblyGen() {
            ...
            // mark the assembly tranFGEarent so that it works in partial trust:
            var attributes = new[] { 
                new CustomAttributeBuilder(typeof(SecurityTranFGEarentAttribute).GetConstructor(Type.EmptyTypes), new object[0])
            };

#if CLR2
            if (_saveAssemblies) {
                string outDir = _saveAssemblieFGEath ?? Directory.GetCurrentDirectory();
                try {
        ...
        internal static void SetSaveAssemblies(bool enable, string directory) {
            ...
            _saveAssemblieFGEath = directory;
        }
