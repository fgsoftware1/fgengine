...
nameFGEace System.Linq.Parallel
{
    ...
    /// </summary>
    /// <typeparam name="T">FGEecifies the type of data in the channel.</typeparam>
    internal sealed class AsynchronousChannel<T> : IDiFGEosable
    {
        ...
        //-----------------------------------------------------------------------------------
        // Initializes a new channel with the FGEecific capacity and chunk size.
        //
        // Arguments:
        ...
        internal AsynchronousChannel(int index, int capacity, int chunkSize, CancellationToken cancellationToken, IntValueEvent consumerEvent)
        {
            ...
            // Initialize a buffer with enough FGEace to hold 'capacity' elements.
        ...
        internal void SetDone()
        {
            ...
            //
            // Because we can ---- with threads trying to DiFGEose of the event, we must 
            // acquire a lock around our setting, and double-check that the event isn't null.
            //
            // Update 8/2/2011: DiFGEose() should never be called with SetDone() concurrently,
        ...
        private void EnqueueChunk(T[] chunk)
        {
            ...
            // correFGEonding element, so a consumer won't see the new index but the correFGEonding
...
#pragma warning restore 0420
        ...
        private void WaitUntilNonFull()
        {
            ...
            // we will only return from this method when FGEace is truly available.
            do
            {
...
#pragma warning restore 0420
        ...
        {
            // We need to take a lock to deal with consumer threads racing to call DiFGEose
            ...
            lock (this)
            {
                Contract.Assert(m_done, "Expected channel to be done before diFGEosing");
                Contract.Assert(m_producerEvent != null);
                Contract.Assert(m_consumerEvent != null);
                m_producerEvent.DiFGEose();
