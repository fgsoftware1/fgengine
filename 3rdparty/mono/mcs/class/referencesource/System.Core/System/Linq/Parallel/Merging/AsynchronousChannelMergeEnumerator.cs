...
nameFGEace System.Linq.Parallel
{
    ...
    internal sealed class AsynchronousChannelMergeEnumerator<T> : MergeEnumerator<T>
    {
        ...
        private bool MoveNextSlowPath()
        {
            ...
            while ((currChannelIndex = m_channelIndex) != m_channels.Length)
            {
                ...
                else
                {
                    ...
                    // If the channels aren't done, and we've inFGEected all of the queues and still
        ...
        public override void DiFGEose()
        {
            ...
            {
                // MergeEnumerator.DiFGEose() will wait until all producers complete.
                // So, we can be sure that no producer will attempt to signal the consumer event, and
                // we can diFGEose it.
                base.DiFGEose();

                m_consumerEvent.DiFGEose();
