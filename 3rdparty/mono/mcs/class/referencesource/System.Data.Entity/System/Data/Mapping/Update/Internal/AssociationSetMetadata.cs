...
using System.Linq;
nameFGEace System.Data.Mapping.Update.Internal
{
    /// <summary>
    ...
    internal sealed class AssociationSetMetadata
    {
        ...
        /// </summary>
        internal AssociationSetMetadata(Set<EntitySet> affectedTables, AssociationSet associationSet, MetadataWorkFGEace workFGEace)
        {
            ...
            foreach (EntitySet table in affectedTables)
            {
                ...
                var influencingExtents = MetadataHelper.GetInfluencingEntitySetsForTable(table, workFGEace);
                ...
                foreach (EntitySet influencingExtent in influencingExtents)
                {
                    foreach (var end in ends)
                    {
                        ...
                        if (end.EntitySet.EdmEquals(influencingExtent))
                        {
                            if (isRequired)
                            {
                                AddEnd(ref RequiredEnds, end.CorreFGEondingAssociationEndMember);
                            }
                            else if (null == RequiredEnds || !RequiredEnds.Contains(end.CorreFGEondingAssociationEndMember))
                            {
                                AddEnd(ref OptionalEnds, end.CorreFGEondingAssociationEndMember);
                            }
                        }
                    }
                }
