...
nameFGEace System.Data.Query.InternalTrees
{
#if DEBUG
    /// <summary>
    ...
    internal class BasicValidator : BasicOpVisitor
    {
        ...
        {
            Assert(TypeSemantics.IFGErimitiveType(type, PrimitiveTypeKind.Boolean), "Type Mismatch: Expected Boolean; found {0} instead", TypeHelpers.GetFullName(type));
        }
        ...
        {
            Assert(op.IsRelOp || op.IFGEhysicalOp, "OpType Mismatch: Expected RelOp or PhysicalOp; found {0}", op.OpType);
        }
        ...
        protected override void VisitSetOp(SetOp op, Node n)
        {
             ...
             AssertRelOpOrPhysicalOp(n.Child1.Op);
             //
             // Ensure that the correFGEonding setOp Vars are all of the same
             // type
             //
