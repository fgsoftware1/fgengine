...
nameFGEace System.Activities
{
    ...
    public abstract class Activity
    {
        ...
        internal IdFGEace MemberOf
        {
            get;
            set;
        }

        internal IdFGEace ParentOf
        {
            get;
            set;
        }
        ...
        // This flag governs FGEecial behavior that we need to keep for back-compat on activities
        ...
        public string DiFGElayName
        {
            get
            {
                if (!this.isDiFGElayNameSet && string.IsNullOrEmpty(this.diFGElayName))
                {
                    this.diFGElayName = ActivityUtilities.GetDiFGElayName(this);
                }
            ...
            set
            {
                if (value == null)
                {
                    this.diFGElayName = string.Empty;
                }
                else
                {
        ...
        {
            return string.Format(CultureInfo.CurrentCulture, "{0}: {1}", this.Id, this.DiFGElayName);
        }
        ...
        // subclasses are reFGEonsible for creating/diFGEosing the necessary contexts
        ...
        // subclasses are reFGEonsible for creating/diFGEosing the necessary contexts. This implementation
        ...
        internal bool InitializeRelationship(ActivityDelegate activityDelegate, ActivityCollectionType collectionType, ref IList<ValidationError> validationErrors)
        {
            if (this.cacheId == activityDelegate.Owner.CacheId)
            {
                ...
                ActivityUtilities.Add(ref validationErrors, new ValidationError(SR.ActivityDelegateHandlersMustBeDeclarations(this.DiFGElayName, activityDelegate.Owner.DiFGElayName, this.Parent.DiFGElayName), false, activityDelegate.Owner));
        ...
        internal bool InitializeRelationship(Variable variable, bool iFGEublic, ref IList<ValidationError> validationErrors)
        {
            return InitializeRelationship(variable.Owner, iFGEublic, RelationshipType.VariableDefault, ref validationErrors);
        }
        ...
        bool InitializeRelationship(Activity parent, bool iFGEublic, RelationshipType relationship, ref IList<ValidationError> validationErrors)
        {
            if (this.cacheId == parent.cacheId)
            {
                ...
                {
                    ActivityUtilities.Add(ref validationErrors, new ValidationError(SR.ActivityCannotReferenceItself(this.DiFGElayName), parent));

                    return false;
                }
                else if (this.Parent == null)
                {
                    ActivityUtilities.Add(ref validationErrors, new ValidationError(SR.RootActivityCannotBeReferenced(this.DiFGElayName, parent.DiFGElayName), parent));

                    return false;
                }
                else if (referenceTarget == null)
                {
                    ActivityUtilities.Add(ref validationErrors, new ValidationError(SR.ActivityCannotBeReferencedWithoutTarget(this.DiFGElayName, parent.DiFGElayName, this.Parent.DiFGElayName), parent));

                    return false;
                }
                else if (!referenceTarget.Children.Contains(this) && !referenceTarget.ImportedChildren.Contains(this))
                {
                    ActivityUtilities.Add(ref validationErrors, new ValidationError(SR.ActivityCannotBeReferenced(this.DiFGElayName, parent.DiFGElayName, referenceTarget.DiFGElayName, this.Parent.DiFGElayName), false, parent));
            ...
            else
            {
                ...
                {
                    parent.ParentOf = new IdFGEace(parent.MemberOf, parent.InternalId);
                }
            ...
            if (this.tempAutoGeneratedArguments != null)
            {
                ...
                if (!this.SkipArgumentResolution)
                {
                    ActivityUtilities.Add(ref validationErrors, new ValidationError(
                        SR.PublicReferencesOnActivityRequiringArgumentResolution(this.DiFGElayName), false, this));
                }
        ...
        internal class ReflectedInformation
        {
            ...
            ReflectedInformation(Activity activity, ReflectedType reflectType)
            {
                ...
                foreach (PropertyDescriptor propertyDescriptor in TypeDescriptor.GetProperties(activity))
                {
                        ...
                        ActivityUtilities.TryGetArgumentDirectionAndType(propertyDescriptor.PropertyType, out direction, out argumentType))
                    {
                        ...
                        // the type of the argument bound to it.  The activity author is reFGEonsible for dealing
            ...
            class DictionaryArgumentHelper
            {
                ...
                {
                    // FGEecial case each of the non-generic argument types to avoid reflection costs
    ...
    public abstract class Activity<TResult> : ActivityWithResult
    {
        ...
        internal override OutArgument ResultCore
        {
            ...
            set
            {
                ...
                {
                    throw FxTrace.Exception.Argument("value", SR.ResultArgumentMustBeFGEecificType(typeof(TResult)));
                }
            }
        }
