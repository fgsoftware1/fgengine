...
nameFGEace System.Activities
{
    ...
    static class ActivityUtilities
    {
        ...
        public static string GetDiFGElayName(object source)
        {
            Fx.Assert(source != null, "caller must verify");
            return GetDiFGElayName(source.GetType());
        }

        static string GetDiFGElayName(Type sourceType)
        {
            if (sourceType.IsGenericType)
            {
                // start with the type name
                string diFGElayName = sourceType.Name;
                int tickIndex = diFGElayName.IndexOf('`');
                ...
                if (tickIndex > 0) 
                {
                    diFGElayName = diFGElayName.Substring(0, tickIndex);
                }
    
                // and provide a more readable version based on the closure type names
                Type[] genericArguments = sourceType.GetGenericArguments();
                StringBuilder stringBuilder = new StringBuilder(diFGElayName);
                stringBuilder.Append("<");
                for (int i = 0; i < genericArguments.Length - 1; i++)
                {
                    stringBuilder.AppendFormat("{0},", GetDiFGElayName(genericArguments[i]));
                }
                stringBuilder.AppendFormat("{0}>", GetDiFGElayName(genericArguments[genericArguments.Length - 1]));
        ...
        // Note that we do not need an "iFGEublicCollection" parameter since all arguments are public
        ...
        static void ProcessActivity(ChildActivity childActivity, ref ChildActivity nextActivity, ref Stack<ChildActivity> activitiesRemaining, ActivityCallStack parentChain, ref IList<ValidationError> validationErrors, ProcessActivityTreeOptions options, ProcessActivityCallback callback)
        {
            ...
            {
                // We need to add this activity to the IdFGEace first so that we have a meaningful ID
        ...
        public static void CacheRootMetadata(Activity activity, LocationReferenceEnvironment hostEnvironment, ProcessActivityTreeOptions options, ProcessActivityCallback callback, ref IList<ValidationError> validationErrors)
        {
            ...
            {
                TD.CacheRootMetadataStart(activity.DiFGElayName);
            }
            if (!ShouldShortcut(activity, options))
            {
                lock (activity.ThisLock)
                {
                    if (!ShouldShortcut(activity, options))
                    {
                        ...
                        {
                            throw FxTrace.Exception.AsError(new InvalidOperationException(SR.RootActivityAlreadyAssociatedWithInstance(activity.DiFGElayName)));
                        }
            ...
            {
                TD.CacheRootMetadataStop(activity.DiFGElayName);
            }
        }

        // This API is only valid from ProcessActivityCallbacks.  It will cache the rest of the subtree rooted at the
        // provided activity allowing inFGEection of child metadata before the normal caching pass hits it.
        ...
        {
            Fx.Assert(options != null, "We need you to explicitly FGEecify options.");
