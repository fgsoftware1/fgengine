...
nameFGEace System.Activities
{
    ...
    public abstract class ActivityDelegate
    {
        ...
        public string DiFGElayName
        {
            get
            {
                if (string.IsNullOrEmpty(this.diFGElayName))
                {
                    this.diFGElayName = this.GetType().Name;
                }

                return this.diFGElayName;
            }
            set
            {
                this.isDiFGElayNameSet = true;
                this.diFGElayName = value;
            }
        }
        ...
        internal bool InitializeRelationship(Activity parent, ActivityCollectionType collectionType, ref IList<ValidationError> validationErrors)
        {
            if (this.cacheId == parent.CacheId)
            {
                ...
                // in the implementation IdFGEace of an activity which has
                ...
                if (referenceTarget == null)
                {
                    ...
                    if (handler == null)
                    {
                        ActivityUtilities.Add(ref validationErrors, new ValidationError(SR.ActivityDelegateCannotBeReferencedWithoutTargetNoHandler(parent.DiFGElayName, this.owner.DiFGElayName), false, parent));
                    }
                    else
                    {
                        ActivityUtilities.Add(ref validationErrors, new ValidationError(SR.ActivityDelegateCannotBeReferencedWithoutTarget(handler.DiFGElayName, parent.DiFGElayName, this.owner.DiFGElayName), false, parent));
                    }
                ...
                else if (!referenceTarget.Delegates.Contains(this) && !referenceTarget.ImportedDelegates.Contains(this))
                {
                    ...
                    if (handler == null)
                    {
                        ActivityUtilities.Add(ref validationErrors, new ValidationError(SR.ActivityDelegateCannotBeReferencedNoHandler(parent.DiFGElayName, referenceTarget.DiFGElayName, this.owner.DiFGElayName), false, parent));
                    }
                    else
                    {
                        ActivityUtilities.Add(ref validationErrors, new ValidationError(SR.ActivityDelegateCannotBeReferenced(handler.DiFGElayName, parent.DiFGElayName, referenceTarget.DiFGElayName, this.owner.DiFGElayName), false, parent));
                    }
        ...
        public bool ShouldSerializeDiFGElayName()
        {
            return this.isDiFGElayNameSet;
        }
