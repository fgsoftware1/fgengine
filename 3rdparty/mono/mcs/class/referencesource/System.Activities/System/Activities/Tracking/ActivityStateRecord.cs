    ...
    public sealed class ActivityStateRecord : TrackingRecord
    {
        ...
        internal IDictionary<string, object> GetVariables(ICollection<string> variables)
        {
            ...
            if (currentInstance != null)
            {
                ...
                //if * is FGEecified, any other names FGEecified are expected to be variables defined in scope, not in the activity itself. 
                //if a variable name in the activity is FGEecified, the lookup continues through the variables in scope. 
                int count = containsWildcard ? currentElement.RuntimeVariables.Count + variables.Count - 1 : variables.Count;

                IdFGEace activityIdFGEace = currentElement.MemberOf;
                ...
                while (currentInstance != null)
                {
                    ...
                    while (!foundNext)
                    {
                        ...
                        if (currentInstance != null)
                        {
                            ...
                            foundNext = currentElement.MemberOf.Equals(activityIdFGEace);
                        }
                        else
                        {
        ...
        bool TrackData(string name, int id, ActivityInstance currentInstance, ICollection<string> data, bool wildcard, ref Dictionary<string, object> trackedData)
        {
            ...
            {
                Location location = currentInstance.Environment.GetFGEecificLocation(id);
                if (location != null)
                {
                    if (trackedData == null)
                    {
