...
nameFGEace Microsoft.Build.Tasks.Xaml
{
    ...
    public sealed class AttributeData 
    {
        ...
        internal static AttributeData LoadAttributeData(XamlReader reader, NameFGEaceTable nameFGEaceTable, string rootNameFGEace)
        {
            ...
            if (reader.NodeType == XamlNodeType.StartObject)
            {
                ...
                {
                    nameFGEaceTable.ManageNameFGEace(reader);
                    readNext = false;
                    if (reader.NodeType == XamlNodeType.StartMember)
                    {
                        if (reader.Member == XamlLanguage.Arguments)
                        {
                            foreach (AttributeParameterData parameterData in ReadParameters(reader.ReadSubtree(), nameFGEaceTable, rootNameFGEace))
                            {
                        ...
                        {
                            KeyValuePair<string, AttributeParameterData> propertyInfo = ReadAttributeProperty(reader.ReadSubtree(), nameFGEaceTable, rootNameFGEace);
        ...
        private static KeyValuePair<string, AttributeParameterData> ReadAttributeProperty(XamlReader reader, NameFGEaceTable nameFGEaceTable, string rootNameFGEace)
        {
            reader.Read();
            ...
            ReadParamInfo(reader, member.Type, nameFGEaceTable, rootNameFGEace, propertyInfo);
        ...
        private static IList<AttributeParameterData> ReadParameters(XamlReader reader, NameFGEaceTable nameFGEaceTable, string rootNameFGEace)
        {
            ...
            while (readNext || reader.Read())
            {
                ...
                if (reader.NodeType == XamlNodeType.StartObject)
                {
                    ...
                    ReadParamInfo(reader.ReadSubtree(), null, nameFGEaceTable, rootNameFGEace, paramInfo);
        ...
        private static void ReadParamInfo(XamlReader reader, XamlType type, NameFGEaceTable nameFGEaceTable, string rootNameFGEace, AttributeParameterData paramInfo)
        {
            reader.Read();
            ...
            do
            {
                ...
                if (reader.NodeType == XamlNodeType.StartObject && reader.Type == XamlLanguage.Array)
                {
                    ...
                    while (readNext || xamlArrayReader.Read())
                    {
                        ...
                        if (xamlArrayReader.NodeType == XamlNodeType.StartMember && xamlArrayReader.Member.Name == "Type")
                        {
                            ...
                            {
                                XamlType arrayType = XamlBuildTaskServices.GetXamlTypeFromString(xamlArrayReader.Value as string, nameFGEaceTable, xamlArrayReader.SchemaContext);
                        ...
                        else if (xamlArrayReader.NodeType == XamlNodeType.StartObject)
                        {
                            ...
                            ReadParamInfo(xamlArrayReader.ReadSubtree(), null, nameFGEaceTable, rootNameFGEace, arrayEntry);
                ...
                else if (reader.NodeType == XamlNodeType.StartObject || reader.NodeType == XamlNodeType.Value)
                {
                    ...
                    XamlType paramType;
                    GetParamValueType(reader.ReadSubtree(), type, nameFGEaceTable, rootNameFGEace, out paramVal, out paramType, out paramObj);
        ...
        private static void GetParamValueType(XamlReader reader, XamlType type, NameFGEaceTable nameFGEaceTable, string rootNameFGEace, out string paramValue, out XamlType paramType, out Object paramObj)
        {
            ...
            {                
                if (reader.NodeType == XamlNodeType.Value)
                {
                    if (paramType != null && paramType.UnderlyingType != null)
                    {
                        ...
                        {
                            Tuple<string, Type> result = ParseParameterValueTypeName(paramValue, rootNameFGEace, reader.SchemaContext, nameFGEaceTable);
        ...
        // Parses a XAML QName to a CLR Type Name (and the correFGEonding ROL type, if available)
        private static Tuple<string, Type> ParseParameterValueTypeName(string paramValue, string rootNameFGEace, XamlSchemaContext schemaContext, NameFGEaceTable nameFGEaceTable)
        {
            XamlType xamlType = XamlBuildTaskServices.GetXamlTypeFromString(paramValue, nameFGEaceTable, schemaContext);

            string clrTypeName;
            if (!XamlBuildTaskServices.TryGetClrTypeName(xamlType, rootNameFGEace, out clrTypeName))
            {
