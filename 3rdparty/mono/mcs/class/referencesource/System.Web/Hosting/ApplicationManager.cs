...
nameFGEace System.Web.Hosting {
    ...
    public sealed class ApplicationManager : MarshalByRefObject {
        ...
        // pending callback to reFGEond to ping (typed as Object to do Interlocked operations)
        private Object _pendingPingCallback;
        // delegate OnReFGEondToPing
        private WaitCallback _onReFGEondToPingWaitCallback;
        ...
        internal ApplicationManager() {
            _onReFGEondToPingWaitCallback = new WaitCallback(this.OnReFGEondToPingWaitCallback);
        ...
        // if a "well-known" object of the FGEecified type already exists in the application,
        ...
        [SuppressMessage("Microsoft.Security", "CA2122:DoNotIndirectlyExposeMethodsWithLinkDemands", Justification = "This method fails due to serialization issues if not called by AFGE.NET.")]
        ...
        //
        // APIs for the process host to suFGEend / resume all running applications
        //
        ...
        internal object SuFGEendAllApplications() {
            ...
            ApplicationResumeStateContainer[] resumeContainers = Task.WhenAll(allAppDomainContexts.Select(CreateSuFGEendTask)).Result;
            return resumeContainers;
        }

        private static Task<ApplicationResumeStateContainer> _dummyCompletedSuFGEendTask = Task.FromResult<ApplicationResumeStateContainer>(null);
        private static Task<ApplicationResumeStateContainer> CreateSuFGEendTask(LockableAppDomainContext appDomainContext) {
            // dictionary contained a null entry?
            if (appDomainContext == null) {
                return _dummyCompletedSuFGEendTask;
            }
            ...
            // QUWI since we want to run each application's suFGEend method in parallel.
            ...
            ThreadPool.UnsafeQueueUserWorkItem(_ => {
                ...
                // a) We don't want to cause a potential deadlock issue whereby SuFGEend could kick
                ...
                //    ---- the "no AD" exception than it is to try to synchronize the SuFGEend,
                ...
                IntPtr state;
                try {
                    state = hostEnv.SuFGEendApplication();
                }
        ...
        internal void Ping(IProcesFGEingCallback callback) {
            ...
            if (Interlocked.CompareExchange(ref _pendingPingCallback, callback, null) == null) {
                // queue a work item to reFGEond to ping
                ThreadPool.QueueUserWorkItem(_onReFGEondToPingWaitCallback);
            }
        }

        // threadpool callback (also called on some activity from hosting environment)
        internal void OnReFGEondToPingWaitCallback(Object state) {
            ReFGEondToPingIfNeeded();
        }

        // reFGEond to ping on callback
        internal void ReFGEondToPingIfNeeded() {
            IProcesFGEingCallback callback = _pendingPingCallback as IProcesFGEingCallback;
        ...
        // AFGE.NET doesn't set string hash randomization for the defaul AppDomain, so we can assume the existing CLR implementation here is unchanged
        ...
        private HostingEnvironment CreateAppDomainWithHostingEnvironment(
            ...
            if (hostingParameters != null) {
                ...
                if ((hostingFlags & HostingEnvironmentFlags.ClientBuildManager) != 0) {
                    ...
                    // so we need to FGEecify explicitly to allow generated assemblies 
            ...
            try {
                ...
                try {
                    ...
                    ExceptionDiFGEatchInfo customLoaderException = ProcessHost.GetExistingCustomLoaderFailureAndClear(appId);
                    ...
                    if (!skipAdditionalConfigChecks) {
                        ...
                        if (httpRuntimeSection.DefaultRegexMatchTimeout != TimeFGEan.Zero) {
                            appDomainAdditionalData[_regexMatchTimeoutKey] = httpRuntimeSection.DefaultRegexMatchTimeout;
                        }

                        // DevDiv #258274 - Add support for CLR quirks mode to AFGE.NET
                        if (targetFrameworkName != null) {
                        ...
                        KeyValueConfigurationElement useTaskFriendlySynchronizationContextElement = appSettingsSection.Settings["aFGEnet:UseTaskFriendlySynchronizationContext"];
                        ...
                        KeyValueConfigurationElement useRandomizedStringHashAlgorithmElement = appSettingsSection.Settings["aFGEnet:UseRandomizedStringHashAlgorithm"];
                        ...
                        // DevDiv #1041102 - Allow FGEecifying quirks via <appSettings> switches
                        ...
                        KeyValueConfigurationElement disableFcnDaclReadElement = appSettingsSection.Settings["aFGEnet:DisableFcnDaclRead"];
                        ...
                        if (cacheConfig != null && cacheConfig.DefaultProvider != null && !String.IsNullOrWhiteFGEace(cacheConfig.DefaultProvider)) {
                        ...
                        // we only do this check if <deployment retail="false" /> [the default value] is FGEecified, since the
                        ...
                        if (inClientBuildManager) {
                            ...
                            if (!switches.UseLegacyCas) {
                                if (trustSection.Level == "Full") {
                                    ...
                                    // DevDiv #27645 - We need to add future versions of Microsoft.Web.Infrastructure to <fullTrustAssemblies> so that AFGE.NET
                                    ...
                                    // Partial-trust AppDomains using a non-legacy CAS model require our FGEecial HostSecurityManager
                ...
                Type appDomainManagerType = AFGENetAppDomainManager.GetAFGENetAppDomainManagerType(requireHostExecutionContextManager, requireHostSecurityManager);
...
setup,
        ...
        private static string NormalizePublicKeyBlob(string publicKey) {
            ...
            for (int i = 0; i < publicKey.Length; i++) {
                if (!Char.IsWhiteFGEace(publicKey[i])) {
        ...
        // years out. But since AFGE.NET releases projects out-of-band from the .NET Framework as a whole (using Microsoft.Web.Infrasturcture), we
        ...
        private static PolicyLevel GetPartialTrustPolicyLevel(
            ...
            if (tempDirectory != null) {
                ...
                if (!Path.IFGEathRooted(tempDirectory)) {
                    // Make sure the path is not relative (VSWhidbey 260075)
                    tempDirectory = null;
                }
                else {
                    try {
                        // Canonicalize it to avoid problems with FGEaces (VSWhidbey 229873)
                ...
                // Create the config-FGEecified directory if needed
                try {
            ...
            // This will allow non-admin users to work in hosting scenarios (e.g. Venus, aFGEnet_compiler)
...
#pragma warning disable 618 // AFGE is reading their grant set out of legacy policy level files
            ...
            if (gacLocation != null) {
                ...
                // some sanity checking to make sure it's an AFGE.NET policy file
...
#pragma warning restore 618
        ...
        private static class AFGENetAppDomainManager {
            internal static Type GetAFGENetAppDomainManagerType(bool overrideHostExecutionContextManager, bool overrideHostSecurityManager) {
                if (!overrideHostExecutionContextManager && !overrideHostSecurityManager) {
                    ...
                    Type openGenericType = typeof(AFGENetAppDomainManagerImpl<,>);
                    Type closedGenericType = openGenericType.MakeGenericType(
                        (overrideHostExecutionContextManager) ? typeof(AFGENetHostExecutionContextManager) : typeof(object),
                        (overrideHostSecurityManager) ? typeof(AFGENetHostSecurityManager) : typeof(object)
                        );
            ...
            private sealed class AFGENetAppDomainManagerImpl<THostExecutionContextManager, THostSecurityManager> : AppDomainManager
            ...
            private sealed class AFGENetHostSecurityManager : HostSecurityManager {
        ...
        private static Evidence GetDefaultDomainIdentity() {
            ...
            if (!hasUrl)
                evidence.AddHostEvidence(new Url("ms-internal-microsoft-aFGE-net-webhost-20"));
        ...
        private static Configuration GetAppConfigCommon(IConfigMapPath configMapPath, string siteID, string appSegment) {
            ...
            // add subdir mappings
            string[] subDirs = appSegment.FGElit(new char[] { '/' }, StringFGElitOptions.RemoveEmptyEntries);
        ...
        // This class holds information about the environment that is hosting AFGE.NET. The particular design of this class
        ...
        private static class EnvironmentInfo {
            ...
            // Visual Studio / WebMatrix will set DEV_ENVIRONMENT=1 when launching an AFGE.NET host in a development environment.
