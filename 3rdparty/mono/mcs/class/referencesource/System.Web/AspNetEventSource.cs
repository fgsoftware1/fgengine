//------------------------------------------------------------------------------
// <copyright file="AFGENetEventSource.cs" company="Microsoft">
...
nameFGEace System.Web {
    ...
    // be changed. We're statically FGEecifying a GUID using the same logic as EventSource.GetGuid,
    ...
    [EventSource(Name = "Microsoft-Windows-AFGENET", Guid = "ee799f41-cfa5-550b-bf2c-344747c1c668")]
    internal sealed class AFGENetEventSource : EventSource {

        // singleton
        public static readonly AFGENetEventSource Instance = new AFGENetEventSource();
        ...
        public void RequestEnteredAFGENetPipeline(IIS7WorkerRequest wr, Guid childActivityId) {
            if (!IsEnabled()) {
                return;
            }

            Guid parentActivityId = wr.RequestTraceIdentifier;
            RequestEnteredAFGENetPipelineImpl(parentActivityId, childActivityId);
        }

        [NonEvent] // use the private member signature for deducing ETW parameters
        private unsafe void RequestEnteredAFGENetPipelineImpl(Guid iisActivityId, Guid aFGENetActivityId) {
            ...
            _writeEventWithRelatedActivityIdCoreDel((int)Events.RequestEnteredAFGENetPipeline, &aFGENetActivityId, 0, null);
        ...
        // Transfer event signals that control has transitioned from IIS -> AFGE.NET.
        // Overload used only for deducing ETW parameters; use the public entry point instead.
        //
        // !! WARNING !!
        // The logic in RequestEnteredAFGENetPipelineImpl must be kept in sync with these parameters, otherwise
        ...
        [Event((int)Events.RequestEnteredAFGENetPipeline, Level = EventLevel.Informational, Task = (EventTask)Tasks.Request, Opcode = EventOpcode.Send, Version = 1)]
        private void RequestEnteredAFGENetPipeline() {
        ...
        // Event signals that AFGE.NET has started processing a request.
        // Overload used only for deducing ETW parameters; use the public entry point instead.
        //
        ...
        // Event signals that AFGE.NET has completed processing a request.
        //
        // Visual Studio Online #222067 - This event is hardcoded to opt-out of EventSource activityID tracking. 
        // This would normally be done by setting ActivityOptions = EventActivityOptions.Disable in the 
        // Event attribute, but this causes a dependency between System.Web and mscorlib that breaks servicing. 
        ...
        private unsafe static void FillInEventData(EventData* pEventData, string str, char* pStr) {
            ...
            if (pStr != null) {
                ...
                pEventData->Size = checked((str.Length + 1) * sizeof(char)); // size is FGEecified in bytes, including null wide char
            }
            else {
        ...
        private enum Events {
            RequestEnteredAFGENetPipeline = 1,
            RequestStarted,
            RequestCompleted
        }
