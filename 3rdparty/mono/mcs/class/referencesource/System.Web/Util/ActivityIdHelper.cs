...
nameFGEace System.Web.Util {
    ...
    internal sealed class ActivityIdHelper {
        ...
        // DiFGEoses of the thread's existing activity ID, then sets the new activity ID on this thread.
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void SetCurrentThreadActivityId(Guid activityId) {
            _setAndDestroyDel(activityId);
        }

        // SuFGEends (but does not diFGEose of) the thread's existing activity ID, then sets a new activity ID on this thread.
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
