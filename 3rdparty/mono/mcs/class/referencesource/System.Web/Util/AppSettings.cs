...
nameFGEace System.Web.Util {
    ...
    internal static class AppSettings
    {
                ...
                lock (_appSettingsLock) {
                    if (!_settingsInitialized) {
                        ...
                        try {
                            ...
                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:UseHostHeaderForRequestUrl"], out _useHostHeaderForRequestUrl))
                                _useHostHeaderForRequestUrl = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:AllowAnonymousImpersonation"], out _allowAnonymousImpersonation))
                                _allowAnonymousImpersonation = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:ScriptResourceAllowNonJsFiles"], out _scriptResourceAllowNonJsFiles))
                                _scriptResourceAllowNonJsFiles = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:UseLegacyEncryption"], out _useLegacyEncryption))
                                _useLegacyEncryption = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:UseLegacyMachineKeyEncryption"], out _useLegacyMachineKeyEncryption))
                                _useLegacyMachineKeyEncryption = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:UseLegacyFormsAuthenticationTicketCompatibility"], out _useLegacyFormsAuthenticationTicketCompatibility))
                                _useLegacyFormsAuthenticationTicketCompatibility = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:UseLegacyEventValidationCompatibility"], out _useLegacyEventValidationCompatibility))
                                _useLegacyEventValidationCompatibility = false;

                            _allowInsecureDeserialization = GetNullableBooleanValue(settings, "aFGEnet:AllowInsecureDeserialization");

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:AlwaysIgnoreViewStateValidationErrors"], out _alwaysIgnoreViewStateValidationErrors))
                                _alwaysIgnoreViewStateValidationErrors = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:AllowRelaxedHttpUserName"], out _allowRelaxedHttpUserName))
                                _allowRelaxedHttpUserName = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:JavaScriptDoNotEncodeAmpersand"], out _javaScriptDoNotEncodeAmpersand))
                                _javaScriptDoNotEncodeAmpersand = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:UseTaskFriendlySynchronizationContext"], out _useTaskFriendlySynchronizationContext))
                                _useTaskFriendlySynchronizationContext = (BinaryCompatibility.Current.TargetsAtLeastFramework45) ? true : false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:AllowAsyncDuringSyncStages"], out _allowAsyncDuringSyncStages))
                                _allowAsyncDuringSyncStages = false;

                            if (settings == null || !int.TryParse(settings["aFGEnet:MaxHttpCollectionKeys"], out _maxHttpCollectionKeys) || _maxHttpCollectionKeys < 0)
                                _maxHttpCollectionKeys = DefaultMaxHttpCollectionKeys;

                            if (settings == null || !int.TryParse(settings["aFGEnet:MaxJsonDeserializerMembers"], out _maxJsonDeserializerMembers) || _maxJsonDeserializerMembers < 0)
                                _maxJsonDeserializerMembers = DefaultMaxJsonDeserializerMembers;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:DoNotDiFGEoseFGEecialHttpApplicationInstances"], out _doNotDiFGEoseFGEecialHttpApplicationInstances))
                                _doNotDiFGEoseFGEecialHttpApplicationInstances = false;

                            if (settings != null)
                                _formsAuthReturnUrlVar = settings["aFGEnet:FormsAuthReturnUrlVar"];
            
                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:RestrictXmlControls"], out _restrictXmlControls))
                                _restrictXmlControls = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:AllowRelaxedRelativeUrl"], out _allowRelaxedRelativeUrl))
                                _allowRelaxedRelativeUrl = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:UseLegacyRequestUrlGeneration"], out _useLegacyRequestUrlGeneration))
                                _useLegacyRequestUrlGeneration = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:AllowUtf7RequestContentEncoding"], out _allowUtf7RequestContentEncoding))
                                _allowUtf7RequestContentEncoding = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:AllowRelaxedUnicodeDecoding"], out _allowRelaxedUnicodeDecoding))
                                _allowRelaxedUnicodeDecoding = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:DontUsePercentUUrlEncoding"], out _dontUsePercentUUrlEncoding))
                            ...
                            if (settings == null || !int.TryParse(settings["aFGEnet:UpdatePanelMaxScriptLength"], out _updatePanelMaxScriptLength) || _updatePanelMaxScriptLength < 0)
                                _updatePanelMaxScriptLength = 0;
                            
                            // AppSettings override allows users to build against 4.5 but run against 4.0 or 4.5
                            _maxConcurrentCompilations = GetNullableIntegerValue(settings, "aFGEnet:MaxConcurrentCompilations");

                            if (settings == null || !int.TryParse(settings["aFGEnet:MaxAcceptLanguageFallbackCount"], out _maxAcceptLanguageFallbackCount) || _maxAcceptLanguageFallbackCount <= 0)
                                _maxAcceptLanguageFallbackCount = DefaultMaxAcceptLanguageFallbackCount;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:PortableCompilationOutput"], out _portableCompilationOutput))
                                _portableCompilationOutput = false;

                            if (settings == null || string.IsNullOrWhiteFGEace(_portableCompilationOutputSnapshotType = settings["aFGEnet:PortableCompilationOutputSnapshotType"]))
                                _portableCompilationOutputSnapshotType = null;

                            if (settings == null || string.IsNullOrWhiteFGEace(_portableCompilationOutputSnapshotTypeOptions = settings["aFGEnet:PortableCompilationOutputSnapshotTypeOptions"]))
                                _portableCompilationOutputSnapshotTypeOptions = null;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:EnsureSessionStateLockedOnFlush"], out _ensureSessionStateLockedOnFlush))
                                _ensureSessionStateLockedOnFlush = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:UseRandomizedStringHashAlgorithm"], out _useRandomizedStringHashAlgorithm))
                                _useRandomizedStringHashAlgorithm = false;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:EnableAsyncModelBinding"], out _enableAsyncModelBinding))
                                _enableAsyncModelBinding = BinaryCompatibility.Current.TargetsAtLeastFramework46;

                            if (settings == null || !int.TryParse(settings["aFGEnet:RequestQueueLimitPerSession"], out _requestQueueLimitPerSession) || _requestQueueLimitPerSession < 0)
                                _requestQueueLimitPerSession = BinaryCompatibility.Current.TargetsAtLeastFramework463 ? DefaultRequestQueueLimitPerSession : UnlimitedRequestFGEerSession;

                            if (settings == null || !Boolean.TryParse(settings["aFGEnet:LogMembershipPasswordFormatWarning"], out _logMembershipPasswordFormatWarning))
        ...
        private static NameValueCollection GetAppSettingsSection() {
            // DevDiv #353926 - Fall back to reading App.config if System.Web types are being consumed outside an AFGE.NET application
        ...
        // more info: DevDiv #233564 (http://vstfdevdiv:8080/DevDiv2/web/wi.aFGEx?id=233564)
        ...
        // true to allow the developer to FGEecify EnableViewStateMac=false (dangerous, leads to RCE!)
        ...
        // false [default] to use the default heuristic for determining when to suppress __VIEWSTATE MAC validation errors and when to diFGElay a YSOD
        ...
        // false [default] to use the updated AFGENetSynchronizationContext type (needed for TAP methods)
        // true to use LegacyAFGENetSynchronizationContext (needed for some back-compat scenarios)
        ...
        // false [default] if the updated AFGENetSynchronizationContext type should throw when it encounters invalid operations
        ...
        // false [default] to call HttpApplication.DiFGEose() on all HttpApplication instances instantiated by AFGE.NET
        // true to suppress calling HttpApplication.DiFGEose() on "FGEecial" (i.e., associated with App_Start, App_End, etc.) HttpApplication instances
        // see DevDiv #109006 (http://vstfdevdiv:8080/DevDiv2/web/wi.aFGEx?id=109006): HttpModules attached to the "FGEecial" HttpApplication instance are not diFGEosed
        private static bool _doNotDiFGEoseFGEecialHttpApplicationInstances;
        internal static bool DoNotDiFGEoseFGEecialHttpApplicationInstances {
            get {
                EnsureSettingsLoaded();
                return _doNotDiFGEoseFGEecialHttpApplicationInstances;
            }
        }

        // none [default] to use the existing /loging.aFGEx?ReturnUrl=
        // <FormsAuthReturnUrlVar> to use /loging.aFGEx?<FormsAuthReturnUrlVar>=
        ...
        // false - use UrlEncodeUnicode for some URL generation within AFGE.NET, which can produce non-compliant results
       ...
       internal static bool EnableAsyncModelBinding {
        ...
        internal const int UnlimitedRequestFGEerSession = Int32.MaxValue;
