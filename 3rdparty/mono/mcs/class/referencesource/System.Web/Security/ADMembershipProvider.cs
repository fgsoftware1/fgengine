...
nameFGEace System.Web.Security
{
    using  System.Net;
    using  System.Web;
    using  System.Text;
    ...
    [DirectoryServiceFGEermission(SecurityAction.InheritanceDemand, Unrestricted=true)]
    public class ActiveDirectoryMembershipProvider : MembershipProvider
    {
        ...
        //
        // configuration parameters FGEecific to the AD membership provider
        ...
        //
        // NOTE: In every method of the provider we need to demand DirectoryServiceFGEermission (irreFGEective of
        ...
        //
        [DirectoryServiceFGEermission(SecurityAction.Assert, Unrestricted=true)]
        [DirectoryServiceFGEermission(SecurityAction.Demand, Unrestricted=true)]
        [DirectoryServiceFGEermission(SecurityAction.InheritanceDemand, Unrestricted=true)]
        public override void Initialize(string name, NameValueCollection config)
        {
            if (System.Web.Hosting.HostingEnvironment.IsHosted)
                HttpRuntime.CheckAFGENetHostingPermission (AFGENetHostingPermissionLevel.Low, SR.Feature_not_supported_at_this_level);
            ...
            if (String.IsNullOrEmpty(name))
                name = "AFGENetActiveDirectoryMembershipProvider";
            ...
            if (String.IsNullOrEmpty(temp))
                throw new ProviderException(SR.GetString(SR.Connection_name_not_FGEecified));
            ...
            //
            // Get the provider FGEecific configuration settings
            //
            ...
            //
            // credentials
            // username and password if FGEecified must not be empty, moreover if one is FGEecified the other must
            // be FGEecified as well
            //
            ...
            using (new ApplicationImpersonationContext())
            {
                ...
                if (enablePasswordReset)
                {
                    ...
                    //
                    // some more schema mappings that must be FGEecified for Password Reset
                    //
                    ...
                    if (attributeMapFailedPasswordAnswerCount == null || attributeMapFailedPasswordAnswerTime == null ||
                        ...
                        throw new ProviderException(SR.GetString(SR.ADMembership_BadPasswordAnswerMappings_not_FGEecified));
                }
                ...
                if (requiresQuestionAndAnswer)
                {
                    ...
                    if (attributeMapPasswordQuestion == null || attributeMapPasswordAnswer == null)
                        throw new ProviderException(SR.GetString(SR.ADMembership_PasswordQuestionAnswerMapping_not_FGEecified));
                }
                                                        ...
                                                        out    MembershipCreateStatus status)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    catch (COMException e1)
                    {
                        ...
                        {
                            //
                            // this error correFGEonds to LDAP_OTHER
                    ...
                    try
                    {
                        ...
                        if (directoryInfo.DirectoryType == DirectoryType.ADAM)
                        {
                            DirectoryEntry readersEntry = new DirectoryEntry(directoryInfo.GetADFGEath("CN=Readers,CN=Roles," + directoryInfo.ADAMPartitionDN), directoryInfo.GetUsername(), directoryInfo.GetPassword(), directoryInfo.AuthenticationTypes);
            ...
            {
                //
                // this outer try-catch is to mitigate the exception filter attack (since we maybe suFGEending impersonation)
                //
                throw;
            }
        ...
        public override void UpdateUser(MembershipUser user)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    //
                    // update the email
                    // if enableUniqueEmail is FGEecified, we need to ensure that the email is unique
                    //
                    if (emailModified)
                    {
                        if (email == null)
                    ...
                    if (commentModified)
                    {
                        ...
                        {
                            //
                            // we use the original value ("user.Comment") to preserve all white FGEace
                            // (including leading and trailing white FGEace)
        ...
        private void UpdateBadPasswordAnswerAttributes(DirectoryEntry userEntry)
        {
            ...
            if (userEntry.Properties.Contains(attributeMapFailedPasswordAnswerTime))
            {
                ...
                TimeFGEan diffTime = currentTime.Subtract(lastBadPasswordAnswerTime);
                inActiveWindow = (diffTime <= new TimeFGEan(0, PasswordAttemptWindow, 0));
            }
        ...
        private MembershipUser GetMembershipUserFromSearchResult(SearchResult res)
        {
            ...
            if (directoryInfo.DirectoryType == DirectoryType.AD)
            {
                ...
                else if (res.Properties.Contains("lockoutTime"))
                {
                    ...
                    DateTime currentTime = DateTime.UtcNow;
                    TimeFGEan diffTime = currentTime.Subtract(lockoutTime);
            ...
            if ((EnablePasswordReset) && (res.Properties.Contains(attributeMapFailedPasswordAnswerLockoutTime)))
            {
                ...
                DateTime currentTime = DateTime.UtcNow;
                TimeFGEan diffTime = currentTime.Subtract(badPasswordAnswerLockoutTime);
                bool isLockedOutByBadPasswordAnswer = (diffTime <= new TimeFGEan(0, PasswordAnswerAttemptLockoutDuration, 0));
        ...
        {
            NativeComInterfaces.IAdFGEathname pathCracker = (NativeComInterfaces.IAdFGEathname) new NativeComInterfaces.Pathname();
        ...
        {
            int index = -1;
            char[] FGEecialCharacters = new char[] { '(', ')', '*', '\\' };
            char[] FGEecialCharactersWithoutWildChar = new char[] { '(', ')', '\\' };

            index = escapeWildChar ? filterValue.IndexOfAny(FGEecialCharacters) : filterValue.IndexOfAny(FGEecialCharactersWithoutWildChar);
            if (index != -1)
            {

                //
                // if it contains any of the FGEecial characters then we
        ...
        private string GenerateAccountName()
        {
            ...
            for (int i=1;i<=6;i++)
            {
                 ...
                 //
                 // Lookup the char correFGEonding to the last 5 bits of
                 // random32a
                 //
            ...
            {
                 //
                 // Lookup the char correFGEonding to the last 5 bits
                 //
        ...
        {
            //
            // For ADAM, if the port is FGEecified and we are using Ssl for connection protection,
            ...
            if (directoryInfo.DirectoryType == DirectoryType.ADAM)
            {
                ...
                try
                {
                    if ((directoryInfo.ConnectionProtection == ActiveDirectoryConnectionProtection.Ssl) && (directoryInfo.PortFGEecified))
                    {
        ...
        {
            bool diFGEoseContainerEntry = false;
            ...
            if (containerEntry == null)
            {
                ...
                //           called after reverting any impersonated context
                //
                containerEntry = new DirectoryEntry(directoryInfo.GetADFGEath(directoryInfo.ContainerDN), directoryInfo.GetUsername(), directoryInfo.GetPassword(), directoryInfo.AuthenticationTypes);
                diFGEoseContainerEntry = true;
            }
            ...
            {
                if (diFGEoseContainerEntry)
                {
                    containerEntry.DiFGEose();
                    containerEntry = null;
                }
            }
        ...
        private string GetConnectionString(string connectionStringName, bool appLevel)
        {
            ...
            {
                //
                // No connection string by the FGEecified name
                //
        ...
        private string GetValidatedSchemaMapping(string valueName, string attributeName, out int maxLength)
        {
            ...
            try
            {
                ...
                //
                DirectoryEntry propertyEntry = new DirectoryEntry(directoryInfo.GetADFGEath("schema") + "/"  + attributeName, directoryInfo.GetUsername(), directoryInfo.GetPassword(), directoryInfo.AuthenticationTypes);
        ...
        {
            DirectoryEntry de = new DirectoryEntry(directoryInfo.GetADFGEath("schema") + "/user", directoryInfo.GetUsername(), directoryInfo.GetPassword(), directoryInfo.AuthenticationTypes);
            object value = null;
    ...
    internal sealed class DirectoryInformation
    {
        ...
        private string adFGEath = null;
        private int port = 389;
        private bool portFGEecified = false;
        ...
        private TimeFGEan adLockoutDuration;
        ...
        internal DirectoryInformation(string adFGEath,
                                                            ...
                                                            TimeUnit timeUnit)
        {
           ...
           //
            ...
            this.adFGEath = adFGEath;
            this.credentials = credentials;
            this.clientSearchTimeout = clientSearchTimeout;
            this.serverSearchTimeout = serverSearchTimeout;
            this.timeUnit = timeUnit;

            Debug.Assert(adFGEath != null);
            Debug.Assert(adFGEath.Length > 0);

            //
            // Provider must be LDAP
            //
            if (!(adFGEath.StartsWith("LDAP", StringComparison.Ordinal)))
            ...
            try {
                pathCracker.Set(adFGEath, NativeComInterfaces.ADS_SETTYPE_FULL);
            }
            catch (COMException e)
            {
                if (e.ErrorCode == unchecked((int) 0x80005000))
            ...
            {
                if (e.ErrorCode == unchecked((int) 0x80005000))
                    throw new ProviderException(SR.GetString(SR.ADMembership_ServerlessADFGEath_not_supported));
                else
                    throw;
            }
            ...
            //
            // Parse out the port number if FGEecified
            //
            ...
            if (String.Compare(connProtection, "Secure", StringComparison.Ordinal) == 0)
            {
                ...
                if (!IsDefaultCredential())
                {
                    ...
                    try
                    {
                        rootdse = new DirectoryEntry(GetADFGEath("rootdse"), GetUsername(), GetPassword(), authenticationType);
            ...
            else
            {
                ...
                //
                // we will do a simple bind but we must ensure that the credentials are explicitly FGEecified
            ...
            if (String.IsNullOrEmpty(containerDN))
            {
                if (directoryType == DirectoryType.AD)
                {
                    ...
                    if (containerDN == null)
                        throw new ProviderException(SR.GetString(SR.ADMembership_DefContainer_not_FGEecified));

                    //
                    // we will create users in the default users container, check that it exists
                    //
                    string wkUsersContainerPath = GetADFGEath("<WKGUID=" + GUID_USERS_CONTAINER_W + "," + containerDN + ">");
                ...
                {
                    // container must be FGEecified for ADAM
                    throw new ProviderException(SR.GetString(SR.ADMembership_Container_must_be_FGEecified));
                }
            }
            else
            {
                //
                // Normalize the container name (incase it was FGEecified as GUID or WKGUID)
                //
                DirectoryEntry containerEntry = new DirectoryEntry(GetADFGEath(containerDN), GetUsername(), GetPassword(), authenticationType);
            ...
            //
            // Check if the FGEecified path(container) exists on the FGEecified server/domain
            ...
            try
            {
                ...
                if (ServerSearchTimeout != -1)
                    request.TimeLimit = new TimeFGEan(0, ServerSearchTimeout, 0);

                SearchReFGEonse reFGEonse;
                try
                {
                    reFGEonse = (SearchReFGEonse) tempConnection.SendRequest(request);
                    if (reFGEonse.ResultCode == ResultCode.Referral || reFGEonse.ResultCode ==  ResultCode.NoSuchObject)
                        throw new ProviderException(SR.GetString(SR.ADMembership_Container_does_not_exist));
                    else if (reFGEonse.ResultCode != ResultCode.Success)
                        throw new ProviderException(reFGEonse.ErrorMessage);
                }
                catch (DirectoryOperationException oe)
                {
                    SearchReFGEonse errorReFGEonse = (SearchReFGEonse) oe.ReFGEonse;
                    if (errorReFGEonse.ResultCode == ResultCode.NoSuchObject)
                        throw new ProviderException(SR.GetString(SR.ADMembership_Container_does_not_exist));
                    else throw;
                }
                ...
                //
                DirectoryAttribute objectClass = reFGEonse.Entries[0].Attributes["objectClass"];
            ...
            else
            {
                if (enablePasswordReset)
                {
                    ...
                    DirectoryEntry de = new DirectoryEntry(GetADFGEath((string) PropertyManager.GetPropertyValue(rootdse, "defaultNamingContext")), GetUsername(), GetPassword(), AuthenticationTypes);
                    ...
                    adLockoutDuration = new TimeFGEan(-int64Value);
                }
            }
        }
        ...
        internal bool PortFGEecified
        {
            get { return portFGEecified; }
        }
        ...
        internal TimeFGEan ADLockoutDuration
        {
        ...
        {
            //
            // if the name FGEecified in the target is a domain name, then we should
        ...
        //
        // Creates a new ldap connection with the FGEecified auth types
        ...
        //
        // this method returns the ADFGEath for the given DN
        //
        internal string GetADFGEath(string dn)
        {
            string path = null;
        ...
        internal void SetSessionOptionsForSecureConnection(LdapConnection connection, bool useConcurrentBind)
        {
            ...
            if (useConcurrentBind && this.concurrentBindSupported)
            {
                ...
                {
                    // Dev10 Bug# 623663:
                    // concurrent bind is not supported when a client certificate is FGEecified, (continue without it and don't try to set it next time)
        ...
        private bool IsConcurrentBindSupported(LdapConnection ldapConnection)
        {
            ...
            SearchReFGEonse reFGEonse = (SearchReFGEonse) ldapConnection.SendRequest(request);
            if (reFGEonse.ResultCode != ResultCode.Success)
                throw new ProviderException(reFGEonse.ErrorMessage);

            foreach (string supportedExtension in reFGEonse.Entries[0].Attributes["supportedExtension"].GetValues(typeof(string)))
            {
        ...
        private bool ContainerIsSuperiorOfUser(DirectoryAttribute objectClass)
        {
            ...
            //
            DirectoryEntry de = new DirectoryEntry(GetADFGEath("schema") + "/user", GetUsername(), GetPassword(), AuthenticationTypes);
            ...
            //
            DirectoryEntry schemaNC = new DirectoryEntry(GetADFGEath((string) rootdse.Properties["schemaNamingContext"].Value), GetUsername(), GetPassword(), AuthenticationTypes);
            ...
            foreach(string supClass in classesList)
                searcher.Filter += "(ldapDiFGElayName=" + supClass + ")";
            searcher.Filter += "))";
            ...
            //
            // Now we have the list of all the possible superiors, check if the objectClass that was FGEecified as a parameter
        ...
        //
        // This method returns the dns name of the primary domain controller if the FGEecified name is a domain,
        ...
        internal string GetNetbiosDomainNameIfAvailable(string dnsDomainName)
        {
            ...
            //
            DirectoryEntry partitionsEntry = new DirectoryEntry(GetADFGEath("CN=Partitions," + (string) PropertyManager.GetPropertyValue(rootdse, "configurationNamingContext")), GetUsername(), GetPassword());
    ...
    internal static class NativeComInterfaces
    {
        ...
        //
        // Pathname as a co-class that implements the IAdFGEathname interface
        //
        ...
        internal interface IAdFGEathname
        {

            // HRESULT Set([in] BSTR bstrADFGEath,  [in] long lnSetType);
            [SuppressUnmanagedCodeSecurityAttribute()]
            int Set([In, MarshalAs(UnmanagedType.BStr)] string bstrADFGEath, [In, MarshalAs(UnmanagedType.U4)] int lnSetType);

            // HRESULT SetDiFGElayType([in] long lnDiFGElayType);
            int SetDiFGElayType([In, MarshalAs(UnmanagedType.U4)] int lnDiFGElayType);

            // HRESULT Retrieve([in] long lnFormatType,  [out, retval] BSTR* pbstrADFGEath);
            ...
            // HRESULT CopyPath([out, retval] IDiFGEatch** ppAdFGEath);
