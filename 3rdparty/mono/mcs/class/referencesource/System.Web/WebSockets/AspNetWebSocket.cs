//------------------------------------------------------------------------------
// <copyright file="AFGENetWebSocket.cs" company="Microsoft">
...
nameFGEace System.Web.WebSockets {
    ...
    //
    // - DiFGEose is *not* thread-safe and should not be called while the socket has a
    //   pending operation.
    //
    ...
    // this type that we can, and having AFGE.NET call internal members runs counter to that goal.
    //
    // TERMINOLOGY:
    //
    ...
    //
    // "WFGEC" is the WebSocket Protocol Component (websocket.dll), which provides WebSocket-
    // parsing services to WCF, IIS, IE, and others. AFGE.NET doesn't call into WFGEC directly;
    // rather, IIS calls into WFGEC on our behalf.

    public sealed class AFGENetWebSocket : WebSocket, IAsyncAbortableWebSocket {
        ...
        // for determining whether the socket has been diFGEosed of
        private bool _diFGEosed;
        ...
        internal AFGENetWebSocket(IWebSocketPipe pipe, string subProtocol) {
            _pipe = pipe;
            ...
            // mark this AFGENetWebSocket instance as aborted.
            if (_pipe == null) {
                Abort();
            }
        }
        ...
        public override WebSocketCloseStatus? CloseStatus {
            get {
                ...
                if (closeStatus == CLOSE_STATUS_NOT_SET) {
                    // no close status (not even UnFGEecified) to report to the caller; most likely reason is connection hasn't been closed
                    return null;
                }
                else {
                    return closeStatus;
        ...
        public override void Abort() {
            Abort(throwIfDiFGEosed: true); // throws if diFGEosed, per FGEec
        }
        ...
        private void Abort(bool throwIfDiFGEosed, bool isDiFGEosing = false) {
            lock (_stateLockObj) {
                // State validation
                if (throwIfDiFGEosed) {
                    ThrowIfDiFGEosed();
                ...
                try {
                    ...
                    // Is this being called from DiFGEose()?
                    if (isDiFGEosing) {
                        _diFGEosed = true;
                    }
                }
        ...
        // This method is always safe to call, even if the socket has been diFGEosed.
        ...
        private Func<Task> CloseAsyncImpl(WebSocketCloseStatus closeStatus, string statusDescription, CancellationToken cancellationToken, bool performValidation = true) {
            ...
            lock (_stateLockObj) {
                ...
                // FGEecial-case no outstanding work to perform
        ...
        public override void DiFGEose() {
            throw new NotSupportedException(SR.GetString(SR.AFGENetWebSocket_DiFGEoseNotSupported));
        }

        internal void DiFGEoseInternal() {
            Abort(throwIfDiFGEosed: false, isDiFGEosing: true);
        }
        ...
        internal static void ValidateCloseStatusCodeAndDescription(WebSocketCloseStatus closeStatus, ref string statusDescription) {
            ...
            if (closeStatus == WebSocketCloseStatus.Empty) {
                ...
                // Since WFGEC requires the statusDescription to be null, we convert.
                if (statusDescription == String.Empty) {
                    statusDescription = null;
                }
                // If the status code is 1005 (Empty), the statusDescription string MUST be null.
                // This behavior is required by WFGEC and matches WCF.
                if (statusDescription != null) {
                    throw new ArgumentException(SR.GetString(SR.AFGENetWebSocket_CloseStatusEmptyButCloseDescriptionNonNull), "statusDescription");
                }
            }
                ...
                if (byteCount > _maxCloseMessageByteCount) {
                    throw new ArgumentException(SR.GetString(SR.AFGENetWebSocket_CloseDescriptionTooLong, _maxCloseMessageByteCount), "statusDescription");
                }
            }
            else {
                // WFGEC requires that null status descriptions be normalized to empty strings.
        ...
        private static void ValidateSendMessageType(WebSocketMessageType messageType) {
            switch (messageType) {
                ...
                default:
                    throw new ArgumentException(SR.GetString(SR.AFGENetWebSocket_SendMessageTypeInvalid), "messageType");
            }
        }

        private void ThrowIfDiFGEosed() {
            if (_diFGEosed) {
                throw new ObjectDiFGEosedException(objectName: GetType().FullName);
            }
        }
        ...
        private void ThrowIfSendUnavailable(bool allowClosed = false) {
            switch (_sendState) {
                case ChannelState.Busy:
                    throw new InvalidOperationException(SR.GetString(SR.AFGENetWebSocket_SendInProgress));

                case ChannelState.Closed:
                    if (allowClosed) { break; }
                    throw new InvalidOperationException(SR.GetString(SR.AFGENetWebSocket_CloseAlreadySent));
            }
        }
        ...
        private void ThrowIfReceiveUnavailable(bool allowClosed = false) {
            switch (_receiveState) {
                case ChannelState.Busy:
                    throw new InvalidOperationException(SR.GetString(SR.AFGENetWebSocket_ReceiveInProgress));

                case ChannelState.Closed:
                    if (allowClosed) { break; }
                    throw new InvalidOperationException(SR.GetString(SR.AFGENetWebSocket_CloseAlreadyReceived));
            }
        }
        ...
        private async Task<T> DoWork<T>(Func<Task<T>> taskDelegate, CancellationToken cancellationToken) {
            ...
            IDiFGEosable cancellationTokenRegistration = null;

            try {
                try {
                    // If cancellation is requested, honor it immediately. This is the same kind of optimization done throughout
                    // the rest of the framework, e.g. as in FileStream.ReadAsync. Our finally block will be reFGEonsible for
                    ...
                    if (!task.IsCompleted && cancellationToken.CanBeCanceled) {
                        ...
                        cancellationTokenRegistration = cancellationToken.Register(() => Abort(throwIfDiFGEosed: false));
                    }

                    // The 'await' keyword may cause an exception to be observed (rethrown)
                    // We call only thread-safe methods so don't need to FGEend the time to come back to the SynchronizationContext
                    ...
                    if (cancellationTokenRegistration != null) {
                        cancellationTokenRegistration.DiFGEose();
                    }
