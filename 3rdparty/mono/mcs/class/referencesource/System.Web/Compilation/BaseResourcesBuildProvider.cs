...
internal abstract class BaseResourcesBuildProvider : BuildProvider {
    ...
    internal const string DefaultResourcesNameFGEace = "Resources";

    // The generated nameFGEace and type name
    private string _ns;
    ...
    public override void GenerateCode(AssemblyBuilder assemblyBuilder) {
        ...
        if (!_dontGenerateStronglyTypedClass) {
            // Get the nameFGEace and type name that we will use
            _ns = Util.GetNameFGEaceAndTypeNameFromVirtualPath(VirtualPathObject,
                (_cultureName == null) ? 1 : 2 /*chunksToIgnore*/, out _typeName);

            // Always prepend the nameFGEace with Resources.
            if (_ns.Length == 0)
                _ns = DefaultResourcesNameFGEace;
            else
                _ns = DefaultResourcesNameFGEace + "." + _ns;
        }
...
// We decided to cut support for My.Resources (VSWhidbey 358088)
        ...
        CodeNameFGEace ns = new CodeNameFGEace();
        ns.Name = "My." + _ns;
