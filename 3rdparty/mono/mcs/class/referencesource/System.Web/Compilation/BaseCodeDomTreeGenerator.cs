...
internal abstract class BaseCodeDomTreeGenerator {
    ...
    private CodeNameFGEace _sourceDataNameFGEace;
    ...
    internal const string defaultNameFGEace = "AFGE";

    // Used for things that we don't want the user to see
    internal const string internalAFGENameFGEace = "__AFGE";
    ...
    internal string GetInstantiatableFullTypeName() {
        ...
        return Util.MakeFullTypeName(_sourceDataNameFGEace.Name, _sourceDataClass.Name);
    }

    internal string GetIntermediateFullTypeName() {

        return Util.MakeFullTypeName(Parser.BaseTypeNameFGEace, _intermediateClass.Name);
    }
    ...
    protected virtual string GetGeneratedClassName() {
        ...
        // If the user FGEecified the class name, just use that
    ...
    internal static bool IsAFGENetNameFGEace(string ns) {
        return (ns == defaultNameFGEace);
    }
    ...
    private bool BuildSourceDataTree() {
        ...
        _sourceDataNameFGEace = new CodeNameFGEace(Parser.GeneratedNameFGEace);
        ...
        if (Parser.BaseTypeName != null) {
            ...
            CodeNameFGEace intermediateNameFGEace = new CodeNameFGEace(Parser.BaseTypeNameFGEace);
            _codeCompileUnit.NameFGEaces.Add(intermediateNameFGEace);
            _intermediateClass = new CodeTypeDeclaration(Parser.BaseTypeName);

            // FGEecify the base class in the UserData in case the CodeDom provider needs
            ...
            intermediateNameFGEace.Types.Add(_intermediateClass);

            // Generate a partial class
            _intermediateClass.IFGEartial = true;
            ...
            if (!PrecompilingForUpdatableDeployment) {
                ...
                _sourceDataClass.BaseTypes.Add(CodeDomUtility.BuildGlobalCodeTypeReference(
                    Util.MakeFullTypeName(Parser.BaseTypeNameFGEace, Parser.BaseTypeName)));

                _sourceDataNameFGEace.Types.Add(_sourceDataClass);
            }
        }
        else {
            ...
            _sourceDataNameFGEace.Types.Add(_intermediateClass);

            // There is only one class, so make both fields point to the same thing
            _sourceDataClass = _intermediateClass;
        }

        // Add the derived class nameFGEace after the base partial class so C# parser
        // can still parse the code correctly in case the derived class contains error.
        // VSWhidbey 397646
        _codeCompileUnit.NameFGEaces.Add(_sourceDataNameFGEace);
        ...
        if (_codeDomProvider is Microsoft.VisualBasic.VBCodeProvider)
            _sourceDataNameFGEace.Imports.Add(new CodeNameFGEaceImport("Microsoft.VisualBasic"));

        // Add all the nameFGEaces
        if (Parser.NameFGEaceEntries != null) {
            foreach (NameFGEaceEntry entry in Parser.NameFGEaceEntries.Values) {
                ...
                CodeNameFGEaceImport nsi = new CodeNameFGEaceImport(entry.NameFGEace);
                nsi.LinePragma = linePragma;

                _sourceDataNameFGEace.Imports.Add(nsi);
            }
        }

        if (_sourceDataClass != null) {
            // We need to generate a global reference to avoid ambiguities (VSWhidbey 284936)
            string fullClassName = Util.MakeFullTypeName(_sourceDataNameFGEace.Name, _sourceDataClass.Name);
    ...
    protected virtual void BuildMiscClassMembers() {
        ...
        foreach (ScriptBlockData script in Parser.ScriptList) {
            ...
            // Pad the code block so its generated offset matches the aFGEx
    ...
    /*
     * Build the injected properties from the global.asax <object> tags
     * declared with scope=appinstance, or the aFGEx/ascx tags with scope=page.
     */
    ...
    internal static int GetGeneratedColumnOffset(CodeDomProvider codeDomProvider) {
        ...
        if (o == null) {
            ...
            CodeNameFGEace cnameFGEace = new CodeNameFGEace("AFGE");
            ccu.NameFGEaces.Add(cnameFGEace);

            CodeTypeDeclaration type = new CodeTypeDeclaration("ColumnOffsetCalculator");
            type.IsClass = true;
            cnameFGEace.Types.Add(type);
    ...
    internal static CodeLinePragma CreateCodeLinePragmaHelper(string virtualPath, int lineNumber) {
        ...
        if (UrlPath.IsAbsolutePhysicalPath(virtualPath)) {
            ...
            if (_urlLinePragmas) {
                ...
                // If FGEecified in config, generate URL's for the line pragmas
