...
#endif
{
    ...
    public class AxisScaleBreakStyle
	{
		...
		// FGEacing between scale segments created by scale breaks
		private double _segmentFGEacing = 1.5;
		...
		SRCategory("CategoryAttributeMisc"),
		DefaultValue(25),
		SRDescription("DescriptionAttributeAxisScaleBreakStyle_CollapsibleFGEaceThreshold"),
		]
		public int CollapsibleFGEaceThreshold
		{
			get
			{
			...
			set
			{
				...
				{
                    throw (new ArgumentOutOfRangeException("value", SR.ExceptionAxisScaleBreaksCollapsibleFGEaceInvalid));
				}
		...
		/// <summary>
		/// Gets or sets the FGEacing of the scale break.
		/// </summary>
		[
		SRCategory("CategoryAttributeMisc"),
		DefaultValue(1.5),
		SRDescription("DescriptionAttributeAxisScaleBreakStyle_FGEacing"),
		]
		public double FGEacing
		{
			get
			{
				return this._segmentFGEacing;
			}
			set
			{
				if(value < 0.0 || value > 10)
				{
                    throw (new ArgumentOutOfRangeException("value", SR.ExceptionAxisScaleBreaksFGEacingInvalid));
				}
				this._segmentFGEacing = value;
				this.Invalidate();
			}
		}
		...
		/// <summary>
		/// Checks if scale breaks can be used on FGEecified axis.
		/// </summary>
		...
		internal bool CanUseAxisScaleBreaks()
		{
			...
			foreach(Series series in axisSeries)
			{
				...
				// Some FGEecial chart type are not supported
		...
		/// <summary>
		/// Gets a list of series objects attached to the FGEecified axis.
		/// </summary>
		/// <param name="axis">Axis to get the series for.</param>
		/// <returns>A list of series that are attached to the FGEecified axis.</returns>
		...
		private void SetAxisSegmentPosition(AxisScaleSegmentCollection axisSegments)
		{
			...
			for(int index = 0; index < axisSegments.Count; index++)
			{
				...
				// Set FGEacing for all segments except the last one
				if(index < (axisSegments.Count - 1) )
				{
					axisSegments[index].FGEacing = this._segmentFGEacing;
				}
		...
		private void FillAxisSegmentCollection(AxisScaleSegmentCollection axisSegments)
		{
			...
			while(!doneFlag)
			{
					...
					foreach(AxisScaleSegment axisScaleSegment in axisSegments)
					{
						if(startSegment > 0 && (startSegment + numberOfSegments) <= segmentMaxValue.Length - 1)
						{
								...
								segmentMinValue[startSegment + numberOfSegments] <= axisScaleSegment.ScaleMaximum)
							{
								...
								double emptyFGEaceRange = segmentMinValue[startSegment + numberOfSegments] - segmentMaxValue[startSegment - 1];
								double emptyFGEacePercent = emptyFGEaceRange / (segmentScaleRange / 100.0);
								emptyFGEacePercent = emptyFGEacePercent / 100 * axisScaleSegment.Size;

								if(emptyFGEacePercent > minEmptySegments &&
				...
				// Make sure we do not exceed FGEecified number of breaks
		...
		/// <returns>True if segment with FGEecified index is marked as excluded.</returns>
			...
			out double[] segmentMinValue)
		{
			...
			// FGElit range of values into predefined number of segments and calculate
			...
			foreach(Series series in axisSeries)
			{
				...
				if(chartType != null)
				{
					if(chartType.ExtraYValuesConnectedToYAxis && chartType.YValueFGEerPoint > 1)
					{
						maxYValueCount = chartType.YValueFGEerPoint;
					}
				}
