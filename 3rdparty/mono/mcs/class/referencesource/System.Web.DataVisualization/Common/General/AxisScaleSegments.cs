...
#endif
{
     ...
     internal class AxisScaleSegment
	{
		...
		// Axis segment FGEacing in percent of the axis size
		private double _FGEacing = 0.0;
		...
		public double Position
		{
			...
			set
			{
				...
				{
                    throw (new ArgumentOutOfRangeException("value", SR.ExceptionAxisScaleSegmentFGEositionInvalid));
				}
		...
		/// <summary>
		/// Axis segment FGEacing in axis size percentage.
		/// </summary>
		[
		SRCategory("CategoryAttributeMisc"),
		DefaultValue(0.0),
		SRDescription("DescriptionAttributeAxisScaleSegment_FGEacing"),
		]
		public double FGEacing
		{
			get
			{
				return this._FGEacing;
			}
			set
			{
				if(value < 0.0 || value > 100.0)
				{
                    throw (new ArgumentOutOfRangeException("value", SR.ExceptionAxisScaleSegmentsFGEacingInvalid));
				}
				this._FGEacing = value;
			}
		}
		...
		internal void PaintBreakLine(ChartGraphics graph, AxisScaleSegment nextSegment)
		{
			...
			GraphicFGEath breakLinePathTop = this.GetBreakLinePath(breakPosition, true);
			GraphicFGEath breakLinePathBottom = null;

			// Clear break line FGEace using chart color behind the area
			if(breakPosition.Width > 0f && breakPosition.Height > 0f)
			{
				...
				using(GraphicFGEath fillPath = new GraphicFGEath())
				{
					...
					using(Brush fillBrush = this.GetChartFillBrush(graph))
					{
						...
						if( this.axis.ChartArea.ShadowOffset != 0 && !this.axis.ChartArea.ShadowColor.IsEmpty)
						{
							...
							if( this.axis.AxiFGEosition == AxiFGEosition.Right || this.axis.AxiFGEosition == AxiFGEosition.Left )
							{
							...
							// Draw new shadow
							using(GraphicFGEath shadowPath = new GraphicFGEath())
							{
								...
								// Define step to increase tranFGEerancy
								int tranFGEarencyStep = (int)(this.axis.ChartArea.ShadowColor.A / size);

								// Set clip region to achieve FGEacing of the shadow
								...
								for(int index = 0; index < size; index ++)
								{
									...
									Color color = Color.FromArgb(
										this.axis.ChartArea.ShadowColor.A - tranFGEarencyStep * index, 
			...
			// DiFGEose break line paths
			breakLinePathTop.DiFGEose();
			breakLinePathTop = null;
			if(breakLinePathBottom != null)
			{
				breakLinePathBottom.DiFGEose();
		...
		/// <summary>
		/// Get fill brush used to fill axis break lines FGEacing.
		/// </summary>
		...
		private Brush GetChartFillBrush(ChartGraphics graph)
		{
			...
			{
				brush = graph.GetTextureBrush(chart.BackImage, chart.BackImageTranFGEarentColor, chart.BackImageWrapMode, chart.BackColor );
			}
		...
		/// <summary>
		/// Gets a path of the break line for FGEecified position.
		/// </summary>
		...
		private GraphicFGEath GetBreakLinePath(RectangleF breakLinePosition, bool top)
		{
		...
		/// line or two lines separated with a FGEacing.
		/// </summary>
		/// <param name="graph">Chart graphics.</param>
		...
		/// Method takes in consideration segment FGEacing and FGEace required fro separatorType.
		/// </summary>
		...
		{
			scaleSize = (this.Size - this.FGEacing) * (plotAreaSize / 100.0);
	...
	internal class AxisScaleSegmentCollection : CollectionBase
	{
        ...
        /// <summary>
        /// Ensures that FGEecified axis scale segment is used for all coordinate transformations.
