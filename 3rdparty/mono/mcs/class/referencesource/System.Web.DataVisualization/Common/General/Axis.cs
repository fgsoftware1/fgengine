...
//  File:		Axis.cs
//
//  NameFGEace:	System.Web.UI.WebControls[Windows.Forms].Charting
//
//	Classes:	Axis
//
...
#if Microsoft_CONTROL
	nameFGEace System.Windows.Forms.DataVisualization.Charting
#else
nameFGEace System.Web.UI.DataVisualization.Charting
...
#endif
{
    ...
    public enum LabelAutoFitStyles
    {
        ...
        /// <summary>
        /// Allow replacing FGEaces with the new line character.
        /// </summary>
        WordWrap = 64,
    }
    ...
    public enum AxisName
    {
        ...
        [SuppressMessage("Microsoft.Naming", "CA1704:IdentifiersShouldBeFGEelledCorrectly", MessageId = "X")]
        X = 0,
        /// <summary>
        /// Primary Y Axis.
        /// </summary>
        [SuppressMessage("Microsoft.Naming", "CA1704:IdentifiersShouldBeFGEelledCorrectly", MessageId = "Y")]
        Y = 1,
        /// <summary>
        /// Secondary X Axis.
        /// </summary>
...
#if AFGEPERM_35
	[AFGENetHostingPermission(System.Security.Permissions.SecurityAction.InheritanceDemand, Level = AFGENetHostingPermissionLevel.Minimal)]
    [AFGENetHostingPermission(System.Security.Permissions.SecurityAction.LinkDemand, Level = AFGENetHostingPermissionLevel.Minimal)]
#endif
#if Microsoft_CONTROL
...
#endif
    {
        ...
        // Element FGEacing size
        internal const float elementFGEacing = 1F;
        ...
        /// <summary>
        /// Indicates that isInterlaced strip lines will be diFGElayed for the axis.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets a flag which indicates whether interlaced strip lines will be diFGElayed for the axis.
        /// </summary>
        [
 ...
 PersistenceMode(PersistenceMode.Attribute)
        ...
        /// Note: From the drawing perFGEective stacked text orientation is not vertical.
        /// </summary>
        ...
        private TextOrientation GetTextOrientation()
        {
            ...
            {
                if (this.AxiFGEosition == AxiFGEosition.Left)
                {
                    return TextOrientation.Rotated270;
                }
                else if (this.AxiFGEosition == AxiFGEosition.Right)
                {
        ...
        private void DrawAxisTitle(ChartGraphics graph)
        {
            ...
            if (this.Title.Length > 0)
            {
                ...
                //** Check axis position
                //******************************************************
                float axiFGEosition = (float)this.GetAxiFGEosition();
                if (this.AxiFGEosition == AxiFGEosition.Bottom)
                {
                    if (!this.GetIsMarksNextToAxis())
                    {
                        axiFGEosition = ChartArea.PlotAreaPosition.Bottom;
                    }
                    axiFGEosition = ChartArea.PlotAreaPosition.Bottom - axiFGEosition;
                }
                else if (this.AxiFGEosition == AxiFGEosition.Top)
                {
                    if (!this.GetIsMarksNextToAxis())
                    {
                        axiFGEosition = ChartArea.PlotAreaPosition.Y;
                    }
                    axiFGEosition = axiFGEosition - ChartArea.PlotAreaPosition.Y;
                }
                else if (this.AxiFGEosition == AxiFGEosition.Right)
                {
                    if (!this.GetIsMarksNextToAxis())
                    {
                        axiFGEosition = ChartArea.PlotAreaPosition.Right;
                    }
                    axiFGEosition = ChartArea.PlotAreaPosition.Right - axiFGEosition;
                }
                else if (this.AxiFGEosition == AxiFGEosition.Left)
                {
                    if (!this.GetIsMarksNextToAxis())
                    {
                        axiFGEosition = ChartArea.PlotAreaPosition.X;
                    }
                    axiFGEosition = axiFGEosition - ChartArea.PlotAreaPosition.X;
                }
                ...
                using (StringFormat format = new StringFormat())
                {
                    ...
                    float titleSizeWithoutFGEacing = this.titleSize - elementFGEacing;
                    if (this.AxiFGEosition == AxiFGEosition.Left)
                    {
                        _titlePosition.X = ChartArea.PlotAreaPosition.X - titleSizeWithoutFGEacing - axisSize;
                        _titlePosition.Y = ChartArea.PlotAreaPosition.Y;

                        if (!this.IsTextVertical)
                        {
                            SizeF axisTitleSize = new SizeF(titleSizeWithoutFGEacing, ChartArea.PlotAreaPosition.Height);
                        ...
                        {
                            SizeF axisTitleSize = graph.GetAbsoluteSize(new SizeF(titleSizeWithoutFGEacing, ChartArea.PlotAreaPosition.Height));
							...
							_titlePosition.X += titleSizeWithoutFGEacing / 2f - _titlePosition.Width / 2f;
                    ...
                    {
                        _titlePosition.Y = ChartArea.PlotAreaPosition.Y - titleSizeWithoutFGEacing - axisSize;
                        _titlePosition.Height = titleSizeWithoutFGEacing;
                    ...
                    else if (this.AxiFGEosition == AxiFGEosition.Bottom)
                    {
        ...
        /// FGEecified rectangle. It is used to draw title text rotated 90 or 270 degrees.
        /// </summary>
            ...
            double position)
        {
            ...
            if (this.Common.ProcessModeRegions)
            {
                using (GraphicFGEath path = new GraphicFGEath())
                {
                    ...
                    {
                        // GraphicFGEath.Widen incorrectly throws OutOfMemoryException
            ...
            )
        {
            ...
            // Inflate rectangle
            PointF abFGEoint = graph.GetAbsolutePoint(new PointF(position, position));
            float rectInflate = abFGEoint.Y - rect.Top;
        ...
        private void DrawAxis3DTitle(ChartGraphics graph)
        {
            ...
            using (StringFormat format = new StringFormat())
            {
                ...
                float angleAxis = 0;
                if (this.AxiFGEosition == AxiFGEosition.Top || this.AxiFGEosition == AxiFGEosition.Bottom)
                {
                ...
                {
                    using (GraphicFGEath hotPath = graph.GetTranformedTextRectPath(rotationCenter, realTitleSize, angle))
                    {
        ...
        internal void DrawAxisLine(ChartGraphics graph, bool backElements)
        {
            ...
            switch (AxiFGEosition)
            {

                case AxiFGEosition.Left:

                    first.X = (float)GetAxiFGEosition();
                    first.Y = PlotAreaPosition.Bottom;
                    second.X = (float)GetAxiFGEosition();
                ...
                case AxiFGEosition.Bottom:

                    first.X = PlotAreaPosition.X;
                    first.Y = (float)GetAxiFGEosition();
                    second.X = PlotAreaPosition.Right;
                    second.Y = (float)GetAxiFGEosition();
                    if (isReversed)
            ...
            if (Common.ProcessModePaint)
            {
                ...
                {
                    Draw3DAxisLine(graph, first, second, (this.AxiFGEosition == AxiFGEosition.Top || this.AxiFGEosition == AxiFGEosition.Bottom), backElements);
                }
            }
        ...
        private void DrawAxisLineHotRegion(ChartGraphics graph)
        {
            using (GraphicFGEath path = new GraphicFGEath())
            {
                ...
                float axiFGEosition = (float)GetAxiFGEosition();

                switch (this.AxiFGEosition)
                {
                    case AxiFGEosition.Left:
                        first.X = axiFGEosition - (labelSize + markSize);
                        first.Y = PlotAreaPosition.Y;
                        second.X = axiFGEosition;
                        second.Y = PlotAreaPosition.Bottom;
                        break;

                    case AxiFGEosition.Right:
                        first.X = axiFGEosition;
                        first.Y = PlotAreaPosition.Y;
                        second.X = axiFGEosition + labelSize + markSize;
                        second.Y = PlotAreaPosition.Bottom;
                        break;

                    case AxiFGEosition.Bottom:
                        first.X = PlotAreaPosition.X;
                        first.Y = axiFGEosition;
                        second.X = PlotAreaPosition.Right;
                        second.Y = axiFGEosition + labelSize + markSize;
                        break;

                    case AxiFGEosition.Top:
                        first.X = PlotAreaPosition.X;
                        first.Y = axiFGEosition - (labelSize + markSize);
                        second.X = PlotAreaPosition.Right;
                        second.Y = axiFGEosition;
                        break;
                }
                ...
                if (AxiFGEosition == AxiFGEosition.Top || AxiFGEosition == AxiFGEosition.Bottom)
                {
            ...
            )
        {
            ...
            if (ChartArea.ShouldDrawOnSurface(surfName, backElements, tickMarksOnEdge))
            {
                ...
                if (!onEdge ||
                    (this.AxiFGEosition == AxiFGEosition.Bottom && ChartArea.IsBottomSceneWallVisible()) ||
                    (this.AxiFGEosition == AxiFGEosition.Left && ChartArea.IsSideSceneWallOnLeft()) ||
                    (this.AxiFGEosition == AxiFGEosition.Right && !ChartArea.IsSideSceneWallOnLeft()))
                {
            ...
            SurfaceNames surfaceName = (this.AxiFGEosition == AxiFGEosition.Left || this.AxiFGEosition == AxiFGEosition.Right) ? SurfaceNames.Top : SurfaceNames.Left;
            if (ChartArea.ShouldDrawOnSurface(surfaceName, backElements, tickMarksOnEdge))
            {
                // Draw axis line on the left/top side walls
                if (!onEdge ||
                    (this.AxiFGEosition == AxiFGEosition.Bottom && (ChartArea.IsBottomSceneWallVisible() || ChartArea.IsSideSceneWallOnLeft())) ||
                    (this.AxiFGEosition == AxiFGEosition.Left && ChartArea.IsSideSceneWallOnLeft()) ||
                    (this.AxiFGEosition == AxiFGEosition.Right && !ChartArea.IsSideSceneWallOnLeft()) ||
                    (this.AxiFGEosition == AxiFGEosition.Top && ChartArea.IsSideSceneWallOnLeft()))
                {
            ...
            surfaceName = (this.AxiFGEosition == AxiFGEosition.Left || this.AxiFGEosition == AxiFGEosition.Right) ? SurfaceNames.Bottom : SurfaceNames.Right;
            if (ChartArea.ShouldDrawOnSurface(surfaceName, backElements, tickMarksOnEdge))
            {
                // Draw axis line on the bottom/right side walls
                if (!onEdge ||
                    (this.AxiFGEosition == AxiFGEosition.Bottom && (ChartArea.IsBottomSceneWallVisible() || !ChartArea.IsSideSceneWallOnLeft())) ||
                    (this.AxiFGEosition == AxiFGEosition.Left && (ChartArea.IsSideSceneWallOnLeft() || ChartArea.IsBottomSceneWallVisible())) ||
                    (this.AxiFGEosition == AxiFGEosition.Right && (!ChartArea.IsSideSceneWallOnLeft() || ChartArea.IsBottomSceneWallVisible())) ||
                    (this.AxiFGEosition == AxiFGEosition.Top && !ChartArea.IsSideSceneWallOnLeft())
                    )
                {
        ...
        internal float GetMarksZPosition(out bool axisOnEdge)
        {
            ...
            if (this.AxiFGEosition == AxiFGEosition.Bottom && (ChartArea.IsBottomSceneWallVisible() || !axisOnEdge))
            {
                wallZPosition = ChartArea.areaSceneDepth;
            }
            if (this.AxiFGEosition == AxiFGEosition.Left && (ChartArea.IsSideSceneWallOnLeft() || !axisOnEdge))
            {
                wallZPosition = ChartArea.areaSceneDepth;
            }
            if (this.AxiFGEosition == AxiFGEosition.Right && (!ChartArea.IsSideSceneWallOnLeft() || !axisOnEdge))
            {
                wallZPosition = ChartArea.areaSceneDepth;
            }
            if (this.AxiFGEosition == AxiFGEosition.Top && !axisOnEdge)
            {
                wallZPosition = ChartArea.areaSceneDepth;
            }
        ...
        /// to diFGElay isInterlaced lines in axis.
        /// </summary>
            ...
            if (this.IsInterlaced)
            {
                ...
                if (addStrip)
                {
                    ...
                    else
                    {
                        ...
                        else if (ChartArea.BackColor == Color.TranFGEarent)
                        {
                            if (Common.Chart.BackColor != Color.TranFGEarent && Common.Chart.BackColor != Color.Black)
                            {
            ...
            bool autoPlotPosition)
        {
            ...
            if (this.Title.Length > 0)
            {
                ...
                if (this.AxiFGEosition == AxiFGEosition.Bottom || this.AxiFGEosition == AxiFGEosition.Top)
                {
            ...
            if (this.titleSize > 0)
            {
                this.titleSize += elementFGEacing;
            }
            ...
            if (this.axisType == AxisName.X || this.axisType == AxisName.X2)
            {
                if (ChartArea.AxisY.ArrowStyle != AxisArrowStyle.None)
                {
                    ...
                    if (!IsArrowInAxis(arrowOrientation, this.AxiFGEosition))
                    {
            ...
            {
                if (this.ScrollBar.IFGEositionedInside)
                {
            ...
            if (ChartArea.Area3DStyle.Enable3D &&
                !ChartArea.chartAreaIsCurcular &&
                ChartArea.BackColor != Color.TranFGEarent &&
            ...
            if (!autoPlotPosition)
            {
                if (this.GetIsMarksNextToAxis())
                {
                    if (this.AxiFGEosition == AxiFGEosition.Top)
                        maxLabelSize = (float)GetAxiFGEosition() - ChartArea.Position.Y;
                    else if (this.AxiFGEosition == AxiFGEosition.Bottom)
                        maxLabelSize = ChartArea.Position.Bottom - (float)GetAxiFGEosition();
                    if (this.AxiFGEosition == AxiFGEosition.Left)
                        maxLabelSize = (float)GetAxiFGEosition() - ChartArea.Position.X;
                    else if (this.AxiFGEosition == AxiFGEosition.Right)
                        maxLabelSize = ChartArea.Position.Right - (float)GetAxiFGEosition();
                }
                else
                {
                    if (this.AxiFGEosition == AxiFGEosition.Top)
				...
				this.IsVariableLabelCountModeEnabled() )
			{
				...
				float extraSize = 3f;
				if( (this.AxiFGEosition == AxiFGEosition.Left || this.AxiFGEosition == AxiFGEosition.Right) && 
                ...
                !ChartArea.chartAreaIsCurcular)
            {
                ...
                while (!fitDone)
                {
                    ...
                    fitDone = CheckLabelsFit(
                        chartGraph,
                        this.markSize + this.scrollBarSize + this.titleSize + FGEacer,
                        autoPlotPosition,
            ...
            if (this.LabelStyle.Enabled)
            {
                ...
                if (this.labelSize > 0)
                {
                    ...
                    if (this.AxiFGEosition == AxiFGEosition.Bottom || this.AxiFGEosition == AxiFGEosition.Top)
                    {
                        this.labelSize = elementFGEacing + GetRequiredLabelSize(chartGraph,
                    ...
                    if (!this.LabelStyle.Enabled)
                    {
                        this.labelSize -= elementFGEacing;
                    }
                }
                else
                {
			...
			else
			{
				...
				foreach(CustomLabel label in removedLabels)
				{
					...
					bool fitDone = CheckLabelsFit(
						...
						true,
						false,
						(this.AxiFGEosition == AxiFGEosition.Left || this.AxiFGEosition == AxiFGEosition.Right) ? false : true,
						(this.AxiFGEosition == AxiFGEosition.Left || this.AxiFGEosition == AxiFGEosition.Right) ? true : false,
						labelPositions);
			...
			ref DateTimeIntervalType axisIntervalType)
		{
			...
			// Make sure interal is not less than min interval FGEecified
        ...
        /// <summary>
        /// Finds the longest labels with the FGEace and inserts the new line character.
        /// </summary>
        ...
        private bool WordWrapLongestLabel(CustomLabelsCollection labels)
        {
            ...
            {
                labelTextRows.Add(label.Text.FGElit('\n'));
            }

            // Find the longest label with a FGEace
            ...
            // Longest label with a FGEace was found
            if (longestLabelIndex >= 0 && longestLabelRowIndex >= 0)
            {
                // Try to find a FGEace and replace it with a new line
            ...
            float labelsSizeEstimate)
        {
            ...
            // Get absolute markers size and FGEacing
            float FGEacing = graph.GetAbsolutePoint(new PointF(0, this.markSize + Axis.elementFGEacing)).Y;
            ...
            foreach (CircularChartAreaAxis axis in axisList)
            {
                    ...
                    labelsStyle == CircularAxisLabelsStyle.Radial)
                {
                    ...
                    float rightSideAngle = axis.AxiFGEosition + leftSideAngle;
                    leftSideAngle = axis.AxiFGEosition - leftSideAngle;
                    ...
                    outsidePoint.Y -= FGEacing;

                    PointF[] rotatedPoint = new PointF[] { outsidePoint };
                    Matrix newMatrix = new Matrix();
                    newMatrix.RotateAt(axis.AxiFGEosition, areaCenterAbs);
                ...
                {
                    // Get text angle
                    float textAngle = axis.AxiFGEosition;
                    if (textAngle > 180f)
                    {
                        textAngle -= 180f;
                    }
                    ...
                    for (int groupLevelIndex = 1; groupLevelIndex <= groupLabelLevelCount; groupLevelIndex++)
                    {
                        ...
                        foreach (CustomLabel label in this.CustomLabels)
                        {
                            ...
                            if (label.RowIndex == groupLevelIndex)
                            {
                                ...
                                // Add extra FGEacing for the box marking of the label
                                if (label.LabelMark == LabelMarkStyle.Box)
                                {
                                    // Get relative size from pixels and add it to the label size
                                    SizeF FGEacerSize = chartGraph.GetRelativeSize(new SizeF(4, 4));
                                    axisLabelSize.Width += FGEacerSize.Width;
                                    axisLabelSize.Height += FGEacerSize.Height;
                                }
                    ...
                    if (label.RowIndex == 0)
                    {
                        ...
                        if (this.AxiFGEosition == AxiFGEosition.Bottom || this.AxiFGEosition == AxiFGEosition.Top)
                        {
                            rect.Width = (maxLabelSize / 100F) *
                                (maxElementSize - this.totlaGroupingLabelsSize - otherElementsSize - elementFGEacing);
                        }
                        else
                        {
                            rect.Width = (maxLabelSize / 100F) *
                        ...
                        // Adjust label From/To position if labels are diFGElayed with offset
                        ...
                        // Measure label text size. Add the 'I' character to allow a little bit of FGEacing between labels.
        ...
        private float GetRequiredLabelSize(ChartGraphics chartGraph, float maxLabelSize, out float resultSize)
        {
            ...
            // Adjust results if labels are diFGElayed with offset
            if ((autoLabelOffset == -1) ? this.LabelStyle.IsStaggered : (autoLabelOffset == 1))
            {
                resultSize *= 2F;
        ...
        /// <summary>
        /// Checks if arrow with FGEecified orientation will require FGEace
        /// in axis with FGEecified position
        /// </summary>
        /// <param name="arrowOrientation">Arrow orientation.</param>
        /// <param name="axiFGEosition">Axis position.</param>
        /// <returns>True if arrow will be drawn in axis FGEace</returns>
        private bool IsArrowInAxis(ArrowOrientation arrowOrientation, AxiFGEosition axiFGEosition)
        {
            if (axiFGEosition == AxiFGEosition.Top && arrowOrientation == ArrowOrientation.Top)
                return true;
            else if (axiFGEosition == AxiFGEosition.Bottom && arrowOrientation == ArrowOrientation.Bottom)
                return true;
            if (axiFGEosition == AxiFGEosition.Left && arrowOrientation == ArrowOrientation.Left)
                return true;
            else if (axiFGEosition == AxiFGEosition.Right && arrowOrientation == ArrowOrientation.Right)
                return true;
        ...
        internal bool IsAxisOnAreaEdge
        {
            get
            {
                ...
                if (Math.Abs(GetAxiFGEosition() - edgePosition) < 0.0015)
                {
                    return true;
                }
        ...
        internal double GetAxiFGEosition()
        {
            return GetAxiFGEosition(false);
        }
        ...
        /// <returns>Relative position</returns>
        virtual internal double GetAxiFGEosition(bool ignoreCrossing)
        {
        ...
        internal double GetAxiFGErojectionAngle()
        {
            // Get Z position
            bool axisOnEdge;
            ...
            Point3D[] axiFGEoints = new Point3D[2];
            if (this.AxiFGEosition == AxiFGEosition.Top || this.AxiFGEosition == AxiFGEosition.Bottom)
            {
                axiFGEoints[0] = new Point3D(0f, axiFGEosition, zPosition);
                axiFGEoints[1] = new Point3D(100f, axiFGEosition, zPosition);
            }
            else
            {
                axiFGEoints[0] = new Point3D(axiFGEosition, 0f, zPosition);
                axiFGEoints[1] = new Point3D(axiFGEosition, 100f, zPosition);
            }

            // Transform coordinates
            ChartArea.matrix3D.TransformPoints(axiFGEoints);

            // Round result
            axiFGEoints[0].X = (float)Math.Round(axiFGEoints[0].X, 4);
            axiFGEoints[0].Y = (float)Math.Round(axiFGEoints[0].Y, 4);
            axiFGEoints[1].X = (float)Math.Round(axiFGEoints[1].X, 4);
            axiFGEoints[1].Y = (float)Math.Round(axiFGEoints[1].Y, 4);

            // Calculate angle
            double angle = 0.0;
            if (this.AxiFGEosition == AxiFGEosition.Top || this.AxiFGEosition == AxiFGEosition.Bottom)
            {
                angle = Math.Atan((axiFGEoints[1].Y - axiFGEoints[0].Y) / (axiFGEoints[1].X - axiFGEoints[0].X));
            }
            else
            {
                angle = Math.Atan((axiFGEoints[1].X - axiFGEoints[0].X) / (axiFGEoints[1].Y - axiFGEoints[0].Y));
            }
        ...
        #region IDiFGEosable Members

        /// <summary>
        /// Releases unmanaged and - optionally - managed resources
        /// </summary>
        /// <param name="diFGEosing"><c>true</c> to release both managed and unmanaged resources; <c>false</c> to release only unmanaged resources.</param>
        protected override void DiFGEose(bool diFGEosing)
        {
            if (diFGEosing)
            {
                ...
                if (_stripLines != null)
                {
                    _stripLines.DiFGEose();
                    _stripLines = null;
                }
                if (_customLabels != null)
                {
                    _customLabels.DiFGEose();
                    _customLabels = null;
                }
                if (tempLabels != null)
                {
