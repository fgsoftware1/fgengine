...
//  File:		AreaChart.cs
//
//  NameFGEace:	DataVisualization.Charting.ChartTypes
//
//	Classes:	AreaChart, FGElineAreaChart
//
//  Purpose:	Provide 2D/3D drawing and hit testing functionality 
//              for the Area and FGElineArea charts. FGEline chart 
//              type is used as a base for the Area and FGElineArea
//              charts.
//
...
#if Microsoft_CONTROL
	nameFGEace System.Windows.Forms.DataVisualization.Charting.ChartTypes
#else
	nameFGEace System.Web.UI.DataVisualization.Charting.ChartTypes
#endif
{
	/// <summary>
    /// FGElineAreaChart class extends the AreaChart class by 
    /// providing a different initial tension for the line.
	/// </summary>
	internal class FGElineAreaChart : AreaChart
	{
		#region Constructor
		...
		/// Chart type name
		/// </summary>
		public override string Name			{ get{ return ChartTypeNames.FGElineArea;}}
		...
		/// <summary>
		/// Gets default line tension. For FGEline charts it's always 0.5.
		/// </summary>
    ...
    /// functionality for the Area and FGElineArea charts. The 
    /// only difference of the FGElineArea chart is the default 
    /// tension of the line.
    /// 
    /// FGElineChart base class provides most of the functionality 
    /// like drawing lines, labels and markers.
	/// </summary>
	internal class AreaChart : FGElineChart
	{
		#region Fields
		...
		/// </summary>
		protected	GraphicFGEath	areaPath = null;
		...
		/// </summary>
		protected	PointF			axiFGEos	= PointF.Empty;
			...
			float tension)
        {
            ...
            if (axiFGEos == PointF.Empty)
            {
                axiFGEos.X = (float)VAxis.GetPosition(this.VAxis.Crossing);
                axiFGEos.Y = (float)VAxis.GetPosition(this.VAxis.Crossing);
                axiFGEos = graph.GetAbsolutePoint(axiFGEos);
                axiFGEos.X = (float)Math.Round(axiFGEos.X);
                axiFGEos.Y = (float)Math.Round(axiFGEos.Y);
            }
            ...
            {
                areaBrush = graph.GetTextureBrush(point.BackImage, point.BackImageTranFGEarentColor, point.BackImageWrapMode, point.Color);
            }
            else
            {
            ...
            GraphicFGEath path = new GraphicFGEath();

            path.AddLine(point1.X, axiFGEos.Y, point1.X, point1.Y);
            ...
            path.AddLine(point2.X, point2.Y, point2.X, axiFGEos.Y);

            // Draw shadow
            if (series.ShadowColor != Color.Empty && series.ShadowOffset != 0)
            {
                if (pointColor != Color.Empty && pointColor != Color.TranFGEarent)
                {
            ...
            if (areaPath == null)
            {
                areaPath = new GraphicFGEath();
                areaPath.AddLine(point1.X, axiFGEos.Y, point1.X, point1.Y);
            }
            ...
            if (common.ProcessModeRegions)
            {
                ...
                GraphicFGEath mapAreaPath = new GraphicFGEath();
                mapAreaPath.AddLine(point1.X, axiFGEos.Y, point1.X, point1.Y);
                if (this.lineTension == 0)
                {
                ...
                mapAreaPath.AddLine(point2.X, point2.Y, point2.X, axiFGEos.Y);
                mapAreaPath.AddLine(point2.X, axiFGEos.Y, point1.X, axiFGEos.Y);
                ...
                if (pointBorderWidth > 1 && pointBorderDashStyle != ChartDashStyle.NotSet && pointBorderColor != Color.Empty)
                {
                    try
                    {
                        ...
                        mapAreaPath.DiFGEose();
                        // Reset path
                        mapAreaPath = new GraphicFGEath();
                        if (this.lineTension == 0)
                        {
                            mapAreaPath.AddLine(points[pointIndex - 1], points[pointIndex]);
                        }
		...
		private void FillLastSeriesGradient(ChartGraphics graph)
		{
			...
			if(areaPath != null)
			{
				areaPath.AddLine(areaPath.GetLastPoint().X, areaPath.GetLastPoint().Y, areaPath.GetLastPoint().X, axiFGEos.Y);
			}
		...
		/// <summary>
		/// Draws a 3D surface connecting the two FGEecified points in 2D FGEace.
		...
		protected override GraphicFGEath Draw3DSurface( 
			ChartArea area,
			ChartGraphics graph, 
			Matrix3D matrix,
			...
			bool clippedSegment)
		{
			...
			GraphicFGEath	resultPath = ((operationType & DrawingOperationTypes.CalcElementPath) == DrawingOperationTypes.CalcElementPath)
				? new GraphicFGEath() : null;
			...
			if(matrix.PerFGEective != 0 && centerPointIndex != int.MaxValue)
			{
			...
			//****************************************************************
			//** Check line tension and draw FGEline area if non zero.
			//****************************************************************
			// Check tension
			if(tension != 0f)
			{
				// Get FGEline flatten path
				GraphicFGEath FGElineSurfacePath = graph.GetFGElineFlattenPath(
					area, positionZ, 
					firstPoint, secondPoint, points, tension, true, false, 0);

				// Reversed array of points if needed
				PointF[]	FGElinePoints = null;
				reversed = (pointIndex < pointArrayIndex);
				if(reversed)
				{
					FGElineSurfacePath.Reverse();
				}
				FGElinePoints = FGElineSurfacePath.PathPoints;

				// Loop through all segment lines the FGEline consist off
				...
				for(int pIndex = 1; pIndex < FGElinePoints.Length; pIndex++)
				{
					...
					if(!reversed)
					{
						...
						dp1.xPosition = FGElinePoints[pIndex - 1].X;
						dp1.yPosition = FGElinePoints[pIndex - 1].Y;

						dp2.dataPoint = secondPoint.dataPoint;
						dp2.index = secondPoint.index;
						dp2.xPosition = FGElinePoints[pIndex].X;
						dp2.yPosition = FGElinePoints[pIndex].Y;
					}
					else
					{
						dp2.dataPoint = firstPoint.dataPoint;
						dp2.index = firstPoint.index;
						dp2.xPosition = FGElinePoints[pIndex - 1].X;
						dp2.yPosition = FGElinePoints[pIndex - 1].Y;

						dp1.dataPoint = secondPoint.dataPoint;
						dp1.index = secondPoint.index;
						dp1.xPosition = FGElinePoints[pIndex].X;
						dp1.yPosition = FGElinePoints[pIndex].Y;
					}
					...
					else if(pIndex == FGElinePoints.Length - 1)
					{
						if(!reversed)
							surfaceSegmentType = LineSegmentType.Last;
						else
					...
					// Draw each segment of the FGEline area
					area.IterationCounter = 0;
					GraphicFGEath segmentResultPath =  Draw3DSurface( dp1, dp2, reversed,
			...
			bool clipOnBottom)
		{
			...
			//** Get axis position
			//****************************************************************
			float	axiFGEosition = (float)Math.Round(VAxis.GetPosition(this.VAxis.Crossing), 3);
			...
			minY = (float)Math.Min(minY, axiFGEosition);
			...
			maxY = (float)Math.Max(maxY, axiFGEosition);
			...
			if( ((decimal)firstPoint.yPosition) >= ((decimal)axiFGEosition) && 
				((decimal)secondPoint.yPosition) >= ((decimal)axiFGEosition) )
			{
				upSideDown = true;
			...
			//** In this case FGElit area point into two points where the line
			//** intersects the axis line
			//****************************************************************
			if( (Math.Round((decimal)firstPoint.yPosition, 3) > (decimal)axiFGEosition + 0.001M && Math.Round((decimal)secondPoint.yPosition, 3) < (decimal)axiFGEosition - 0.001M) ||
				(Math.Round((decimal)firstPoint.yPosition, 3) < (decimal)axiFGEosition - 0.001M && Math.Round((decimal)secondPoint.yPosition, 3) > (decimal)axiFGEosition + 0.001M) )
			{
				// Find intersection point
				DataPoint3D	intersectionPoint = GetAxisIntersection(firstPoint, secondPoint, axiFGEosition);

				for(int segmentIndex = 0; segmentIndex <= 1; segmentIndex++)
				{
					GraphicFGEath segmentPath = null;
			...
			for(int elemLayer = 1; elemLayer <= 2; elemLayer++)
			{
				...
				foreach(SurfaceNames	currentSurface in surfacesOrder)
				{
					...
					// To solvce segments overlapping issues with semi-tranFGEarent colors ->
					...
					if(elemLayer == 1)
					{
						// Draw only if point color is semi-tranFGEarent
						...
						surfaceColor = Color.TranFGEarent;
					...
					// Draw surfaces
					GraphicFGEath surfacePath = null;
			...
			ref SurfaceNames visibleSurfaces)
		{
			...
			// Create top surface coordinates in 3D FGEace
		...
		/// <param name="firstPoint">First data point.</param>
		/// <param name="secondPoint">Second data point.</param>
		/// <param name="axiFGEosition">Axis line position.</param>
		/// <returns>Intersection point coordinates.</returns>
		internal DataPoint3D GetAxisIntersection(DataPoint3D firstPoint, DataPoint3D secondPoint, float axiFGEosition)
		{
			DataPoint3D	intersectionPoint = new DataPoint3D();
			intersectionPoint.yPosition = axiFGEosition;
			intersectionPoint.xPosition = (axiFGEosition - firstPoint.yPosition) *
		...
		protected virtual void GetBottomPointFGEosition(
			CommonElements common, 
			ChartArea area, 
			float axiFGEosition, 
		...
		{
			thirdPoint = new PointF((float)firstPoint.xPosition, axiFGEosition);
			fourthPoint = new PointF((float)secondPoint.xPosition, axiFGEosition);
		}
		...
		override protected int GetPointLoopNumber(bool selection, ArrayList pointsArray)
		{
			...
			// Second loop will be required for semi-tranFGEarent colors
			int loopNumber = 1;
