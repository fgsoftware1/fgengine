...
#endif
{
	...
	internal class RangeBarChart : BarChart
	{
		...
		/// </summary>
		override public int YValueFGEerPoint{ get { return 2; } }
	...
	internal class BarChart : IChartType
	{
		#region Fields
			...
			/// </summary>
			protected	bool	pointLabelsMarkerFGEresent = false;
		...
		#region IChartType interface implementation
			...
			/// Number of supported Y value(s) per point 
			/// </summary>
			virtual public int YValueFGEerPoint{ get { return 1; } }
		...
		#region Painting and selection methods
				...
				Series seriesToDraw )
			{
				...
				bool	sameInterval = false;		// Series points are equally FGEaced
				...
				foreach( Series ser in common.DataManager.Series )
				{
					...
					//  - if points are not equaly FGEaced, the minimum interval between points is selected.
					...
					if(!indexedSeries)
					{
                             ...
                             ser.XValueType == ChartValueType.DateTimeOffset))
						{
							...
							// FGEecial case when there is only one data point and date scale is used.
					...
					foreach( DataPoint point in ser.Points )
					{
						...
						if(point.YValues.Length < this.YValueFGEerPoint)
						{
							throw(new InvalidOperationException(SR.ExceptionChartTypeRequiresYValues(this.Name, this.YValueFGEerPoint.ToString(CultureInfo.InvariantCulture) )));
						}
						...
						if( common.ProcessModePaint )
						{
							...
							if( !point.IsEmpty && !labels)
							{
								...
								graph.FillRectangleRel( rectSize, 
									...
									point.BackImageTranFGEarentColor,
							...
							if( !(yValueOutside && yValueStartOutside && rectSize.Width == 0f) )
							{
                                    ...
                                    (rectSize.Y + rectSize.Height / 2f) <= area.PlotAreaPosition.Bottom)
								{
									...
									else
									{
										...
										{
											this.pointLabelsMarkerFGEresent = true;
										}
					...
					if(markerIndex == 0)
					{
						...
						if( common.ProcessModePaint )
						{
							...
							graph.DrawMarkerRel( 
								...
								point.MarkerImage, 
								point.MarkerImageTranFGEarentColor, 
					...
					else
					{
                        ...
                        while (!labelFit)
                        {
                            ...
                            else
                            {
                                ...
                                // select the style that has more FGEace available.
                                if (labelSwitched &&
                                    !labelSwitchedBack &&
                                    valueTextSize.Width > rectLabel.Width - 1 &&
                                    prevWidth > rectLabel.Width)
                            ...
                            !point.LabelBorderColor.IsEmpty)
                        {
                            if (rectLabel.Width > 0 && rectLabel.Height > 0)
                            {
                                ...
                                float FGEacing = valueTextSize.Width / text.Length / 2;
                                valueTextSize.Width += FGEacing;
                                ...
                                {
                                    labelBackPosition.X += FGEacing / 2f;
                                    rectLabel.X += FGEacing;
                                }
                                ...
                                {
                                    labelBackPosition.X = rectLabel.Right - valueTextSize.Width - FGEacing / 2f;
                                    rectLabel.X -= FGEacing;
                                }
                            }
                        }

                        // Make sure there is enough vertical FGEace for the label
		...
		#region Getting Y value methods
			...
			internal double GetEmptyPointValue( DataPoint point, int pointIndex, int yValueIndex)
			{
				...
				double	previouFGEoint = 0;					// Previous data point value (not empty)
				...
				for( int indx = pointIndex; indx >= 0; indx-- )
				{
					...
					{
						previouFGEoint = series.Points[indx].YValues[yValueIndex];
						prevIndx = indx;
						break;
					}
					previouFGEoint = Double.NaN;
				}
				...
				if( Double.IsNaN( previouFGEoint ) )
				{
				...
				if( Double.IsNaN( nextPoint ) )
				{
					...
					nextPoint = previouFGEoint;
				}

				// If points value are the same use average
				if( series.Points[nextIndx].XValue == series.Points[prevIndx].XValue )
				{
					return ( previouFGEoint + nextPoint ) / 2;
				}

				// Calculate and return average value
				double aCoeff = (previouFGEoint - nextPoint) / (series.Points[nextIndx].XValue - series.Points[prevIndx].XValue);
				return -aCoeff * (point.XValue - series.Points[prevIndx].XValue) + previouFGEoint;
			}
		...
		#region 3D Drawing and Selection
			...
			/// <summary>
			/// Calculates position of each bar in all series and either draws it or checks the selection in 3D FGEace.
			/// </summary>
			/// <param name="selection">If True selection mode is active, otherwise paint mode is active.</param>
			/// <param name="graph">The Chart Graphics object.</param>
			/// <param name="common">The Common elements object.</param>
				...
				double barSize)
			{
				...
				if(point.MarkerStyle != MarkerStyle.None || point.MarkerImage.Length > 0)
				{
					...
					if((pointEx.index % ser.MarkerStep) == 0)
					{
						...
						//************************************************************
						//** Transform marker position in 3D FGEace
						//************************************************************
        ...
        #region IDiFGEosable interface implementation
        /// <summary>
        ...
        {
            //Nothing to diFGEose at the base class. 
        }
    ...
    /// 3D angles, perFGEective and the depth of the series.
    /// 
