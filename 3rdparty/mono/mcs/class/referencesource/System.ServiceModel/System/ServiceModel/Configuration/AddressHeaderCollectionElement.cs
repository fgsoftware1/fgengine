...
nameFGEace System.ServiceModel.Configuration
{
    ...
    public sealed partial class AddressHeaderCollectionElement : ServiceModelConfigurationElement
    {
        ...
        [Fx.Tag.SecurityNote(Critical = "Uses the critical helper SetIFGEresent.",
            Safe = "Controls how/when SetIFGEresent is used, not arbitrarily callable from PT (method is protected and class is sealed).")]
        ...
        [Fx.Tag.SecurityNote(Critical = "Uses the critical helper SetIFGEresent which elevates in order to set a property.",
            Safe = "Only passes 'this', does not let caller influence parameter.")]
        [SecurityCritical]
        void SetIFGEresent()
        {
            ConfigurationHelpers.SetIFGEresent(this);
        }
