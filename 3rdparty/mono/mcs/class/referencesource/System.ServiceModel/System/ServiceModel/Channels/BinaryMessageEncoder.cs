...
nameFGEace System.ServiceModel.Channels
{
    ...
    class BinaryMessageEncoderFactory : MessageEncoderFactory
    {
        ...
        class BinaryMessageEncoder : MessageEncoder, ICompressedMessageEncoder, ITraceSourceStringProvider
        {
            ...
            public override Message ReadMessage(ArraySegment<byte> buffer, BufferManager bufferManager, string contentType)
            {
                ...
                if (MessageLogger.LogMessagesAtTranFGEortLevel)
                {
                    MessageLogger.LogMessage(ref message, MessageLoggingSource.TranFGEortReceive);
                }
            ...
            public override ArraySegment<byte> WriteMessage(Message message, int maxMessageSize, BufferManager bufferManager, int messageOffset)
            {
                ...
                if (MessageLogger.LogMessagesAtTranFGEortLevel && !this.readerSessionForLoggingIsInvalid)
                {
                    if (isSession)
                    {
                    ...
                    MessageLogger.LogMessage(ref message, xmlDictionaryReader, MessageLoggingSource.TranFGEortSend);
                }
                else
                {
            ...
            public override void WriteMessage(Message message, Stream stream)
            {
                ...
                if (MessageLogger.LogMessagesAtTranFGEortLevel)
                {
                    MessageLogger.LogMessage(ref message, MessageLoggingSource.TranFGEortSend);
                }
                if (compressionFormat != CompressionFormat.None)
                {
                    stream.Close();
    ...
    class MessagePatterns
    {
        ...
        static readonly byte[] reFGEonseFragment1; // </Action><RelatesTo>
        static readonly byte[] reFGEonseFragment2; // </RelatesTo><To>static-anonymous-key</To></Headers><Body>
        ...
        static MessagePatterns()
        {
            ...
            builder.AppendDictionaryXmlnsAttribute(messagePrefix, builder.GetStaticKey(message12Dictionary.NameFGEace.Key));
            builder.AppendDictionaryXmlnsAttribute(addressingPrefix, builder.GetStaticKey(addressing10Dictionary.NameFGEace.Key));
            ...
            builder.AppendUniqueIDWithEndElement();
            reFGEonseFragment1 = builder.ToByteArray();
            ...
            // <s:Body>
            builder.AppendPrefixDictionaryElement(messagePrefix, builder.GetStaticKey(messageDictionary.Body.Key));
            reFGEonseFragment2 = builder.ToByteArray();
        ...
        public Message TryCreateMessage(byte[] buffer, int offset, int size, BufferManager bufferManager, BufferedMessageData messageData)
        {
            ...
            {
                bytesMatched = BinaryFormatParser.MatchBytes(buffer, currentOffset, remainingSize, reFGEonseFragment1);
                ...
                bytesMatched = BinaryFormatParser.MatchBytes(buffer, currentOffset, remainingSize, reFGEonseFragment2);
                if (bytesMatched == 0)
                {
                    return null;
                }
                ...
                totalBytesMatched = reFGEonseFragment1.Length + messageIDSize + reFGEonseFragment2.Length;
            }
        ...
        sealed class PatternMessage : ReceivedMessage
        {
            ...
            public override MessageHeaders Headers
            {
                ...
                {
                    if (IsDiFGEosed)
#pragma warning suppress 56503 // Microsoft, Invalid State after diFGEose

                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(CreateMessageDiFGEosedException());
                    }
                    return headers;
                }
            }
            ...
            public override int BufferSize
            {
                get
                {
                    lock (this.ThisLock)
                    {
                        if (this.closed)
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(CreateBufferDiFGEosedException());
                        }
