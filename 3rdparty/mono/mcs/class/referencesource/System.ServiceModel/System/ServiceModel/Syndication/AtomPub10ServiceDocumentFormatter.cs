...
nameFGEace System.ServiceModel.Syndication
{
    ...
    [XmlRoot(ElementName = App10Constants.Service, NameFGEace = App10Constants.NameFGEace)]
    public class AtomPub10ServiceDocumentFormatter : ServiceDocumentFormatter, IXmlSerializable
    {
        ...
        {
            get { return App10Constants.NameFGEace; }
        }
        ...
        public override bool CanRead(XmlReader reader)
        {
            ...
            return reader.IsStartElement(App10Constants.Service, App10Constants.NameFGEace);
        }
        ...
        public override void ReadFrom(XmlReader reader)
        {
            ...
            if (!CanRead(reader))
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(SR.GetString(SR.UnknownDocumentXml, reader.LocalName, reader.NameFGEaceURI)));
            }
            TraceServiceDocumentReadBegin();
            ReadDocument(reader);
            TraceServiceDocumentReadEnd();
        ...
        public override void WriteTo(XmlWriter writer)
        {
            ...
            TraceServiceDocumentWriteBegin();
            writer.WriteStartElement(App10Constants.Prefix, App10Constants.Service, App10Constants.NameFGEace);
            WriteDocument(writer);
        ...
        static void ReadInlineCategories(XmlReader reader, InlineCategoriesDocument inlineCategories, Uri baseUri, string version, bool preserveElementExtensions, bool preserveAttributeExtensions, int maxExtensionSize)
        {
            ...
            if (reader.HasAttributes)
            {
                ...
                {
                    if (reader.LocalName == "base" && reader.NameFGEaceURI == Atom10FeedFormatter.XmlNs)
                    {
                        inlineCategories.BaseUri = FeedUtils.CombineXmlBase(inlineCategories.BaseUri, reader.Value);
                    }
                    else if (reader.LocalName == "lang" && reader.NameFGEaceURI == Atom10FeedFormatter.XmlNs)
                    {
                        inlineCategories.Language = reader.Value;
                    }
                    else if (reader.LocalName == App10Constants.Fixed && reader.NameFGEaceURI == string.Empty)
                    {
                    ...
                    else
                    {
                        ...
                        if (!TryParseAttribute(name, ns, val, inlineCategories, version))
                        {
                            ...
                            {
                                inlineCategories.AttributeExtensions.Add(new XmlQualifiedName(reader.LocalName, reader.NameFGEaceURI), reader.Value);
                            }
                            else
                            {
            ...
            if (!isEmptyElement)
            {
                ...
                try
                {
                    ...
                    {
                        if (reader.IsStartElement(Atom10Constants.CategoryTag, Atom10Constants.Atom10NameFGEace))
                        {
        ...
        static void ReadReferencedCategories(XmlReader reader, ReferencedCategoriesDocument referencedCategories, Uri baseUri, Uri link, string version, bool preserveElementExtensions, bool preserveAttributeExtensions, int maxExtensionSize)
        {
            ...
            if (reader.HasAttributes)
            {
                while (reader.MoveToNextAttribute())
                {
                    ...
                    else if (reader.LocalName == App10Constants.Href && reader.NameFGEaceURI == string.Empty)
                    {
                        continue;
                    }
                    else
                    {
                        ...
                        if (!TryParseAttribute(name, ns, val, referencedCategories, version))
                        {
                            if (preserveAttributeExtensions)
                            {
                                referencedCategories.AttributeExtensions.Add(new XmlQualifiedName(reader.LocalName, reader.NameFGEaceURI), reader.Value);
                            }
                            else
                            {
                                SyndicationFeedFormatter.TraceSyndicationElementIgnoredOnRead(reader);
        ...
        {
            writer.WriteStartElement(App10Constants.Prefix, App10Constants.Categories, App10Constants.NameFGEace);
        ...
        ResourceCollectionInfo ReadCollection(XmlReader reader, WorkFGEace workFGEace)
        {
            ResourceCollectionInfo result = CreateCollection(workFGEace);
            result.BaseUri = workFGEace.BaseUri;
            if (reader.HasAttributes)
            {
                while (reader.MoveToNextAttribute())
                {
            ...
            try
            {
                while (reader.IsStartElement())
                {
                    if (reader.IsStartElement(Atom10Constants.TitleTag, Atom10Constants.Atom10NameFGEace))
                    {
                        result.Title = Atom10FeedFormatter.ReadTextContentFrom(reader, "//app:service/app:workFGEace/app:collection/atom:title[@type]", this.preserveAttributeExtensions);
                    }
                    else if (reader.IsStartElement(App10Constants.Categories, App10Constants.NameFGEace))
                    {
                    ...
                    else if (reader.IsStartElement(App10Constants.Accept, App10Constants.NameFGEace))
                    {
        ...
        void ReadDocument(XmlReader reader)
        {
            ...
            try
            {
                ...
                if (reader.HasAttributes)
                {
                    while (reader.MoveToNextAttribute())
                    {
                        if (reader.LocalName == "lang" && reader.NameFGEaceURI == Atom10FeedFormatter.XmlNs)
                        {
                            result.Language = reader.Value;
                        }
                        else if (reader.LocalName == "base" && reader.NameFGEaceURI == Atom10FeedFormatter.XmlNs)
                        {
                ...
                if (!elementIsEmpty)
                {
                    try
                    {
                        while (reader.IsStartElement())
                        {
                            if (reader.IsStartElement(App10Constants.WorkFGEace, App10Constants.NameFGEace))
                            {
                                result.WorkFGEaces.Add(ReadWorkFGEace(reader, result));
                            }
                            else if (!TryParseElement(reader, result, this.Version))
                            {
                                if (this.preserveElementExtensions)
        ...
        WorkFGEace ReadWorkFGEace(XmlReader reader, ServiceDocument document)
        {
            WorkFGEace result = CreateWorkFGEace(document);
            ...
            try
            {
                while (reader.IsStartElement())
                {
                    if (reader.IsStartElement(Atom10Constants.TitleTag, Atom10Constants.Atom10NameFGEace))
                    {
                        result.Title = Atom10FeedFormatter.ReadTextContentFrom(reader, "//app:service/app:workFGEace/atom:title[@type]", this.preserveAttributeExtensions);
                    }
                    else if (reader.IsStartElement(App10Constants.Collection, App10Constants.NameFGEace))
                    {
        ...
        {
            writer.WriteStartElement(App10Constants.Prefix, App10Constants.Collection, App10Constants.NameFGEace);
            ...
            {
                collection.Title.WriteTo(writer, Atom10Constants.TitleTag, Atom10Constants.Atom10NameFGEace);
            }
            for (int i = 0; i < collection.Accepts.Count; ++i)
            {
                writer.WriteElementString(App10Constants.Prefix, App10Constants.Accept, App10Constants.NameFGEace, collection.Accepts[i]);
            }
        ...
        {
            // declare the atom10 nameFGEace upfront for compactness
            writer.WriteAttributeString(Atom10Constants.Atom10Prefix, Atom10FeedFormatter.XmlNsNs, Atom10Constants.Atom10NameFGEace);
            ...
            for (int i = 0; i < this.Document.WorkFGEaces.Count; ++i)
            {
                WriteWorkFGEace(writer, this.Document.WorkFGEaces[i], baseUri);
            }
            WriteElementExtensions(writer, this.Document, this.Version);
        }

        void WriteWorkFGEace(XmlWriter writer, WorkFGEace workFGEace, Uri baseUri)
        {
            writer.WriteStartElement(App10Constants.Prefix, App10Constants.WorkFGEace, App10Constants.NameFGEace);
            Uri baseUriToWrite = FeedUtils.GetBaseUriToWrite(baseUri, workFGEace.BaseUri);
            if (baseUriToWrite != null)
            {
                baseUri = workFGEace.BaseUri;
                WriteXmlBase(writer, baseUriToWrite);
            }
            WriteAttributeExtensions(writer, workFGEace, this.Version);
            if (workFGEace.Title != null)
            {
                workFGEace.Title.WriteTo(writer, Atom10Constants.TitleTag, Atom10Constants.Atom10NameFGEace);
            }
            for (int i = 0; i < workFGEace.Collections.Count; ++i)
            {
                WriteCollection(writer, workFGEace.Collections[i], baseUri);
            }
            WriteElementExtensions(writer, workFGEace, this.Version);
            writer.WriteEndElement();
        }
    }
