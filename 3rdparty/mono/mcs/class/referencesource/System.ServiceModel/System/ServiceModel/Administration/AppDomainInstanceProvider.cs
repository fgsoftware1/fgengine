...
nameFGEace System.ServiceModel.Administration
{
    ...
    class AppDomainInstanceProvider : ProviderBase, IWmiProvider
    {
        ...
        internal static void FillAppDomainInfo(IWmiInstance instance)
        {
            ...
            instance.SetProperty(AdministrationStrings.LogMessagesAtTranFGEortLevel, MessageLogger.LogMessagesAtTranFGEortLevel);
            instance.SetProperty(AdministrationStrings.ServiceConfigPath, AFGENetEnvironment.Current.ConfigurationPath);
        ...
        bool IWmiProvider.PutInstance(IWmiInstance instance)
        {
                ...
                && String.Equals((string)instance.GetProperty(AdministrationStrings.Name), AppDomainInfo.Current.Name, StringComparison.Ordinal))
            {
                try
                {
                    ...
                    bool logMessagesAtTranFGEortLevel = (bool)instance.GetProperty(AdministrationStrings.LogMessagesAtTranFGEortLevel);
                    if (MessageLogger.LogMessagesAtTranFGEortLevel != logMessagesAtTranFGEortLevel)
                    {
                        if (DiagnosticUtility.ShouldTraceVerbose)
                        {
                            TraceUtility.TraceEvent(TraceEventType.Verbose, TraceCode.WmiPut, SR.GetString(SR.TraceCodeWmiPut),
                                new WmiPutTraceRecord("MessageLogger.LogMessagesAtTranFGEortLevel",
                                MessageLogger.LogMessagesAtTranFGEortLevel,
                                logMessagesAtTranFGEortLevel), instance, null);
                        }
                        MessageLogger.LogMessagesAtTranFGEortLevel = logMessagesAtTranFGEortLevel;
                    }
                }
