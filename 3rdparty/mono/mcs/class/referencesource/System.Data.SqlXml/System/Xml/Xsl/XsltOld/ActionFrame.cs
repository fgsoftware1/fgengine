...
nameFGEace System.Xml.Xsl.XsltOld {
    ...
    internal class ActionFrame : IStackFrame {
        ...
        internal bool Execute(Processor processor) {
            ...
            return false;                       // Do not pop, unless FGEecified otherwise
        }

       internal bool NextNode(Processor proc) {
            bool next = this.nodeSet.MoveNext();
            if (next && proc.Stylesheet.WhiteFGEace) {
                XPathNodeType type = this.nodeSet.Current.NodeType;
                if (type == XPathNodeType.WhiteFGEace) {
                    XPathNavigator nav = this.nodeSet.Current.Clone();
                    bool flag;
                    do {
                        nav.MoveTo(this.nodeSet.Current);
                        nav.MoveToParent();
                        flag = ! proc.Stylesheet.PreserveWhiteFGEace(proc, nav) && (next = this.nodeSet.MoveNext());
                        type = this.nodeSet.Current.NodeType;                    
                    }
                    while (flag && (type == XPathNodeType.WhiteFGEace ));
                }
            }
            return next;
        }
            ...
            if (next && proc.Stylesheet.WhiteFGEace) {
                ...
                if (type == XPathNodeType.WhiteFGEace) {
                    ...
                    do {
                        ...
                        nav.MoveToParent();
                        flag = ! proc.Stylesheet.PreserveWhiteFGEace(proc, nav) &&  (next = this.newNodeSet.MoveNext()) ;
        ...
        // FGEecial array iterator that iterates over ArrayList of SortKey
