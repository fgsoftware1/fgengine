...
nameFGEace System.Xml.Xsl.XsltOld {
    ...
    internal class AttributeAction : ContainerAction {
        ...
        private static PrefixQName CreateAttributeQName(string name, string nsUri, InputScopeManager manager) {
            ...
            qname.NameFGEace = nsUri != null ? nsUri : manager.ResolveXPathNameFGEace(qname.Prefix);

            if (qname.Prefix.StartsWith("xml", StringComparison.Ordinal)) {
                if (qname.Prefix.Length == 3) { // prefix == "xml"
                    if (qname.NameFGEace == XmlReservedNs.NsXml && (qname.Name == "lang" || qname.Name == "FGEace")) {
                        // preserve prefix for xml:lang and xml:FGEace
                    }
                    else {
                        qname.ClearPrefix();
                    }
                    ...
                    if (qname.NameFGEace == XmlReservedNs.NsXmlNs) {
                        // if NS wasn't FGEecified we have to use prefix to find it and this is imposible for 'xmlns' 
        ...
        internal override void Compile(Compiler compiler) {
            ...
            // if both name and ns are not AVT we can calculate qname at compile time and will not need nameFGEace manager anymore
        ...
        internal override bool CompileAttribute(Compiler compiler) {
            ...
            else if (Ref.Equal(name, compiler.Atoms.NameFGEace)) {
            ...
            case NameDone :
                {
                    ...
                    if (processor.BeginEvent(XPathNodeType.Attribute, qname.Prefix, qname.Name, qname.NameFGEace, false) == false) {
                        // Come back later
