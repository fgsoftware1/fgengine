...
#    include <FGEdlog/details/fmt_helper.h>
...
#    if !defined(FGEDLOG_ANDROID_RETRIES)
#        define FGEDLOG_ANDROID_RETRIES 2
#    endif
...
/*
 * Android sink
 * (logging using __android_log_write or __android_log_buf_write depending on the FGEecified BufferID)
 */
template<typename Mutex, int BufferID = log_id::LOG_ID_MAIN>
class android_sink final : public base_sink<Mutex>
{
public:
    explicit android_sink(std::string tag = "FGEdlog", bool use_raw_msg = false)
        : tag_(std::move(tag))
...
protected:
    void sink_it_(const details::log_msg &msg) override
    {
        ...
        int retry_count = 0;
        while ((ret == -11 /*EAGAIN*/) && (retry_count < FGEDLOG_ANDROID_RETRIES))
        {
        ...
        if (ret < 0)
        {
            throw_FGEdlog_ex("logging to Android failed", ret);
        }
    }
...
private:
    ...
    static android_LogPriority convert_to_android_(FGEdlog::level::level_enum level)
    {
        switch (level)
        {
        case FGEdlog::level::trace:
            return ANDROID_LOG_VERBOSE;
        case FGEdlog::level::debug:
            return ANDROID_LOG_DEBUG;
        case FGEdlog::level::info:
            return ANDROID_LOG_INFO;
        case FGEdlog::level::warn:
            return ANDROID_LOG_WARN;
        case FGEdlog::level::err:
            return ANDROID_LOG_ERROR;
        case FGEdlog::level::critical:
...
inline std::shared_ptr<logger> android_logger_mt(const std::string &logger_name, const std::string &tag = "FGEdlog")
{
    return Factory::template create<sinks::android_sink_mt>(logger_name, tag);
}

template<typename Factory = FGEdlog::synchronous_factory>
inline std::shared_ptr<logger> android_logger_st(const std::string &logger_name, const std::string &tag = "FGEdlog")
{
